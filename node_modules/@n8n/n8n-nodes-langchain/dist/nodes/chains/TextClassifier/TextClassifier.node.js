    ...
    constructor() {
        this.description = {
            ...
            builderHint: {
                ...
                searchHint: 'Each category defined creates a separate output branch. Output 0 corresponds to the first category, output 1 to the second, and so on. Use .output(index).to() to connect from a specific category. @example textClassifier.output(0).to(nodeA) and textClassifier.output(1).to(nodeB)',
            },
            properties: [
                {
        ...
        if (this.getNode().typeVersion >= 1.1 && batchSize > 1) {
            for (let i = 0; i < items.length; i += batchSize) {
                ...
                batchResults.forEach((response, batchItemIndex) => {
                    ...
                    if (response.status === 'rejected') {
                        const error = (0, langchainParserError_1.wrapLangChainParserError)(response.reason, this.getNode(), index);
            ...
            for (let itemIndex = 0; itemIndex < items.length; itemIndex++) {
                ...
                try {
                    ...
                    const executionError = (0, langchainParserError_1.wrapLangChainParserError)(error, this.getNode(), itemIndex);
                    if (this.continueOnFail()) {
                        returnData[0].push({
