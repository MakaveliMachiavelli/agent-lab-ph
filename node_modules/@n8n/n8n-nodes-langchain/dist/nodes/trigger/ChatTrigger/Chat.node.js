...
function getToolFlowResponse(context, data) {
    const isToolExecution = (0, n8n_workflow_1.isToolType)(context.getNode().type);
    if (!isToolExecution)
        return data;
    const json = { ...data.json, sent: true };
    if (json.chatInput === '') {
...
class Chat {
        ...
        this.description = {
            ...
            properties: [
                ...
                },
                {
                    ...
                    placeholder: 'Add Option',
                    default: {},
                    options: [descriptions_1.limitWaitTimeOption, highlightedData_1.autoSaveHighlightedDataProperty],
                    displayOptions: {
                        show: {
                            '@tool': [true],
                            [`/${n8n_workflow_1.CHAT_WAIT_USER_REPLY}`]: [true],
    ...
    async onMessage(context, data) {
        ...
        if (!waitForReply) {
            if (nodeVersion >= 1.3)
                return [[getToolFlowResponse(context, data)]];
            const inputData = context.getInputData();
            return [inputData];
        }
        const message = data.json?.chatInput;
        if (context.getNodeParameter('options.autoSaveHighlightedData', 0, true) !== false) {
            context.customData.set((0, n8n_workflow_1.getHighlightedInputKey)(context.getNode().name), message);
        }
    ...
    async execute() {
        ...
        if (this.getNodeParameter('options.autoSaveHighlightedData', 0, true) !== false) {
            ...
            this.customData.set((0, n8n_workflow_1.getHighlightedResponseKey)(this.getNode().name), responseText);
        }
        if (options.memoryConnection) {
