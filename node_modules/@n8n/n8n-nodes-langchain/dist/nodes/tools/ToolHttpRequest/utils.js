...
const assertCredentialUrlAllowed = (ctx, credentials, options) => {
    if (!credentials)
        return;
    const allowedDomains = (0, n8n_workflow_1.assertCredentialAllowsUrl)({
        node: ctx.getNode(),
...
const genericCredentialRequest = async (ctx, itemIndex) => {
    ...
    if (genericType === 'httpTemplatedCustomAuth') {
        ...
        return async (options) => {
            ...
            return await ctx.helpers.httpRequestWithAuthentication.call(ctx, 'httpTemplatedCustomAuth', options);
        };
    }
...
const redactSecrets = (content) => content
    .replace(/\b(authorization)(["']?\s*[:=]\s*["']?\s*(?:(?:bearer|basic)\s+)?)[^\s"',;}]+/gi, `$1$2${SECRET_REDACTION}`)
    .replace(/([\w-]*(?:api[_-]?key|access[_-]?token|refresh[_-]?token|token|password|passwd|secret|credential|private[_-]?key))(["']?\s*[:=]\s*)(["']?)[^\s"',;}]+\3/gi, `$1$2$3${SECRET_REDACTION}$3`);
    ...
    return async (query) => {
        ...
        if (options) {
            try {
                ...
                const httpCode = error.httpCode ?? status;
                ...
                if (errorBody !== undefined && !message.includes(errorBody)) {
                    response += `\nResponse body: ${errorBody.slice(0, MAX_ERROR_BODY_LENGTH)}${errorBody.length > MAX_ERROR_BODY_LENGTH ? '... [truncated]' : ''}`;
                }
            }
            if (!response) {
                try {
