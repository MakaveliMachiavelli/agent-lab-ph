...
exports.createSandboxLogger = createSandboxLogger;
exports.createProductionConsoleLog = createProductionConsoleLog;
...
function isLoggerKey(level) {
    return level !== 'silent';
}
function createSandboxLogger(logger) {
    return n8n_workflow_1.LOG_LEVELS.filter(isLoggerKey).reduce((result, level) => {
        result[level] = (...args) => logger[level].apply(logger, args);
        return result;
    }, {});
}
function createProductionConsoleLog(workflowId, nodeName, redactConsole) {
    return (...args) => console.log(`[Workflow "${workflowId}"][Node "${nodeName}"]`, ...(redactConsole ? [n8n_workflow_1.CONSOLE_OUTPUT_REDACTED_MESSAGE] : args));
}
...
function getSandbox(code, options) {
    ...
    sandbox.on('output', workflowMode === 'manual'
        ...
        : createProductionConsoleLog(this.getWorkflow().id, node.name, this.isConsoleOutputRedacted()));
    return sandbox;
}
class Code {
    constructor() {
