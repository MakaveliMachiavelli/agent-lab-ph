...
exports.properties = [
    ...
    {
        ...(0, descriptions_1.modelRLC)('textToVideoModelSearch'),
        displayOptions: {
...
async function execute(itemIndex) {
    ...
    const model = nodeVersion >= 1.1
        ? this.getNodeParameter('modelId', itemIndex, '', { extractValue: true })
        : this.getNodeParameter('modelId', itemIndex);
    ...
    const shotType = this.getNodeParameter('shotType', itemIndex);
    const simplify = this.getNodeParameter('simplify', itemIndex, true);
    const downloadVideo = this.getNodeParameter('downloadVideo', itemIndex, true);
    const videoOptions = this.getNodeParameter('videoOptions', itemIndex, {});
    const body = {
        model,
        input: {
            prompt,
    ...
    if (videoOptions.promptExtend !== undefined) {
        body.parameters.prompt_extend = videoOptions.promptExtend;
    }
    if (videoOptions.audio !== undefined) {
        body.parameters.audio = videoOptions.audio;
    }
    const audioInputType = videoOptions.audioInputType;
    if (audioInputType === 'binary') {
        const binaryPropertyName = videoOptions.audioBinaryPropertyName || 'audio';
        ...
        body.input.audio_url = videoOptions.audioUrl;
    }
    const createResponse = await transport_1.apiRequest.call(this, 'POST', '/api/v1/services/aigc/video-generation/video-synthesis', {
        headers: {
    ...
    if (!taskId) {
        throw new n8n_workflow_1.NodeOperationError(this.getNode(), `Failed to create video generation task: ${createResponse?.message || 'No task_id returned'}`);
    }
    const result = await transport_1.pollTaskResult.call(this, taskId);
