...
function shouldPassthroughBinary(data, options) {
    if (isImageFile(data.mimeType))
        return options.passthroughBinaryImages === true;
    if (isPdfFile(data.mimeType))
        return options.passthroughBinaryPdfs === true;
    if (isTextFile(data.mimeType))
        return true;
    return false;
}
function resolveBinaryContentFormat(model) {
    if (!model)
        return 'standard';
    const candidate = model;
    try {
        const usesResponsesApi = typeof candidate._useResponsesApi === 'function'
...
async function processBinaryForAgentPassthrough(ctx, data, type, contentFormat = 'standard') {
    let base64Data;
    if (data.id) {
        const binaryBuffer = await ctx.helpers.binaryToBuffer(await ctx.helpers.getBinaryStream(data.id));
        base64Data = Buffer.from(binaryBuffer).toString(n8n_workflow_1.BINARY_ENCODING);
    }
    else {
        base64Data = data.data.includes('base64,') ? data.data.split('base64,')[1] : data.data;
    }
    const maxSizeInBytes = di_1.Container.get(config_1.AiConfig)?.maxAgentPassthroughBinarySizeBytes ??
    ...
    if (sizeInBytes > maxSizeInBytes) {
        ...
        throw new n8n_workflow_1.NodeOperationError(ctx.getNode(), `The file "${fileName}" is ${sizeInMb} MB, which exceeds the ${limitInMb} MB limit for passing binary data to the model`, {
            description: 'Reduce the file size, or disable the binary passthrough option for this input.',
        });
    }
    if (type === 'file') {
...
async function extractBinaryMessages(ctx, itemIndex, options, contentFormat = 'standard') {
