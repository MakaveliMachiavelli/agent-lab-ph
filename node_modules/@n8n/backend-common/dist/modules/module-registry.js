...
exports.ModuleRegistry = exports.getModuleEntryUrl = void 0;
...
const getModuleEntryUrl = (modulesDir, moduleName, isEnterprise = false) => (0, url_1.pathToFileURL)(path_1.default.join(modulesDir, isEnterprise ? `${moduleName}.ee` : moduleName, `${moduleName}.module.js`)).href;
exports.getModuleEntryUrl = getModuleEntryUrl;
let ModuleRegistry = class ModuleRegistry {
    ...
    async loadModules(modules) {
        ...
        for (const moduleName of modules ?? this.eligibleModules) {
            try {
                await import((0, exports.getModuleEntryUrl)(modulesDir, moduleName));
            }
            catch (primaryError) {
                try {
                    await import((0, exports.getModuleEntryUrl)(modulesDir, moduleName, true));
                }
                catch (error) {
                    const loggedError = primaryError instanceof Error &&
                        'code' in primaryError &&
                        primaryError.code !== 'MODULE_NOT_FOUND'
                        ? primaryError
                        : error;
                    throw new missing_module_error_1.MissingModuleError(moduleName, loggedError instanceof Error ? loggedError.message : '');
                }
            }
        }
        for (const ModuleClass of this.moduleMetadata.getClasses()) {
            ...
            const loaders = await di_1.Container.get(ModuleClass).nodeLoaders?.();
            if (loaders?.length)
                this.nodeLoaders.push(...loaders);
...
exports.ModuleRegistry = ModuleRegistry = __decorate([
    (0, di_1.Service)(),
    __metadata("design:paramtypes", [decorators_1.ModuleMetadata, license_state_1.LicenseState, logger_1.Logger, modules_config_1.ModulesConfig])
], ModuleRegistry);
