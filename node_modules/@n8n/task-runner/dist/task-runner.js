...
class TaskRunner extends node_events_1.EventEmitter {
    constructor(opts) {
        ...
        this.id = opts.runnerId || (0, nanoid_1.nanoid)();
        ...
        this.ws.addEventListener('error', (event) => {
            const error = (0, ensure_error_1.ensureError)(event.error);
            if ('code' in error &&
    ...
    async executeTask(_taskParams, _signal) {
        throw new n8n_workflow_1.UnexpectedError('Unimplemented');
    }
    ...
    releaseBrokerDrainWait() {
        if (this.brokerDrainResolve) {
            this.brokerDrainResolve();
            this.brokerDrainResolve = undefined;
        }
    }
    async waitForBrokerDrain(timeoutMs) {
        if (this.isDraining || timeoutMs <= 0)
            return;
    ...
    async stop({ deferToBrokerDrain = true } = {}) {
        ...
        try {
            ...
            console.error((0, ensure_error_1.ensureError)(error).message);
        }
    ...
    async waitUntilAllTasksAreDone(maxWaitTimeInMs = 30_000) {
        ...
        while (this.runningTasks.size > 0) {
            if (Date.now() - start > maxWaitTimeInMs) {
                throw new n8n_workflow_1.OperationalError('Timeout while waiting for tasks to finish');
            }
