...
class ClientOAuth2 {
    ...
    async accessTokenRequest(options) {
        ...
        if (ssrfBridge) {
            const parsed = (0, utils_1.tryParseUrl)(url);
            if (parsed) {
        ...
        const lookup = (0, proxy_1.resolveProxyUrl)(url) ? undefined : ssrfBridge?.createSecureLookup();
        if (options.ignoreSSLIssues || lookup) {
            requestConfig.httpsAgent = (0, proxy_1.createHttpsProxyAgent)(url, undefined, {
                ...(options.ignoreSSLIssues ? { rejectUnauthorized: false } : {}),
                ...(lookup ? { lookup } : {}),
            });
        }
        if (lookup) {
            requestConfig.httpAgent = (0, proxy_1.createHttpProxyAgent)(url, undefined, { lookup });
        }
    ...
    parseResponseBody(response) {
        ...
        try {
            ...
            throw new ResponseError(response.status, body, undefined, `Expected JSON response from OAuth2 token endpoint (content-type: ${contentType || 'none'}) but received: ${preview}`);
        }
    }
}
