...
exports.DEFAULT_OBSERVATION_LOG_OBSERVER_THRESHOLD_TOKENS = 8_000;
exports.DEFAULT_OBSERVATION_LOG_TAIL_LIMIT = 20;
exports.DEFAULT_OBSERVATION_LOG_REFLECTOR_THRESHOLD_TOKENS = 12_000;
exports.DEFAULT_OBSERVATION_LOG_RENDER_TOKEN_BUDGET = 13_500;
...
function createObservationLogObserveFn(model, options = {}) {
    return async (input) => {
        const { text, usage, providerMetadata } = await (0, lazy_ai_1.loadAi)().generateText({
            model: (0, model_factory_1.createModel)(model),
            instructions: options.observerPrompt ?? exports.DEFAULT_OBSERVATION_LOG_OBSERVER_PROMPT,
            prompt: buildObservationLogObserverPrompt(input),
            ...(0, telemetry_options_1.buildAiSdkTelemetry)(input.telemetry, { functionSuffix: 'memory-observer' }),
        });
        (0, execution_counter_1.incrementTokenCountFromUsage)(input.executionCounter, usage);
        if (options.onUsage) {
            const tokenUsage = (0, stream_1.toTokenUsage)(usage, providerMetadata);
            if (tokenUsage) {
                await options.onUsage({
                    task: 'observer',
                    model: (0, model_1.getModelIdString)(model),
                    usage: tokenUsage,
...
function createObservationLogReflectFn(model, options = {}) {
    return async (input) => {
        const { text, usage, providerMetadata } = await (0, lazy_ai_1.loadAi)().generateText({
            model: (0, model_factory_1.createModel)(model),
            instructions: options.reflectorPrompt ?? exports.DEFAULT_OBSERVATION_LOG_REFLECTOR_PROMPT,
            prompt: buildObservationLogReflectorPrompt(input),
            ...(0, telemetry_options_1.buildAiSdkTelemetry)(input.telemetry, { functionSuffix: 'memory-reflector' }),
        });
        (0, execution_counter_1.incrementTokenCountFromUsage)(input.executionCounter, usage);
        if (options.onUsage) {
            const tokenUsage = (0, stream_1.toTokenUsage)(usage, providerMetadata);
