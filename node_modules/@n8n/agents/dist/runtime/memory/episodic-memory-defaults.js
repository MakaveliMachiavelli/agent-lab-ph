...
function createEpisodicMemoryExtractFn(model, options = {}) {
    return async (input) => {
        ...
        const response = await generateText({
            ...
            instructions: options.extractionPrompt ?? exports.DEFAULT_EPISODIC_MEMORY_EXTRACTION_PROMPT,
            prompt: buildEpisodicMemoryExtractorPrompt(input),
            output: Output.object({ schema: EpisodicMemoryExtractionSchema }),
        });
        (0, execution_counter_1.incrementTokenCountFromUsage)(input.executionCounter, response.usage);
...
function createEpisodicMemoryReflectFn(model, options = {}) {
    return async (input) => {
        ...
        const response = await generateText({
            model: (0, model_factory_1.createModel)(model),
            instructions: options.reflectionPrompt ?? exports.DEFAULT_EPISODIC_MEMORY_REFLECTION_PROMPT,
