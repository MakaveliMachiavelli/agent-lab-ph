...
exports.resolveProxyOption = resolveProxyOption;
...
exports.isProxyPotentiallyActive = isProxyPotentiallyActive;
...
exports.validateProxySsrf = validateProxySsrf;
exports.resolveLegacyRequestTarget = resolveLegacyRequestTarget;
...
const proxy_resolution_1 = require("../../proxy/proxy-resolution");
...
function sniFor(host) {
    ...
    if (node_net_1.default.isIP(unbracketed)) {
        return undefined;
    }
    return host;
}
...
function resolveProxyOption(customProxyUrl) {
    if (!customProxyUrl) {
        return 'env';
    }
    if ((0, node_agents_1.isSupportedProxyUrl)(customProxyUrl)) {
        return customProxyUrl;
    }
    di_1.Container.get(backend_common_1.Logger).warn(`Ignoring unsupported proxy URL "${customProxyUrl}", falling back to environment proxy settings`);
    return 'env';
}
const getBeforeRedirectFn = (agentOptions, axiosConfig, proxyConfig, sendCredentialsOnCrossOriginRedirect, allowedDomains, ssrf = 'disabled') => (redirectedRequest) => {
    ...
    const redirectAgentOptions = {
        ...agentOptions,
        servername: sniFor(redirectedRequest.hostname),
    };
    ...
    const { httpAgent, httpsAgent } = (0, node_agents_1.buildNodeAgents)(proxy, ssrf, redirectAgentOptions);
...
function buildAgentOptions(n8nRequest) {
    const servername = sniFor((0, exports.getHostFromRequestObject)(n8nRequest));
...
function isProxyPotentiallyActive(proxyConfig) {
    const configSupported = (0, node_agents_1.isSupportedProxyUrl)(getUrlFromProxyConfig(proxyConfig));
    return configSupported || (0, proxy_resolution_1.hasProxyEnvironmentVariables)();
}
async function generateContentLengthHeader(config) {
    ...
    try {
        const length = await new Promise((res, rej) => {
            formData.getLength((error, dataLength) => {
                if (error)
                    rej(error);
                else
                    res(dataLength);
...
function setAxiosAgents(config, agentOptions, proxyConfig, ssrf = 'disabled') {
    ...
    const customProxyUrl = proxyConfig ? getUrlFromProxyConfig(proxyConfig) : null;
    const proxy = resolveProxyOption(customProxyUrl);
    const { httpAgent, httpsAgent } = (0, node_agents_1.buildNodeAgents)(proxy, ssrf, agentOptions);
...
async function validateProxySsrf(proxyConfig, ssrfBridge) {
    const proxyUrl = getUrlFromProxyConfig(proxyConfig);
    if (!(0, node_agents_1.isSupportedProxyUrl)(proxyUrl))
        return;
    await validateUrlSsrf(proxyUrl, ssrfBridge);
}
function resolveLegacyRequestTarget(requestObject) {
