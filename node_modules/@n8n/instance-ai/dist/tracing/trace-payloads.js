...
exports.sanitizeTracePayload = sanitizeTracePayload;
exports.rawTracePayload = rawTracePayload;
...
exports.GEN_AI_PROMPT = 'gen_ai.prompt';
exports.GEN_AI_COMPLETION = 'gen_ai.completion';
...
function toTelemetryMetadata(...records) {
    const metadata = {};
...
function redactTelemetryJsonValue(value, keyHint, depth = 0, maxDepth = MAX_TRACE_DEPTH) {
    if (depth > maxDepth) {
...
function maxRedactionDepthForAttribute(key) {
    if (key === 'ai.prompt.messages' || key === exports.GEN_AI_PROMPT) {
        return MAX_PROMPT_SCHEMA_TRACE_DEPTH;
    }
    if (key === 'ai.toolCall.args' || key === 'ai.toolCall.result' || key === exports.GEN_AI_COMPLETION) {
        return MAX_TOOL_IO_TRACE_DEPTH;
    }
    return MAX_TRACE_DEPTH;
}
function redactTelemetryAttribute(key, value, completionDepth) {
    ...
    const maxDepth = key === exports.GEN_AI_COMPLETION && completionDepth !== undefined
        ? completionDepth
...
function parseTelemetryTools(value) {
    if (!Array.isArray(value)) {
...
function readToolCallPayload(part) {
    if ('input' in part)
        return part.input;
    if ('args' in part)
        return part.args;
...
function readToolResultPayload(part) {
    if ('output' in part)
        return part.output;
    if ('result' in part)
        return part.result;
...
function normalizeAssistantMessageForLangSmith(message) {
    ...
    for (const part of content) {
        if (!(0, is_record_1.isRecord)(part))
            continue;
...
function normalizeToolMessageForLangSmith(message) {
    const content = message.content;
    if (!Array.isArray(content)) {
        return [message];
    }
...
function normalizeMessagesForLangSmith(messages) {
    ...
    for (const message of messages) {
        if (!(0, is_record_1.isRecord)(message) || typeof message.role !== 'string') {
...
function toolNameFromDefinition(tool) {
    if (!(0, is_record_1.isRecord)(tool))
        return undefined;
    if (typeof tool.name === 'string')
        return tool.name;
    if (typeof tool.id === 'string')
        return tool.id;
    if ((0, is_record_1.isRecord)(tool.function) && typeof tool.function.name === 'string') {
        return tool.function.name;
    }
    return undefined;
}
function enrichLangSmithToolAttributes(attributes) {
    const tools = parseTelemetryTools(attributes['ai.prompt.tools']);
    if (!tools) {
        return undefined;
    }
    const redactedTools = redactTelemetryJsonValue(tools, undefined, 0, MAX_PROMPT_SCHEMA_TRACE_DEPTH);
    ...
    if (toolChoice !== undefined) {
        const redactedToolChoice = redactTelemetryJsonValue(toolChoice, undefined, 0, MAX_PROMPT_SCHEMA_TRACE_DEPTH);
...
function enrichLangSmithPromptAttribute(attributes) {
    ...
    const prompt = { input: normalizeMessagesForLangSmith(messages) };
    if (tools) {
        prompt.tools = tools;
    }
    if (toolChoice !== undefined) {
    ...
    attributes['gen_ai.prompt'] = JSON.stringify(redactTelemetryJsonValue(prompt, undefined, 0, MAX_PROMPT_SCHEMA_TRACE_DEPTH));
}
...
function readTokenDetail(details, keys) {
    ...
    if (!(0, is_record_1.isRecord)(parsedDetails)) {
        return undefined;
    }
...
function readNestedRecord(record, keys) {
    let current = record;
    for (const key of keys) {
        if (!(0, is_record_1.isRecord)(current)) {
            return {};
        }
        current = current[key];
    }
    return (0, is_record_1.isRecord)(current) ? current : {};
}
function readProviderAnthropicUsage(attributes) {
    ...
    return (0, is_record_1.isRecord)(providerMetadata)
        ? readNestedRecord(providerMetadata, ['anthropic', 'usage'])
        : {};
}
function firstNumberAttribute(attributes, keys) {
    for (const key of keys) {
...
function calculateInputTokenAccounting(inputTokens, cacheReadTokens, cacheCreationTokens) {
    const cachedInputTokens = cacheReadTokens + cacheCreationTokens;
    const observedInputTokens = inputTokens ?? 0;
    const regularInputTokens = observedInputTokens >= cachedInputTokens
...
function normalizeUsageForLangSmith(attributes) {
    ...
    const inputTokenDetails = (0, is_record_1.isRecord)(usageMetadata.input_token_details)
...
function inferNativeLlmRole(attributes) {
...
function displayNameForNativeLlmSpan(attributes) {
    const suffixedFunctionId = readStringAttribute(attributes, [
        'ai.telemetry.functionId',
        'resource.name',
    ]);
    if (suffixedFunctionId?.endsWith('.memory-observer'))
        return 'llm: memory observer';
    if (suffixedFunctionId?.endsWith('.memory-reflector'))
        return 'llm: memory reflector';
    const role = inferNativeLlmRole(attributes);
    if (role === 'thread_title') {
        return 'llm: title';
    }
    if (role) {
        return `llm: ${(0, trace_labels_1.formatAgentRoleLabel)(role)}`;
    }
    ...
    if (functionId) {
        return `llm: ${(0, trace_labels_1.formatAgentRoleLabel)(functionId.replace(/^instance-ai[._-]?/, ''))}`;
    }
    return 'llm';
}
function parseTelemetryObject(value) {
    const parsed = typeof value === 'string' ? parseTelemetryJson(value) : value;
    return (0, is_record_1.isRecord)(parsed) ? parsed : undefined;
}
...
function toolActionForNativeToolSpan(attributes) {
...
function setLangSmithMetadataAttribute(attributes, key, value) {
    attributes[key] = value;
    if (!key.startsWith('langsmith.metadata.')) {
        attributes[`langsmith.metadata.${key}`] = value;
    }
}
function renameNativeLlmSpanForLangSmith(span, attributes) {
    ...
    setLangSmithMetadataAttribute(attributes, 'display_group', displayGroup ? (0, trace_labels_1.formatAgentRoleLabel)(displayGroup) : 'llm');
    ...
    if (executionMode) {
        setLangSmithMetadataAttribute(attributes, 'display_phase', (0, trace_labels_1.formatTraceLabel)(executionMode));
    }
...
function renameNativeToolSpanForLangSmith(span, attributes) {
...
function isLangSmithLlmSpan(attributes) {
    const operationId = readStringAttribute(attributes, [AI_OPERATION_ID]);
    return (attributes[LANGSMITH_SPAN_KIND] === 'llm' ||
        attributes.display_kind === 'llm' ||
        (typeof operationId === 'string' && LLM_AI_SDK_OPERATION_IDS.has(operationId)));
}
function isCountedUsageAttribute(key) {
    return (key === LANGSMITH_USAGE_METADATA ||
        key === 'usage_metadata' ||
        key === 'prompt_tokens' ||
...
function neutralUsageAttributeKey(key) {
    return `instance_ai.usage.${key}`;
}
function moveNonLlmUsageAttributes(attributes) {
...
function redactLangSmithTelemetrySpan(span) {
    if (!(0, is_record_1.isRecord)(span) || !(0, is_record_1.isRecord)(span.attributes)) {
        return span;
    }
    const completionDepth = span.attributes['langsmith.metadata.raw_trace_payload'] === true
...
function classifyToolSource(name, toolRecord) {
    ...
    if (tool_ids_1.ORCHESTRATION_TOOL_NAMES.has(name)) {
...
function classifyToolCategory(name) {
    ...
    if (name === tool_ids_1.DOMAIN_TOOL_IDS.NODES || name === 'materialize-node-type')
        return 'node';
    if (name === tool_ids_1.DOMAIN_TOOL_IDS.EXECUTIONS)
        return 'execution';
    if (name.includes('research'))
        return 'research';
    if (name.includes('plan') || name === tool_ids_1.ORCHESTRATION_TOOL_IDS.CREATE_TASKS) {
        return 'planning';
    }
    if (name.startsWith('workspace_'))
        return 'workspace';
...
function classifyToolSideEffect(name) {
    if (name.includes('browser'))
        return 'browser';
    if (name.includes('research'))
        return 'network';
    if (name === tool_ids_1.DOMAIN_TOOL_IDS.EXECUTIONS || name.includes('execute') || name.includes('run')) {
        return 'execute';
    }
    ...
    if (name.includes(tool_ids_1.DOMAIN_TOOL_IDS.ASK_USER) || name.includes('pause-for-user'))
        return 'none';
    return 'read';
}
function getToolInputSchema(tool) {
    ...
    const toJSONSchema = (0, is_record_1.isRecord)(inputSchema) ? inputSchema.toJSONSchema : undefined;
    ...
    if ((0, is_record_1.isRecord)(inputSchema) && typeof inputSchema.safeParse === 'function') {
        return { type: 'zod' };
    }
    return inputSchema;
}
function summarizeToolForManifest(name, tool) {
    ...
    const toolRecord = (0, is_record_1.isRecord)(tool) ? tool : {};
    const description = summarizeToolDescription(tool);
    const providerOptions = (0, is_record_1.isRecord)(toolRecord.providerOptions)
...
function summarizeMemoryBinding(memory) {
    if (!(0, is_record_1.isRecord)(memory)) {
        return {};
    }
    return {
        memory_enabled: true,
...
function summarizeRuntimeSkillRegistry(registry) {
    if (!registry || registry.skills.length === 0) {
        return {};
    }
    const categories = Array.from(new Set(registry.skills
        .map((skill) => skill.category)
        .filter((category) => typeof category === 'string'))).sort();
    return {
...
function rawTracePayload(value) {
    if ((0, is_record_1.isRecord)(value)) {
        return value;
    }
    if (value === undefined) {
...
function serializeModelIdForTrace(modelId) {
    ...
    if ((0, is_record_1.isRecord)(modelId) && typeof modelId.id === 'string') {
        return truncateString(modelId.id);
    }
    return sanitizeTraceValue(modelId);
}
function mergeTraceInputs(baseInputs, inputs) {
    const existingInputs = (0, is_record_1.isRecord)(baseInputs) && !Array.isArray(baseInputs) ? { ...baseInputs } : {};
    return {
        ...existingInputs,
        ...(inputs ?? {}),
    };
...
//# sourceMappingURL=trace-payloads.js.map