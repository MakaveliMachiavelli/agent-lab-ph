...
function formatSourceFileInstruction(sourceFilePath) {
    if (!sourceFilePath) {
...
function formatWorkflowLoopGuidance(action, options = {}) {
    switch (action.type) {
        ...
        case 'verify':
            return (`VERIFY: Inspect the persisted workflow ${action.workflowId} with \`workflows(action="get-as-code", workflowId)\` or read the bound workspace source file, then compare it to the requested outcome. ` +
        ...
        case 'rebuild':
            return (`REBUILD NEEDED: Workflow "${action.workflowId}" needs structural repair. ` +
