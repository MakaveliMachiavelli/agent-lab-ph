...
const SENSITIVE_KEY_PATTERN = '(?:api[_-]?key|access[_-]?token|refresh[_-]?token|id[_-]?token|session[_-]?token|client[_-]?secret|private[_-]?key|token|password|passwd|secret|credentials?)';
const URL_PATTERN = /\bhttps?:\/\/[^\s"'<>]+/gi;
const QUOTED_SECRET_PATTERN = new RegExp(`(["'])(${SENSITIVE_KEY_PATTERN})\\1\\s*:\\s*(["'])([^"']*)(\\3)`, 'gi');
const KEY_VALUE_SECRET_PATTERN = new RegExp(`\\b(${SENSITIVE_KEY_PATTERN})\\b(\\s*[:=]\\s*)(["']?)([^\\s"',;&}]+)(\\3)`, 'gi');
const AUTHORIZATION_PATTERN = /\b(authorization)(\s*[:=]\s*)(["']?)(?:(Bearer|Basic)\s+)?[^\s"',;&}]+(\3)/gi;
...
function getNumberProperty(value, keys) {
    if (!(0, is_record_1.isRecord)(value))
        return undefined;
    for (const key of keys) {
        const candidate = Reflect.get(value, key);
...
function truncate(value, maxLength = MAX_LOG_ERROR_LENGTH) {
...
function sanitizeForLog(value) {
    return value
        ...
        .replace(AUTHORIZATION_PATTERN, (_match, key, separator, quote, scheme) => `${key}${separator}${quote}${scheme ? `${scheme} ` : ''}[REDACTED]${quote}`)
        .replace(QUOTED_SECRET_PATTERN, '$1$2$1: $3[REDACTED]$5')
        .replace(KEY_VALUE_SECRET_PATTERN, '$1$2$3[REDACTED]$5');
}
function takeSanitizedLogSample(value) {
    const trimmed = value.trimStart();
    const sanitizable = trimmed.length > MAX_LOG_ERROR_SANITIZE_INPUT_LENGTH
        ? trimmed.slice(0, MAX_LOG_ERROR_SANITIZE_INPUT_LENGTH)
        : trimmed;
    const sanitized = sanitizeForLog(sanitizable);
    const sample = sanitized.length > MAX_LOG_ERROR_INPUT_LENGTH
...
function extractHtmlTagText(value, tag) {
...
function extractCloudflareHost(value, text) {
...
function appendHttpContext(message, error) {
...
function formatErrorForLog(error) {
    const message = getStringProperty(error, ['responseBody', 'body', 'data']) ??
