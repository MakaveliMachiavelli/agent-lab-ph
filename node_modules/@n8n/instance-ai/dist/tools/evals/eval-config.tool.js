"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.EVAL_CONFIG_TOOL_ID = void 0;
exports.createEvalConfigTool = createEvalConfigTool;
const agents_1 = require("@n8n/agents");
const api_types_1 = require("@n8n/api-types");
const nanoid_1 = require("nanoid");
const zod_1 = require("zod");
const sanitize_mcp_schemas_1 = require("../../agent/sanitize-mcp-schemas");
const tool_ids_1 = require("../tool-ids");
Object.defineProperty(exports, "EVAL_CONFIG_TOOL_ID", { enumerable: true, get: function () { return tool_ids_1.EVAL_CONFIG_TOOL_ID; } });
const metricInputSchema = zod_1.z.object({
    name: zod_1.z.string().min(1).describe('Human-readable metric name, e.g. "Correctness"'),
    preset: zod_1.z
        .enum(['correctness', 'helpfulness'])
        .describe('Judged metric preset. "correctness" compares the answer to an expected answer (requires expectedAnswer); "helpfulness" judges the answer against the user query (requires userQuery).'),
    provider: zod_1.z
        .enum(api_types_1.LLM_JUDGE_PROVIDER_NODE_TYPES)
        .optional()
        .describe('Optional. The LLM judge chat-model node type (e.g. "@n8n/n8n-nodes-langchain.lmChatOpenAi"). ' +
        'Usually omit this — it is derived from credentialId (each credential type maps to one provider). ' +
        'Only set it if you know the exact node type.'),
    credentialId: zod_1.z
        .string()
        .min(1)
        .describe('Credential id for the LLM judge model; this also determines the provider'),
    model: zod_1.z.string().min(1).describe('LLM judge model name, e.g. "gpt-4o"'),
    outputType: zod_1.z
        .enum(['numeric', 'boolean'])
        .default('numeric')
        .describe('Score type the judge returns'),
    actualAnswer: zod_1.z
        .string()
        .min(1)
        .describe("n8n expression for the workflow's produced answer (read from the workflow output). " +
        'Must begin with `=` so it is stored as an expression, not literal text, e.g. ={{ $json.output }}'),
    userQuery: zod_1.z
        .string()
        .optional()
        .describe('n8n expression for the user query — a dataset column (required for the "helpfulness" preset). ' +
        'Write it as `={{ $json.<column> }}` (the eval reads it from the dataset row); do not reference the trigger by name. Must begin with `=`, e.g. ={{ $json.input }}'),
    expectedAnswer: zod_1.z
        .string()
        .optional()
        .describe('n8n expression for the expected/ground-truth answer — a dataset column (required for the "correctness" preset). ' +
        'Write it as `={{ $json.<column> }}` (the eval reads it from the dataset row); do not reference the trigger by name. Must begin with `=`, e.g. ={{ $json.expected_output }}'),
    prompt: zod_1.z.string().optional().describe('Optional override for the judge prompt'),
});
const configFields = {
    name: zod_1.z.string().min(1).describe('Evaluation name'),
    startNodeName: zod_1.z
        .string()
        .min(1)
        .describe('Name of the node where the evaluation run begins (fed the test input). Must be a node with an incoming connection — NOT a trigger. An eval run replaces the trigger with a dataset-driven one, so a trigger as the start node fails to compile; use the first node after the trigger (often the same as endNodeName for a single-agent workflow).'),
    endNodeName: zod_1.z.string().min(1).describe('Name of the node whose output is judged'),
    dataTableId: zod_1.z
        .string()
        .min(1)
        .describe('Id of the Data Table holding the test dataset (create/populate it via data-tables first)'),
    metrics: zod_1.z.array(metricInputSchema).min(1).describe('One or more judged metrics'),
};
const listAction = zod_1.z.object({
    action: zod_1.z.literal('list').describe('List config-based evaluations on a workflow'),
    workflowId: zod_1.z.string().min(1),
});
const getAction = zod_1.z.object({
    action: zod_1.z
        .literal('get')
        .describe('Get a single config-based evaluation (summary: metric names and types only)'),
    workflowId: zod_1.z.string().min(1),
    configId: zod_1.z.string().min(1),
});
const describeAction = zod_1.z.object({
    action: zod_1.z
        .literal('describe')
        .describe('Get a config-based evaluation with full metric detail — the expression strings ' +
        '(actualAnswer/userQuery/expectedAnswer), judge model, and prompt that "get" omits. ' +
        'Use this before "update": an update replaces the whole config, so you must read the ' +
        'current values first and resend the ones you are not changing.'),
    workflowId: zod_1.z.string().min(1),
    configId: zod_1.z.string().min(1),
});
const createAction = zod_1.z.object({
    action: zod_1.z.literal('create').describe('Create a config-based evaluation on a workflow'),
    workflowId: zod_1.z.string().min(1),
    ...configFields,
});
const updateAction = zod_1.z.object({
    action: zod_1.z.literal('update').describe('Update an existing config-based evaluation'),
    workflowId: zod_1.z.string().min(1),
    configId: zod_1.z.string().min(1),
    ...configFields,
});
const deleteAction = zod_1.z.object({
    action: zod_1.z.literal('delete').describe('Delete a config-based evaluation'),
    workflowId: zod_1.z.string().min(1),
    configId: zod_1.z.string().min(1),
});
const allActions = [
    listAction,
    getAction,
    describeAction,
    createAction,
    updateAction,
    deleteAction,
];
const confirmationSuspendSchema = zod_1.z.object({
    requestId: zod_1.z.string(),
    message: zod_1.z.string(),
    severity: api_types_1.instanceAiConfirmationSeveritySchema,
});
const confirmationResumeSchema = zod_1.z.object({
    approved: zod_1.z.boolean(),
});
const UNAVAILABLE = {
    error: 'Config-based evaluations are not available on this instance.',
};
function toUpsertInput(input) {
    return {
        name: input.name,
        startNodeName: input.startNodeName,
        endNodeName: input.endNodeName,
        dataTableId: input.dataTableId,
        metrics: input.metrics,
    };
}
function errorMessage(error) {
    return error instanceof Error ? error.message : String(error);
}
async function handleList(context, input) {
    const service = context.evaluationConfigService;
    if (!service)
        return UNAVAILABLE;
    const configs = await service.list(input.workflowId);
    return { configs };
}
async function handleGet(context, input) {
    const service = context.evaluationConfigService;
    if (!service)
        return UNAVAILABLE;
    const config = await service.get(input.workflowId, input.configId);
    if (!config)
        return { error: `Evaluation config "${input.configId}" not found` };
    return { config };
}
async function handleDescribe(context, input) {
    const service = context.evaluationConfigService;
    if (!service)
        return UNAVAILABLE;
    const config = await service.describe(input.workflowId, input.configId);
    if (!config)
        return { error: `Evaluation config "${input.configId}" not found` };
    return { config };
}
async function handleCreate(context, input, ctx) {
    const service = context.evaluationConfigService;
    if (!service)
        return UNAVAILABLE;
    const resumeData = ctx.resumeData;
    if (resumeData === undefined || resumeData === null) {
        return await ctx.suspend({
            requestId: (0, nanoid_1.nanoid)(),
            message: `Set up evaluation "${input.name}"`,
            severity: 'info',
        });
    }
    if (!resumeData.approved)
        return { denied: true, reason: 'User denied the action' };
    try {
        const config = await service.create(input.workflowId, toUpsertInput(input));
        return { config };
    }
    catch (error) {
        return { error: errorMessage(error) };
    }
}
async function handleUpdate(context, input, ctx) {
    const service = context.evaluationConfigService;
    if (!service)
        return UNAVAILABLE;
    const resumeData = ctx.resumeData;
    if (resumeData === undefined || resumeData === null) {
        return await ctx.suspend({
            requestId: (0, nanoid_1.nanoid)(),
            message: `Update evaluation "${input.name}"`,
            severity: 'warning',
        });
    }
    if (!resumeData.approved)
        return { denied: true, reason: 'User denied the action' };
    try {
        const config = await service.update(input.workflowId, input.configId, toUpsertInput(input));
        return { config };
    }
    catch (error) {
        return { error: errorMessage(error) };
    }
}
async function handleDelete(context, input, ctx) {
    const service = context.evaluationConfigService;
    if (!service)
        return { success: false, ...UNAVAILABLE };
    const resumeData = ctx.resumeData;
    if (resumeData === undefined || resumeData === null) {
        return await ctx.suspend({
            requestId: (0, nanoid_1.nanoid)(),
            message: `Delete evaluation "${input.configId}"`,
            severity: 'destructive',
        });
    }
    if (!resumeData.approved)
        return { success: false, denied: true, reason: 'User denied the action' };
    await service.delete(input.workflowId, input.configId);
    return { success: true };
}
function createEvalConfigTool(context) {
    const inputSchema = (0, sanitize_mcp_schemas_1.sanitizeInputSchema)(zod_1.z.discriminatedUnion('action', [...allActions]));
    return new agents_1.Tool(tool_ids_1.EVAL_CONFIG_TOOL_ID)
        .description('Manage configuration-based evaluations on a workflow. ' +
        'A config-based eval attaches a name, a start/end node, judged metrics, and a Data Table ' +
        'dataset to the workflow via the evaluation-config API (no eval nodes are added to the canvas). ' +
        'Create/populate the dataset Data Table with the data-tables tool first, then link it by id. ' +
        '"get" returns a summary (metric names/types); "describe" returns full metric detail — ' +
        'read with "describe" before "update", which replaces the entire config.')
        .input(inputSchema)
        .suspend(confirmationSuspendSchema)
        .resume(confirmationResumeSchema)
        .handler(async (input, ctx) => {
        switch (input.action) {
            case 'list':
                return await handleList(context, input);
            case 'get':
                return await handleGet(context, input);
            case 'describe':
                return await handleDescribe(context, input);
            case 'create':
                return await handleCreate(context, input, ctx);
            case 'update':
                return await handleUpdate(context, input, ctx);
            case 'delete':
                return await handleDelete(context, input, ctx);
            default:
                return { error: 'Unknown eval-config action' };
        }
    })
        .build();
}
//# sourceMappingURL=eval-config.tool.js.map