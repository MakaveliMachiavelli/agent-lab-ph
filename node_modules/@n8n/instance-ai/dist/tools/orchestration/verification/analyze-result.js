...
exports.buildNodePreviews = buildNodePreviews;
exports.countProducedOutputRows = countProducedOutputRows;
exports.buildSimulationNote = buildSimulationNote;
exports.namesOrDataKeys = namesOrDataKeys;
exports.analyzeVerificationResult = analyzeVerificationResult;
...
function stringifyForToolOutput(value) {
...
function countOutputItems(nodeOutput) {
...
function maxEmittedItemCount(resultData) {
    if (!resultData)
        return 0;
    let max = 0;
    for (const nodeOutput of Object.values(resultData)) {
...
function classifyVerificationFailure(error, status, buildOutcome) {
    ...
    if ((0, placeholder_1.isPlaceholderValue)(error)) {
        return (0, remediation_1.createRemediation)({
            category: 'needs_setup',
            shouldEdit: false,
    ...
    if (messageMatchesAny(normalized, TRANSIENT_FAILURE_KEYWORDS)) {
        return (0, remediation_1.createRemediation)({
            ...
            guidance: 'Workflow submitted successfully, but verification is blocked by an external service or timeout. Stop code edits and explain the blocker to the user.',
        });
    }
    return (0, remediation_1.createRemediation)({
...
function buildCoverageNote(nodesNotReached, result, success, reachedHaltedGates) {
    if (nodesNotReached.length === 0)
        return undefined;
    if (success && reachedHaltedGates.length > 0) {
        return (`Verification pauses at wait gate(s) ${reachedHaltedGates.join(', ')} — in a live run the ` +
...
function buildNodeErrorMessage(nodeErrors) {
    if (nodeErrors.length === 0)
        return undefined;
    return nodeErrors
        .map((nodeError) => nodeError.message
...
function analyzeVerificationResult(args) {
    ...
    const success = nodeErrors.length === 0 &&
        (result.status === 'success' ||
            (!hasSimulationPlan && result.status === 'waiting' && !errorMessage && hasOutput));
    const failureRemediation = success
        ? undefined
        : classifyVerificationFailure(errorMessage, nodeErrors.length > 0 && result.status === 'success' ? 'error' : result.status, buildOutcome);
    const budgetRemediation = failureRemediation?.shouldEdit === true
        ? (0, remediation_1.terminalRemediationFromState)(stateBefore, runId)
        : undefined;
    ...
    return {
        ...
        coverageNote: buildCoverageNote(nodesNotReached, result, success, (haltedGateNames ?? []).filter((name) => reachedNames.has(name))),
        errorMessage,
        nodeErrors,
    };
}
//# sourceMappingURL=analyze-result.js.map