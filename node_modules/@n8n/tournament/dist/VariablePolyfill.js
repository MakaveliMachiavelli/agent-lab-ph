...
const customPatches = {
    ...
    ArrowFunctionExpression(path, parent, dataNode) {
        if (parent.body === path.node) {
            polyfillVar(path, dataNode);
        }
    },
    SpreadElement(path, parent, dataNode) {
        if (parent.argument === path.node) {
            polyfillVar(path, dataNode);
        }
    },
    SpreadProperty(path, parent, dataNode) {
        if (parent.argument === path.node) {
            polyfillVar(path, dataNode);
        }
    },
    MethodDefinition(path, parent, dataNode) {
        if (parent.computed && parent.key === path.node) {
            polyfillVar(path, dataNode);
        }
    },
    SwitchCase(path, parent, dataNode) {
        if (parent.test === path.node) {
            polyfillVar(path, dataNode);
        }
    },
    ClassDeclaration(path, parent, dataNode) {
        if (parent.superClass === path.node) {
            polyfillVar(path, dataNode);
        }
    },
    ClassExpression(path, parent, dataNode) {
        if (parent.superClass === path.node) {
            polyfillVar(path, dataNode);
        }
    },
