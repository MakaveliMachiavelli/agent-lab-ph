...
exports.ParseValidateHandler = exports.WorkflowCodeParseError = void 0;
const workflow_sdk_1 = require("@n8n/workflow-sdk");
const extract_code_1 = require("../utils/extract-code");
class WorkflowCodeParseError extends Error {
    constructor(message, options) {
        super(message, options);
        this.name = 'WorkflowCodeParseError';
    }
}
exports.WorkflowCodeParseError = WorkflowCodeParseError;
class ParseValidateHandler {
    ...
    validateJSON(json) {
        ...
        this.collectValidationIssues(graphValidation.errors, allWarnings, 'GRAPH VALIDATION ERRORS', 'warn');
        this.collectValidationIssues(graphValidation.warnings, allWarnings, 'GRAPH VALIDATION WARNINGS', 'info');
        const jsonValidation = (0, workflow_sdk_1.validateWorkflow)(json, {
            nodeTypesProvider: this.nodeTypesProvider,
            allowDisconnectedNodes: true,
            allowNoTrigger: true,
        });
        this.collectValidationIssues(jsonValidation.errors, allWarnings, 'JSON VALIDATION ERRORS', 'warn');
        this.collectValidationIssues(jsonValidation.warnings, allWarnings, 'JSON VALIDATION WARNINGS', 'info');
        return allWarnings;
    }
    async parseAndValidate(code, currentWorkflow) {
        ...
        try {
            ...
            const existingIdsByName = new Map((currentWorkflow?.nodes ?? [])
            ...
            const existingGroupIdsByName = new Map((currentWorkflow?.nodeGroups ?? []).map((group) => [group.name, group.id]));
            const workflowJson = builder.toJSON({ tidyUp: true, existingGroupIdsByName });
            ...
            throw new WorkflowCodeParseError(`Failed to parse generated workflow code: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
