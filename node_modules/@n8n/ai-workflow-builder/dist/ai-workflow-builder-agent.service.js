...
let AiWorkflowBuilderService = class AiWorkflowBuilderService {
    ...
    nodeTypes;
    sessionManager;
    constructor(parsedNodeTypes, sessionStorage, client, logger, instanceId, instanceUrl, n8nVersion, onCreditsUpdated, onTelemetryEvent, nodeDefinitionDirs, resourceLocatorCallbackFactory, ssrf, modelFetch) {
        this.client = client;
        this.logger = logger;
    ...
    async getAnthropicClaudeModel({ baseUrl, authHeaders = {}, apiKey = '-', } = {}) {
    ...
    async getApiProxyAuthHeaders(user, userMessageId) {
        ...
        const authHeaders = {
            ...
            ...(0, api_types_1.buildProxyHeaders)({
    ...
    async *chat(payload, user, abortSignal) {
        ...
        if (this.onTelemetryEvent && userId) {
            try {
                await this.trackBuilderReplyTelemetry(agent, workflowId, userId, payload.id, threadId, true);
            }
            catch (error) {
    ...
    async trackBuilderReplyTelemetry(agent, workflowId, userId, userMessageId, threadId, isCodeBuilder) {
        ...
        const webFetchToolNames = messages.filter((m) => m instanceof messages_1.ToolMessage && m.name === 'web_fetch');
        const properties = {
            user_id: userId,
    ...
    static webFetchApprovalInterruptSchema = zod_1.z.object({
        type: zod_1.z.literal('web_fetch_approval'),
    ...
    extractHitlFromStreamOutput(output) {
        ...
        for (const message of messages) {
            ...
            if (m.type === 'web_fetch_approval') {
                const parsed = AiWorkflowBuilderService_1.webFetchApprovalInterruptSchema.safeParse(m);
                if (parsed.success)
                    return parsed.data;
...
exports.AiWorkflowBuilderService = AiWorkflowBuilderService = AiWorkflowBuilderService_1 = __decorate([
    (0, di_1.Service)(),
    __metadata("design:paramtypes", [Array, Object, ai_assistant_sdk_1.AiAssistantClient, backend_common_1.Logger, String, String, String, Function, Function, Array, Function, Object, Function])
