...
function createGetResourceLocatorOptionsTool(nodeTypes, resourceLocatorCallback, logger) {
    const dynamicTool = (0, tools_1.tool)(async (input, config) => {
        ...
        try {
            ...
            const property = (0, node_catalog_1.findNodeParameterProperty)(nodeType.properties, parameterPath);
            const lookup = property ? (0, node_catalog_1.getDynamicNodeParameterLookup)(property) : null;
            if (!lookup || lookup.kind === 'loadOptionsRouting') {
                const error = new errors_1.ValidationError(`Parameter "${parameterPath}" on node "${node.name}" is not a resource locator or load-options parameter with list search capability`, { extra: { nodeId, parameterPath } });
                reporter.error(error);
                return (0, response_1.createErrorResponse)(config, error);
            }
            const credentials = node.credentials;
            const credentialSlots = (0, node_catalog_1.getRequiredNodeCredentialSlots)(nodeType);
            ...
            try {
                const result = await Promise.race([
                    resourceLocatorCallback(lookup.methodName, (0, node_catalog_1.toDynamicParameterPath)(parameterPath), { name: node.type, version: node.typeVersion }, node.parameters ?? {}, credentials, filter),
                    new Promise((_, reject) => setTimeout(() => reject(new Error('Request timed out')), DEFAULT_TIMEOUT_MS)),
                ]);
                const formattedOptions = (0, node_catalog_1.formatResourceLocatorOptionsForLLM)(result.results, parameterPath);
                reporter.complete({
                    parameterPath,
