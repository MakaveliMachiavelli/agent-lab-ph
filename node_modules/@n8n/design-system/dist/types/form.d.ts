import { N8nLocaleTranslateFnOptions } from './i18n';
...
export type IFormInput<Metadata = Record<string, unknown>> = {
    ...
    properties: {
        label?: string;
        type?: 'text' | 'email' | 'password' | 'select' | 'multi-select' | 'number' | 'info' | 'checkbox' | 'toggle';
        maxlength?: number;
        required?: boolean;
...
export type InputAutocompletePropType = 'off' | 'new-password' | 'current-password' | 'given-name' | 'family-name' | 'one-time-code' | 'email';
