import { AcceptableValue as _AcceptableValue } from 'reka-ui';
...
export type GetObjectField<MaybeObject, Key extends string> = MaybeObject extends Record<string, unknown> ? MaybeObject[Key] : never;
...
type DotPathKeys<T> = T extends unknown[] ? never : T extends object ? {
    [K in keyof T & string]: T[K] extends Record<string, unknown> ? K | `${K}.${DotPathKeys<T[K]>}` : K;
...
export type GetModelValue<T, VK extends GetItemKeys<T> | undefined, M extends boolean> = M extends true ? Array<GetItemValue<T, VK>> : GetItemValue<T, VK>;
