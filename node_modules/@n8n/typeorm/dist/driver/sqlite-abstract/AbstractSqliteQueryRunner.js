...
class AbstractSqliteQueryRunner extends BaseQueryRunner_1.BaseQueryRunner {
    ...
    async rollbackTransaction() {
        try {
            ...
            throw new TransactionRollbackFailedError_1.TransactionRollbackFailedError(rollbackError);
        }
    }
    /**
    ...
    async loadTableRecords(tablePath, tableOrIndex) {
        ...
        const masterTable = `${schema ? `${this.escapePath(schema)}.` : ''}${this.escapePath('sqlite_master')}`;
        const query = tableOrIndex === 'table'
    ...
    async loadTables(tableNames) {
        ...
        if (!tableNames) {
            ...
            const queryPromises = (type) => {
                ...
                if (tableNamesWithoutDot.length) {
                    ...
                    promises.push(this.query(`SELECT * FROM "sqlite_master" WHERE "type" = ? AND ${columnName} IN (${placeholders})`, [type, ...tableNamesWithoutDot]));
                }
                return promises;
            };
        ...
        return Promise.all(dbTables.map(async (dbTable) => {
            ...
            table.foreignKeys = tableForeignKeyConstraints.map((foreignKey) => {
                ...
                const fkMapping = fkMappings.find((it) => it.referencedTableName === foreignKey['table'] &&
    ...
    dropIndexSql(indexOrName) {
        let indexName = InstanceChecker_1.InstanceChecker.isTableIndex(indexOrName) ? indexOrName.name : indexOrName;
    ...
    async recreateTable(newTable, oldTable, migrateData = true) {
        ...
        const triggers = await this.query(triggerQuery, [
            tableNameOldInitial,
        ]);
        ...
        triggers.forEach((trigger) => {
            ...
            upQueries.push(new Query_1.Query(trigger.sql));
            // downQueries: recreate triggers on the old table (rollback)
            downQueries.push(new Query_1.Query(trigger.sql));
        });
