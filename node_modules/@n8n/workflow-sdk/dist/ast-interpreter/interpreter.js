...
class SDKInterpreter {
    ...
    visitVariableDeclaration(node) {
        if (node.kind !== 'const') {
            throw new errors_1.SecurityError(node.kind, node.loc ?? undefined, this.sourceCode, `'${node.kind}' declarations are not allowed. Use 'const' only.`);
        }
        for (const declarator of node.declarations) {
            ...
            if ((0, validators_1.isAllowedSDKFunction)(name)) {
                ...
                throw new errors_1.SecurityError(name, declarator.loc ?? undefined, this.sourceCode, `'${name}' is a reserved SDK function name and cannot be used as a variable name. ` +
    ...
    visitCallExpression(node) {
        ...
        if (node.callee.type === 'Identifier') {
            ...
            if (!(0, validators_1.isAllowedMethod)(methodName)) {
                throw new errors_1.SecurityError(methodName, memberExpr.property.loc ?? undefined, this.sourceCode, `Method '${methodName}' is not an allowed SDK method. ` +
                    `Allowed methods: ${(0, validators_1.allowedMethodNames)().join(', ')}. ` +
