...
function serializeNode(mapKey, graphNode, nodePositions) {
    ...
    const parsedParams = (0, n8n_workflow_1.deepCopy)(config.parameters ?? {});
    let serializedParams;
    if (isFromJson) {
        serializedParams = parsedParams;
    }
    else {
        const normalized = (0, string_utils_1.normalizeResourceLocators)(parsedParams);
    ...
    return (0, n8n_workflow_1.normalizeNodeShape)(n8nNode);
}
    ...
    serialize(ctx) {
        ...
        if (ctx.nodeGroups && ctx.nodeGroups.length > 0) {
            ...
            json.nodeGroups = ctx.nodeGroups.map((group) => {
                const description = (0, n8n_workflow_1.normalizeGroupDescription)(group.description);
                const id = group.id ??
                    ctx.existingGroupIdsByName?.get(group.name) ??
                    (0, string_utils_1.generateDeterministicGroupId)(ctx.workflowId, group.name);
                return {
                    id,
                    name: group.name,
