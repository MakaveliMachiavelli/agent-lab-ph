...
exports.PIN_DATA_SYSTEM_PROMPT = void 0;
exports.buildNodeSchemaSection = buildNodeSchemaSection;
exports.buildPinDataUserPrompt = buildPinDataUserPrompt;
const ai_root_shapes_1 = require("./ai-root-shapes");
const context_1 = require("./context");
const mermaid_1 = require("./mermaid");
exports.PIN_DATA_SYSTEM_PROMPT = `You are a test data generator for n8n workflow automation. Generate realistic mock API response data for service nodes in a workflow.
...
function buildNodeSchemaSection(ctx) {
    const lines = [];
    if (ctx.resource || ctx.operation) {
        const parts = [];
        if (ctx.resource)
            parts.push(`Resource: ${ctx.resource}`);
        if (ctx.operation)
            parts.push(`Operation: ${ctx.operation}`);
        lines.push(`- ${parts.join(' | ')}`);
    }
    const isAiRoot = (0, ai_root_shapes_1.isAiRootNodeType)(ctx.nodeType);
    if (isAiRoot) {
        if (!ctx.schema) {
            lines.push(`- AI ROOT OUTPUT SHAPE: every item MUST be ${(0, ai_root_shapes_1.describeAiRootShape)(ctx.nodeType)}`);
        }
        if (ctx.outputParser?.schemaText) {
            const envelopeKey = (0, context_1.resolveEnvelopeKey)(ctx.nodeType, ctx.schema);
            ...
            const truncated = schemaStr.length > MAX_EMBEDDED_SCHEMA_CHARS
    ...
    if (ctx.dataTableColumns && ctx.dataTableColumns.length > 0) {
        ...
        lines.push('- REAL Data Table columns — every pinned row MUST contain exactly these keys plus ' +
...
function buildPinDataUserPrompt(workflow, contexts, options) {
    const { instructions, dateAnchors } = options;
    const mermaid = (0, mermaid_1.workflowToMermaid)(workflow);
    ...
    for (const ctx of contexts) {
        ...
        const consumers = (0, context_1.collectDownstreamConsumers)(workflow, ctx.nodeName);
    ...
    sections.push('');
    sections.push('Return a JSON object where each key is the exact node name and the value is an array of items, each wrapped in a "json" key:');
    sections.push('');
    sections.push('```json');
    sections.push('{');
