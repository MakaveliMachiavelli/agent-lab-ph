...
function buildNodeConfigType(paramsTypeName, options = {}) {
...
function compareVersionTuplesDesc(a, b) {
...
function versionMatchesCondition(version, condition) {
    ...
    if (condition._cnd) {
        ...
        if (eq !== undefined && version !== eq)
            return false;
        if (not !== undefined && version === not)
            return false;
        ...
        if (lte !== undefined && !(version <= lte))
            return false;
        if (between !== undefined && !(version >= between.from && version <= between.to))
            return false;
        return true;
    }
    return false;
...
function generateDiscriminatorFile(node, version, combo, props, schema, _importDepth = 5) {
    ...
    const configType = buildNodeConfigType(configName, {
        credentialsTypeName: node.credentials && node.credentials.length > 0 ? 'Credentials' : undefined,
...
function hasMainConnection(connections) {
    if (!Array.isArray(connections))
        return false;
    return connections.some((connection) => typeof connection === 'string' ? connection === 'main' : connection.type === 'main');
}
...
function generateSingleVersionTypeFile(node, specificVersion) {
    ...
    if (individualNodeTypes.length === 1) {
        ...
        lines.push(`${INDENT}config: ${buildNodeConfigType('Record<string, unknown>', { credentialsTypeName: credTypeName })};`);
        lines.push('};');
    }
    return lines.join('\n');
}
...
function generateNodeTypeFile(nodes) {
    ...
    for (const n of sortedNodes) {
        ...
        lines.push(`${INDENT}config: ${buildNodeConfigType(`${nodeName}${entryVersionSuffix}Params`, {
