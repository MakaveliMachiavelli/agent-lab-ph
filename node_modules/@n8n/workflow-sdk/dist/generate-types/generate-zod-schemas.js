...
exports.collectDeclarationsByName = collectDeclarationsByName;
exports.generateOneOfSchemaLine = generateOneOfSchemaLine;
exports.generateMergedSchemaLine = generateMergedSchemaLine;
...
function combineSchemaAlternatives(schemas) {
...
function generateNestedSchemaPropertyLines(properties) {
...
function cloneConditionMap(conditions) {
    if (!conditions)
        return undefined;
    return Object.fromEntries(Object.entries(conditions).map(([key, values]) => [key, [...values]]));
}
function cloneDisplayOptions(displayOptions) {
    const cloned = {};
...
function collectDeclarationsByName(properties) {
...
function generateOneOfSchemaLine(variants, allProperties = []) {
    ...
    for (const { prop, displayOptions } of variants) {
        ...
        variantStrs.push(`{ schema: ${zodSchema}, required: ${required}, displayOptions: ${displayOptionsStr}${defaultsStr} }`);
    }
    return `${INDENT}${propName}: resolveOneOfSchemas({ parameters, variants: [${variantStrs.join(', ')}] }),`;
}
function getSchemaVariantsForDuplicateDeclarations(declarations, keysToStrip) {
    const variants = [];
    let allConditional = true;
    for (const declaration of declarations) {
        const { displayOptions, fullyDisabled } = (0, generate_types_1.narrowDisplayOptionsByDisabled)(declaration);
        if (fullyDisabled) {
            continue;
        }
...
function hasDuplicateConditionalDeclarations(properties, keysToStrip) {
    for (const declarations of collectDeclarationsByName(properties).values()) {
        if (declarations.length < 2) {
            continue;
        }
        const { allConditional, variants } = getSchemaVariantsForDuplicateDeclarations(declarations, keysToStrip);
...
function generateMergedSchemaLine(mergedProp, declarations, allProperties, keysToStrip) {
    if (declarations.length > 1) {
        ...
        if (allConditional) {
            ...
            if (variants.length === 1) {
                return generateConditionalSchemaLine({ ...variants[0].prop, displayOptions: variants[0].displayOptions }, allProperties);
            }
            return '';
        }
    }
    const strippedDisplayOptions = mergedProp.displayOptions
        ...
        : undefined;
    if (strippedDisplayOptions) {
        return generateConditionalSchemaLine({ ...mergedProp, displayOptions: strippedDisplayOptions }, allProperties);
    }
