...
function appendNodeConfigOptions(configParts, node) {
    if (node.json.webhookId) {
        configParts.push(`webhookId: '${(0, string_utils_1.escapeString)(node.json.webhookId)}'`);
    }
    if (node.json.disabled) {
        configParts.push('disabled: true');
    }
    if (node.json.notes) {
        configParts.push(`notes: '${(0, string_utils_1.escapeString)(node.json.notes)}'`);
    }
    ...
    if (node.json.extendsCredential) {
        configParts.push(`extendsCredential: '${(0, string_utils_1.escapeString)(node.json.extendsCredential)}'`);
    }
}
    ...
    if (json.nodeGroups && json.nodeGroups.length > 0) {
        ...
        for (const node of json.nodes) {
            if (node.name !== undefined)
                idToNodeName.set(node.id, node.name);
        }
        for (const group of json.nodeGroups) {
            const memberVars = group.nodeIds.flatMap((id) => {
                const nodeName = idToNodeName.get(id);
                return nodeName !== undefined ? [(0, variable_names_1.getVarName)(nodeName, ctx)] : [];
            });
            const options = group.description
                ? `, { description: '${(0, string_utils_1.escapeString)(group.description)}' }`
                : '';
            workflowCalls.push(`  .group('${(0, string_utils_1.escapeString)(group.name)}', [${memberVars.join(', ')}]${options})`);
        }
    }
    lines.push('');
