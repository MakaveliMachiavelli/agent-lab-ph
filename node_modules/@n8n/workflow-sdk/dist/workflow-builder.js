...
class WorkflowBuilderImpl {
    constructor(id, name, settings = {}, nodes, currentNode, pinData, meta, registry, nodeGroups) {
        this._branchDepth = 0;
    ...
    branchFromCurrent(outputIndex, target, methodName) {
        ...
        const matches = sourceType
            ? wantsSwitch
                ? (0, node_types_1.isSwitchNodeType)(sourceType)
                : (0, node_types_1.isIfNodeType)(sourceType)
            : false;
        if (!matches) {
            ...
            throw new Error(`.${methodName}() must immediately follow adding a ${expected} node. Use it as ${usage}.`);
        }
    ...
    regenerateNodeIds(existingIdsByName) {
        ...
        for (const [mapKey, graphNode] of this._nodes) {
            ...
            const newId = existingIdsByName?.get(mapKey) ??
                (0, string_utils_1.generateDeterministicNodeId)(this.id, instance.type, mapKey);
    ...
    collectValidationIssues(issues, errors, warnings, ValidationErrorClass, ValidationWarningClass) {
        for (const issue of issues) {
            ...
            if (issue.severity === 'error') {
                ...
                warnings.push(new ValidationWarningClass(code, issue.message, issue.nodeName, issue.parameterPath, issue.originalName, issue.violationLevel, issue.severity === 'informational' ? 'informational' : 'warning'));
            }
        }
    }
    toString() {
...
function fromJSON(json) {
    ...
    return new WorkflowBuilderImpl(parsed.id, parsed.name, parsed.settings, parsed.nodes, parsed.lastNode, parsed.pinData, parsed.meta, undefined, parsed.nodeGroups);
}
