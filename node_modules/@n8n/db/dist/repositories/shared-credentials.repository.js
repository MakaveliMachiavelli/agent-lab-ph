...
let SharedCredentialsRepository = class SharedCredentialsRepository extends typeorm_1.Repository {
    ...
    async findPairsWithCredentialAccess(pairs, scope, credentialRoles, trx) {
        if (pairs.length === 0 || credentialRoles.length === 0)
            return [];
        ...
        const rows = await manager
            .createQueryBuilder(entities_1.SharedCredentials, 'sc')
            .select(['sc.credentialsId AS "credentialId"', 'pr.userId AS "userId"'])
            .distinct(true)
            .innerJoin(entities_1.ProjectRelation, 'pr', 'pr.projectId = sc.projectId')
