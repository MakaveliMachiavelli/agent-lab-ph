...
let RoleRepository = class RoleRepository extends typeorm_1.Repository {
    ...
    async findUsersWithGlobalRole(roleSlug) {
    ...
    async reassignUsersAndRemove(role, toSlug) {
        await this.manager.transaction(async (trx) => {
            if (role.roleType === 'global') {
                ...
                await trx
                    .createQueryBuilder()
                    .update(entities_1.ProjectRelation)
                    .set({ role: { slug: toSlug } })
                    .where('role = :fromSlug', { fromSlug: role.slug })
                    .execute();
            }
            const result = await trx.delete(entities_1.Role, { slug: role.slug });
            if (result.affected !== 1) {
