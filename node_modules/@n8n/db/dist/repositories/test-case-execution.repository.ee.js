...
let TestCaseExecutionRepository = class TestCaseExecutionRepository extends typeorm_1.Repository {
    ...
    async getManyByTestRunId(testRunId, { skip, take } = {}) {
    ...
    async countByTestRunId(testRunId) {
    ...
    async createPendingBatch(testRunId, cases) {
        const rows = cases.map(({ runIndex, inputs }) => this.create({
    ...
    async tryMarkCaseAsRunning(id) {
        const result = await this.update({ id, status: 'new' }, { status: 'running', runAt: new Date() });
        return (result.affected ?? 0) > 0;
    }
    async cancelIfNew(testRunId, id) {
        const result = await this.update({ id, status: 'new', testRun: { id: testRunId } }, { status: 'cancelled', completedAt: new Date() });
        return (result.affected ?? 0) > 0;
    }
