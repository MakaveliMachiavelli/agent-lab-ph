...
let WorkflowHistoryRepository = class WorkflowHistoryRepository extends base_repository_1.BaseRepository {
    ...
    async updateVersionMetadata({ workflowId, versionId, name, description, }, ctx) {
        const result = await this.managerFor(ctx).update(entities_1.WorkflowHistory, { workflowId, versionId }, { name, ...(description !== undefined ? { description } : {}) });
    ...
    async deleteEarlierThanExceptCurrentAndActive(date, preserveNamedVersions = false) {
        ...
        const publishedVersionIdsSubquery = this.manager
            ...
            .from(entities_1.WorkflowPublishedVersion, 'wpv')
            .getQuery();
        const openReviewPinnedVersionIdsSubquery = this.manager
            .createQueryBuilder()
            .subQuery()
            .select('wrrw.workflowVersionId')
            .from(workflow_review_request_workflow_ee_1.WorkflowReviewRequestWorkflow, 'wrrw')
            .innerJoin(workflow_review_request_ee_1.WorkflowReviewRequest, 'wrr', 'wrr.id = wrrw.workflowReviewRequestId')
    ...
    async pruneHistory(workflowId, startDate, endDate, rules = [], skipRules = [], metaData) {
        ...
        const grouped = (0, n8n_workflow_1.groupWorkflows)(workflows, rules, [
            this.makeSkipActiveAndNamedVersionsRule(new Set(publishedVersions.map((v) => v.versionId).filter((v) => v !== null))),
...
exports.WorkflowHistoryRepository = WorkflowHistoryRepository = __decorate([
    (0, di_1.Service)(),
    __metadata("design:paramtypes", [typeorm_1.DataSource, workflow_publish_history_repository_1.WorkflowPublishHistoryRepository])
