...
let SecretsProviderConnectionRepository = class SecretsProviderConnectionRepository extends typeorm_1.Repository {
    ...
    async findIdByProviderKey(providerKey) {
    ...
    async findIdsByProviderKeys(providerKeys) {
        if (providerKeys.length === 0)
            return [];
        const connections = await this.find({
            select: ['id'],
            where: { providerKey: (0, typeorm_1.In)(providerKeys) },
        });
    ...
    async findEnabledGlobalConnections(filters = {}) {
    ...
    async findEnabledByProjectId(projectId, filters = {}) {
        const { providerKeys } = filters;
        if (providerKeys && providerKeys.length === 0) {
            return [];
        }
