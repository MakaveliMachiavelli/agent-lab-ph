...
let UserRepository = class UserRepository extends typeorm_1.Repository {
    ...
    async findEligibleByProjectOrGlobalRoles({ projectId, projectRoleSlugs, globalRoleSlugs, }) {
        const where = [];
        if (globalRoleSlugs.length > 0) {
            where.push({ disabled: false, role: { slug: (0, typeorm_1.In)(globalRoleSlugs) } });
        }
        if (projectRoleSlugs.length > 0) {
            where.push({
                disabled: false,
                projectRelations: { projectId, role: { slug: (0, typeorm_1.In)(projectRoleSlugs) } },
            });
        }
        if (where.length === 0) {
            return [];
    ...
    applyUserListFilter(queryBuilder, filter) {
        ...
        if (filter?.projectId !== undefined) {
            queryBuilder.innerJoin('user.projectRelations', 'userListProjectFilter', 'userListProjectFilter.projectId = :userListProjectId', { userListProjectId: filter.projectId });
        }
        return queryBuilder;
    }
