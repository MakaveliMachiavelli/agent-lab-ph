...
let ExecutionRepository = class ExecutionRepository extends typeorm_1.Repository {
    ...
    async findMultipleExecutions(queryParams, options) {
        ...
        return (await Promise.all(valid.map(async (execution) => {
    ...
    async markAsCrashed(executionIds) {
        ...
        while (processed < executionIds.length) {
            ...
            await this.update({ id: (0, typeorm_1.In)(batch), status: (0, typeorm_1.In)(n8n_workflow_1.CRASHABLE_EXECUTION_STATUSES) }, { status: 'crashed', stoppedAt: new Date(), waitTill: null });
    ...
    async setRunning(executionId) {
        ...
        return await this.manager.transaction(async (manager) => {
            ...
            await manager.update(entities_1.ExecutionEntity, { id: executionId }, { status: 'running', startedAt: effectiveStartedAt, waitTill: null });
            return effectiveStartedAt;
        });
    }
    async updateExistingExecution(executionId, execution, conditions) {
        const { id, data, workflowId, workflowData, workflowVersionId, createdAt, startedAt, customData, jsonSizeBytes, binaryDataSizeBytes, ...executionInformation } = execution;
        const executionData = {};
        if (workflowData)
    ...
    async countInWorkflows(workflowIds, options) {
    ...
    getFindManyInWorkflowsCondition(workflowIds, options = {}) {
        const where = {
            ...this.getIdCondition({
    ...
    async stopBeforeRun(execution) {
        ...
        await this.update({ id: execution.id }, { status: execution.status, stoppedAt: execution.stoppedAt, waitTill: execution.waitTill });
        return execution;
    }
    async cancelMany(executionIds) {
        await this.update({ id: (0, typeorm_1.In)(executionIds) }, { status: 'canceled', stoppedAt: new Date(), waitTill: null });
    }
    ...
    toQueryBuilder(query) {
        const { user, sharingOptions, status, finished, workflowId, startedBefore, startedAfter, metadata, annotationTags, vote, projectId, workflowVersionId, isArchived, workflowBooleanSettings, } = query;
        ...
        if (workflowVersionId) {
            qb.innerJoin('execution.executionData', 'executionData', 'executionData.workflowVersionId = :workflowVersionId');
        ...
        if (workflowBooleanSettings?.length) {
            for (const { key, value } of workflowBooleanSettings) {
                (0, apply_workflow_boolean_setting_filter_1.applyWorkflowBooleanSettingFilter)(qb, this.globalConfig, key, value);
            }
        }
        return qb;
    }
    ...
    async getWorkflowIdsWithExecutionsSince(date) {
        const result = await this.createQueryBuilder('execution')
            ...
            date: DateUtils_1.DateUtils.mixedDateToUtcDatetimeString(date),
        })
            .getRawMany();
        return result.map((row) => row.workflowId);
    }
    async getDistinctVersionIds(workflowId) {
        const result = await this.createQueryBuilder('execution')
    ...
    async handleExecutionRunData(data, options = {}) {
        if (options.unflattenData) {
            const deserializedData = await (0, backend_common_1.parseFlatted)(data);
            if (deserializedData) {
...
exports.ExecutionRepository = ExecutionRepository = __decorate([
    (0, di_1.Service)(),
    __metadata("design:paramtypes", [typeorm_1.DataSource, config_1.GlobalConfig, backend_common_1.Logger, n8n_core_1.ErrorReporter, n8n_core_1.BinaryDataService, shared_workflow_repository_1.SharedWorkflowRepository])
], ExecutionRepository);
