...
class ModifyWorkflowHistoryNodesAndConnections1695829275184 {
    async up({ schemaBuilder: { addColumns, dropColumns, column } }) {
        ...
        await addColumns(tableName, [column('nodes').json.notNull, column('connections').json.notNull], { recreatesOnSqlite: true });
    }
    async down({ schemaBuilder: { dropColumns, addColumns, column } }) {
        await dropColumns(tableName, ['nodes', 'connections'], { recreatesOnSqlite: true });
        await addColumns(tableName, [column('nodes').text.notNull, column('connections').text.notNull], { recreatesOnSqlite: true });
    }
}
