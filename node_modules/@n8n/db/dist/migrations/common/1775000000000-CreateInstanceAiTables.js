...
exports.CreateInstanceAiTables1775000000000 = void 0;
const table = {
...
class CreateInstanceAiTables1775000000000 {
    async up({ schemaBuilder: { createTable, column } }) {
        await createTable(table.threads)
            .withColumns(column('id').uuid.primary.notNull, column('resourceId').varchar(255).notNull, column('title').text.default("''").notNull, column('metadata').json)
            .withIndexOn('resourceId').withTimestamps;
        await createTable(table.messages)
            .withColumns(column('id').varchar(36).primary.notNull, column('threadId').uuid.notNull, column('content').text.notNull, column('role').varchar(16).notNull, column('type').varchar(32), column('resourceId').varchar(255))
        ...
        await createTable(table.workflowSnapshots)
            .withColumns(column('runId').varchar(36).primary.notNull, column('workflowName').varchar(255).primary.notNull, column('resourceId').varchar(255), column('status').varchar(), column('snapshot').text.notNull)
            .withIndexOn(['workflowName', 'status']).withTimestamps;
        await createTable(table.runSnapshots)
            .withColumns(column('threadId').uuid.primary.notNull, column('runId').varchar(36).primary.notNull, column('messageGroupId').varchar(36), column('runIds').json, column('tree').text.notNull)
        ...
        await createTable(table.iterationLogs)
            .withColumns(column('id').varchar(36).primary.notNull, column('threadId').uuid.notNull, column('taskKey').varchar().notNull, column('entry').text.notNull)
...
exports.CreateInstanceAiTables1775000000000 = CreateInstanceAiTables1775000000000;
//# sourceMappingURL=1775000000000-CreateInstanceAiTables.js.map