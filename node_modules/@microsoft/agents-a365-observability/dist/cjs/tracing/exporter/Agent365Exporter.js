...
class Agent365Exporter {
    ...
    async exportGroup(identityKey, spans) {
        ...
        const chunks = (0, utils_1.chunkBySize)(mappedSpans, (ms) => (0, utils_1.estimateSpanBytes)(ms.span), this.options.maxPayloadBytes);
        if (chunks.length > 1) {
            logging_1.default.info(`[Agent365Exporter] Split ${spans.length} spans into ${chunks.length} chunks for tenantId: ${tenantId}, agentId: ${agentId}`);
        }
        ...
        if (domainOverride) {
            ...
            logging_1.default.info(`[Agent365Exporter] Using default endpoint: ${url}`);
        }
        const headers = {
        ...
        for (let i = 0; i < chunks.length; i++) {
            ...
            logging_1.default.info(`[Agent365Exporter] Sending chunk ${i + 1} of ${chunks.length} (${chunk.length} spans, ${bodyBytes} bytes)`);
            const { ok, correlationId } = await this.postWithRetries(url, body, headers);
            lastCorrelationId = correlationId;
            if (!ok) {
                const duration = Date.now() - startTime;
                logging_1.default.event(ExporterEventNames_1.ExporterEventNames.EXPORT_GROUP, false, duration, `chunk ${i + 1} of ${chunks.length} failed`, { tenantId, agentId, correlationId });
                throw new Error(`Failed to export spans (chunk ${i + 1} of ${chunks.length})`);
            }
        }
        const duration = Date.now() - startTime;
        logging_1.default.event(ExporterEventNames_1.ExporterEventNames.EXPORT_GROUP, true, duration, `${chunks.length} chunk(s) exported successfully`, { tenantId, agentId, correlationId: lastCorrelationId });
    }
    /**
     ...
     */
    mapAndTruncateSpans(spans) {
        logging_1.default.info('[Agent365Exporter] Mapping and truncating spans');
        return spans.map(sp => {
            ...
            return {
                span: (0, utils_1.truncateSpan)(this.mapSpan(sp)),
     ...
     */
    buildEnvelope(mappedSpans, resourceAttrs) {
