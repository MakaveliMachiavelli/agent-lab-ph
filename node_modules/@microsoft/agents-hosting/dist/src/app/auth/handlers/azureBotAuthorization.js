...
class AzureBotAuthorization {
    ...
    async getBaseToken(context) {
        ...
        if (!(token === null || token === void 0 ? void 0 : token.trim())) {
            ...
            const { tokenResponse } = await userTokenClient.getTokenOrSignInResource((_a = activity.from) === null || _a === void 0 ? void 0 : _a.id, this.options.azureBotOAuthConnectionName, activity.channelId, activity.getConversationReference({ forceBaseChannel: true }), activity.relatesTo, '');
    ...
    async setToken(storage, context, active, code, reason = { message: '' }) {
        ...
        const { activity } = context;
        const userTokenClient = await this.getUserTokenClient(context);
        const { tokenResponse, signInResource } = await userTokenClient.getTokenOrSignInResource((_a = activity.from) === null || _a === void 0 ? void 0 : _a.id, this.options.azureBotOAuthConnectionName, activity.channelId, activity.getConversationReference({ forceBaseChannel: true }), activity.relatesTo, code !== null && code !== void 0 ? code : '');
