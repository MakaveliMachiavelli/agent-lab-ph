...
class AuthorizationManager {
    ...
    constructor(app, connections) {
        ...
        this._envLatest = {
            ...
            parser: (0, settings_1.envParser)({
                // Common
                type: settings_1.envParserUtils.bypass,
                // Azure Bot
                azureBotOAuthConnectionName: settings_1.envParserUtils.bypass,
                title: settings_1.envParserUtils.bypass,
                text: settings_1.envParserUtils.bypass,
                invalidSignInRetryMessage: settings_1.envParserUtils.bypass,
                invalidSignInRetryMessageFormat: settings_1.envParserUtils.bypass,
                invalidSignInRetryMaxExceededMessage: settings_1.envParserUtils.bypass,
                oboConnectionName: settings_1.envParserUtils.bypass,
                enableSso(value) {
        ...
        this._envLegacy = {
            ...
            parser: (0, settings_1.envParser)({
                // Common
                type: settings_1.envParserUtils.redirect(this._envLatest.parser, 'type'),
                // Azure Bot
                connectionName: settings_1.envParserUtils.redirect(this._envLatest.parser, 'azureBotOAuthConnectionName'),
                connectionTitle: settings_1.envParserUtils.redirect(this._envLatest.parser, 'title'),
                connectionText: settings_1.envParserUtils.redirect(this._envLatest.parser, 'text'),
                maxAttempts: settings_1.envParserUtils.redirect(this._envLatest.parser, 'invalidSignInRetryMax'),
                messages_invalidCode: settings_1.envParserUtils.redirect(this._envLatest.parser, 'invalidSignInRetryMessage'),
                messages_invalidCodeFormat: settings_1.envParserUtils.redirect(this._envLatest.parser, 'invalidSignInRetryMessageFormat'),
                messages_maxAttemptsExceeded: settings_1.envParserUtils.redirect(this._envLatest.parser, 'invalidSignInRetryMaxExceededMessage'),
                obo_connection: settings_1.envParserUtils.redirect(this._envLatest.parser, 'oboConnectionName'),
                obo_scopes: settings_1.envParserUtils.redirect(this._envLatest.parser, 'oboScopes'),
                enableSso: settings_1.envParserUtils.redirect(this._envLatest.parser, 'enableSso'),
                // Agentic
                scopes: settings_1.envParserUtils.redirect(this._envLatest.parser, 'scopes'),
                altBlueprintConnectionName: settings_1.envParserUtils.redirect(this._envLatest.parser, 'altBlueprintConnectionName')
            })
        };
        this.createHandlers();
    ...
    createHandlers() {
        ...
        const runtimeOptionEntries = Object.entries((_a = this.app.options.authorization) !== null && _a !== void 0 ? _a : {});
        ...
        for (const [envKey, envValue] of Object.entries(process.env)) {
            ...
            if (!upperEnvKey.startsWith(this._envLatest.key.prefix.toUpperCase())) {
                const [id] = (_b = runtimeOptionEntries.find(([id]) => upperEnvKey.startsWith(`${id.toUpperCase()}${this._envLegacy.key.separator}`))) !== null && _b !== void 0 ? _b : [];
                if (!id) {
                    continue;
                }
                ...
                (_c = (_f = result.legacy)[id]) !== null && _c !== void 0 ? _c : (_f[id] = {});
            ...
            (_d = (_g = result.latest)[realId]) !== null && _d !== void 0 ? _d : (_g[realId] = {});
        ...
        for (const [id] of [...runtimeEntries, ...latestEntries]) {
            ...
            if (options.type === AZURE_BOT) {
                ...
                (_e = options.oboScopes) !== null && _e !== void 0 ? _e : (options.oboScopes = []);
            ...
            if ('oboScopes' in options && options.oboScopes && options.oboScopes.length > 0) {
                maskedOptions.oboScopes = (0, agents_telemetry_1.redactScopes)(options.oboScopes);
            }
            if ('scopes' in options && options.scopes && options.scopes.length > 0) {
                maskedOptions.scopes = (0, agents_telemetry_1.redactScopes)(options.scopes);
            }
     ...
     */
    resolveHandlerConfiguration(id, runtimeEntries, latestEntries, legacyEntries) {
        var _a, _b, _c, _d, _e, _f, _g, _h, _j, _k, _l, _m, _o, _p, _q;
        const matchesId = ([_id]) => _id.toLowerCase() === id.toLowerCase();
        // Find entries case-insensitively for later processing
        const [, runtime] = (_a = runtimeEntries.find(matchesId)) !== null && _a !== void 0 ? _a : [];
        const [, latest] = (_b = latestEntries.find(matchesId)) !== null && _b !== void 0 ? _b : [];
        const [, legacy] = (_c = legacyEntries.find(matchesId)) !== null && _c !== void 0 ? _c : [];
        ...
        if (isRuntimeLegacy && runtime.type === AZURE_BOT) {
            (_d = runtime.azureBotOAuthConnectionName) !== null && _d !== void 0 ? _d : (runtime.azureBotOAuthConnectionName = runtime.name);
            (_e = runtime.invalidSignInRetryMax) !== null && _e !== void 0 ? _e : (runtime.invalidSignInRetryMax = runtime.maxAttempts);
            (_f = runtime.invalidSignInRetryMessage) !== null && _f !== void 0 ? _f : (runtime.invalidSignInRetryMessage = (_g = runtime.messages) === null || _g === void 0 ? void 0 : _g.invalidCode);
            (_h = runtime.invalidSignInRetryMessageFormat) !== null && _h !== void 0 ? _h : (runtime.invalidSignInRetryMessageFormat = (_j = runtime.messages) === null || _j === void 0 ? void 0 : _j.invalidCodeFormat);
            (_k = runtime.invalidSignInRetryMaxExceededMessage) !== null && _k !== void 0 ? _k : (runtime.invalidSignInRetryMaxExceededMessage = (_l = runtime.messages) === null || _l === void 0 ? void 0 : _l.maxAttemptsExceeded);
            (_m = runtime.oboConnectionName) !== null && _m !== void 0 ? _m : (runtime.oboConnectionName = (_o = runtime.obo) === null || _o === void 0 ? void 0 : _o.connection);
            (_p = runtime.oboScopes) !== null && _p !== void 0 ? _p : (runtime.oboScopes = (_q = runtime.obo) === null || _q === void 0 ? void 0 : _q.scopes);
            delete runtime.name;
        ...
        if (runtime !== undefined && legacy !== undefined) {
            return { format: `${runtimeLegacyFormat || 'runtime options'} + .env variables (legacy)`, options: { ...(0, utils_2.prune)(legacy), ...(0, utils_2.prune)(runtime) } };
        }
        if (runtime !== undefined) {
            return { format: runtimeLegacyFormat || 'runtime options', options: (0, utils_2.prune)(runtime) };
        }
        if (latest !== undefined) {
            return { format: '.env variables', options: (0, utils_2.prune)(latest) };
        }
        if (legacy !== undefined) {
            ...
            return { format: '.env variables (legacy)', options: (0, utils_2.prune)(legacy) };
        }
