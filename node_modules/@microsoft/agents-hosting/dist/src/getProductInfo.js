...
exports.applyUserAgentHeader = applyUserAgentHeader;
exports.applyAgenticHeaders = applyAgenticHeaders;
 ...
 */
const getProductInfo = () => `${DEFAULT_AGENT_NAME.toLowerCase()}/${package_json_1.version} nodejs/${process.version} ${os_1.default.platform()}-${os_1.default.arch()}/${os_1.default.release()}`;
 ...
 */
function applyUserAgentHeader(headers) {
    var _a, _b, _c;
    const userAgentKey = (_b = (_a = headers.key) === null || _a === void 0 ? void 0 : _a.call(headers, 'User-Agent')) !== null && _b !== void 0 ? _b : 'User-Agent';
    const userAgent = (_c = headers.outgoing[userAgentKey]) !== null && _c !== void 0 ? _c : headers.incoming[userAgentKey];
    const productInfo = (0, exports.getProductInfo)();
    if (!userAgent) {
 ...
 */
function applyAgenticHeaders(headers, activity, agentName) {
    var _a;
    headers.override({
        AgentRegistrar: AGENT_REGISTRAR,
        AgentName: normalizeAgentName(agentName),
        AgentID: resolveAgentId(activity),
        'Agent-Referrer': (_a = activity.channelId) !== null && _a !== void 0 ? _a : '',
    });
}
function resolveAgentId(activity) {
    var _a;
    const agentId = (_a = activity.getAgenticInstanceId()) !== null && _a !== void 0 ? _a : activity.getAgenticUser();
    if (!agentId) {
        throw agents_activity_1.ExceptionHelper.generateException(Error, errorHelper_1.Errors.AgentIdRequired);
    }
    return agentId;
}
function normalizeAgentName(agentName) {
    const normalizedAgentName = agentName === null || agentName === void 0 ? void 0 : agentName.trim();
    if (!normalizedAgentName) {
        return DEFAULT_AGENT_NAME;
    }
    if (!AGENT_NAME_PATTERN.test(normalizedAgentName)) {
        throw agents_activity_1.ExceptionHelper.generateException(Error, errorHelper_1.Errors.AgentNameInvalid, undefined, { agentName: normalizedAgentName });
    }
