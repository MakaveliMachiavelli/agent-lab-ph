...
exports.coerceUploadSource = coerceUploadSource;
exports.wrapWithUploadProgress = wrapWithUploadProgress;
...
function parseContentLength(headersBlock) {
...
async function processDownloadFilesResponseWithBusboy(stream, headers, metadataMap, onFileStream) {
    ...
    const busboy = (0, Import_1.dynamicRequire)('busboy', errPrefix);
    const Buffer = (0, Import_1.dynamicRequire)('buffer', errPrefix).Buffer;
    const fileTasks = [];
    const boundary = (0, Multipart_1.extractBoundary)((0, Multipart_1.getHeader)(headers, 'content-type') || '');
    ...
    await new Promise((resolve, reject) => {
        ...
        feedStreamToBusboy(stream, bb, observer).catch((err) => bb.destroy(err));
    });
...
async function feedStreamToBusboy(stream, bb, observer) {
    ...
    if (typeof stream?.pipe === 'function') {
        if (observer) {
            const { Transform } = await (0, Import_1.dynamicImport)('stream', '"downloadFiles" is not supported: ');
            ...
            stream.on('error', (err) => {
                if (!tapStream.destroyed)
                    tapStream.destroy(normalizeDownloadStreamError(err));
            });
            tapStream.on('error', (err) => {
                if (!bb.destroyed)
                    bb.destroy(err);
            });
            ...
            stream.on('error', (err) => {
                if (!bb.destroyed)
                    bb.destroy(normalizeDownloadStreamError(err));
            });
            stream.pipe(bb);
        }
        return;
    ...
    if (stream?.[Symbol.asyncIterator]) {
        for await (const chunk of stream) {
            const buffer = Buffer.isBuffer(chunk) ? chunk : Buffer.from((0, Binary_1.toUint8Array)(chunk));
            bb.write(tap(buffer));
        }
        bb.end();
        return;
 ...
 */
function normalizeDownloadStreamError(err) {
    const e = err;
    if (e.code === 'ERR_CANCELED' || e.name === 'CanceledError' || e.name === 'AbortError') {
        return new DaytonaError_1.DaytonaError('Download cancelled');
    }
    return err;
}
/** Construct the cancellation error thrown when an upload is aborted. */
function createAbortError(remotePath) {
    return new DaytonaError_1.DaytonaError(`Upload cancelled: ${remotePath}`);
}
/**
 ...
 * through unchanged.
 */
async function coerceUploadSource(source) {
    const errPrefix = 'Uploading files is not supported: ';
    const stream = await (0, Import_1.dynamicImport)('stream', errPrefix);
    ...
    if (typeof source.getReader === 'function') {
        return stream.Readable.fromWeb(source);
    }
    throw new DaytonaError_1.DaytonaError(`Unsupported upload source: ${source.constructor?.name ?? typeof source}`);
}
/**
 ...
 */
function wrapWithUploadProgress(source, onProgress, signal) {
    if (!onProgress && !signal)
        return source;
    const errPrefix = 'Uploading files is not supported: ';
    const { Transform, pipeline } = (0, Import_1.dynamicRequire)('stream', errPrefix);
    let bytesSent = 0;
