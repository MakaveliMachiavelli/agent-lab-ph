        ...
        list(query) {
            ...
            async function* generator() {
                ...
                while (true) {
                    ...
                    let response;
                    try {
                        response = await api_1.context.with(api_1.trace.setSpan(api_1.context.active(), span), () => sandboxApi.listSandboxes(undefined, cursor, query?.limit, query?.id, query?.name, query?.labels ? JSON.stringify(query.labels) : undefined, undefined, query?.states, query?.snapshots, query?.targets, undefined, query?.minCpu, query?.maxCpu, query?.minMemoryGib, query?.maxMemoryGib, query?.minDiskGib, query?.maxDiskGib, query?.isPublic, query?.isRecoverable, query?.createdAtAfter, query?.createdAtBefore, query?.lastActivityAfter, query?.lastActivityBefore, query?.sort, query?.order));
                    ...
                    for (const sandbox of response.data.items) {
                        ...
                        yield new Sandbox_1.Sandbox(sandbox, structuredClone(clientConfig), Daytona.createAxiosInstance(), sandboxApi);
                    }
