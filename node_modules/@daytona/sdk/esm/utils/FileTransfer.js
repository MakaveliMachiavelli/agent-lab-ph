...
export async function processDownloadFilesResponseWithBusboy(stream, headers, metadataMap, onFileStream) {
    ...
    const Buffer = dynamicRequire('buffer', errPrefix).Buffer;
    const fileTasks = [];
...
export function createAbortError(remotePath) {
 ...
 * through unchanged.
 */
export async function coerceUploadSource(source) {
    ...
    throw new DaytonaError(`Unsupported upload source: ${source.constructor?.name ?? typeof source}`);
}
/**
 ...
 */
export function wrapWithUploadProgress(source, onProgress, signal) {
    if (!onProgress && !signal)
        return source;
    const errPrefix = 'Uploading files is not supported: ';
    const { Transform, pipeline } = dynamicRequire('stream', errPrefix);
    let bytesSent = 0;
