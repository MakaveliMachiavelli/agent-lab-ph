    ...
    validate() {
        ...
        for (const { serviceClass, methodName } of handlers) {
            if (!di_1.Container.has(serviceClass)) {
                throw new n8n_workflow_1.UserError(`Component "${serviceClass.name}" is not registered with the DI container. Any component using @OnShutdown() must be decorated with @Service()`);
            }
            const service = di_1.Container.get(serviceClass);
            if (!service[methodName]) {
                throw new n8n_workflow_1.UserError(`Component "${serviceClass.name}" does not have a "${methodName}" method`);
            }
        }
    }
    shutdown() {
...
exports.ShutdownService = ShutdownService = __decorate([
    (0, di_1.Service)(),
    __metadata("design:paramtypes", [backend_common_1.Logger, n8n_core_1.ErrorReporter, decorators_1.ShutdownMetadata])
], ShutdownService);
