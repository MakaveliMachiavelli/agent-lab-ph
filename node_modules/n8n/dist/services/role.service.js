...
const role_deletion_check_proxy_service_1 = require("./role-deletion-check-proxy.service");
let RoleService = class RoleService {
    constructor(license, roleRepository, scopeRepository, roleCacheService, logger, roleDeletionCheckProxy) {
        this.license = license;
        this.roleRepository = roleRepository;
        this.scopeRepository = scopeRepository;
        this.roleCacheService = roleCacheService;
        this.logger = logger;
        this.roleDeletionCheckProxy = roleDeletionCheckProxy;
    }
    ...
    async getRoleMembers(slug) {
        ...
        if (role.roleType !== 'global')
            throw new bad_request_error_1.BadRequestError('Role is not a global role');
    ...
    async removeCustomRole(slug, reassignRoleSlug) {
        const role = await this.roleRepository.findBySlug(slug);
        if (!role) {
            throw new not_found_error_1.NotFoundError('Role not found');
        }
        ...
        if (blockers.length > 0) {
            throw new bad_request_error_1.BadRequestError(`Cannot delete role: ${blockers.join('; ')}`);
        }
        if (usersWithRole > 0 && reassignRoleSlug) {
    ...
    async reassignUsersAndRemoveRole(role, reassignRoleSlug) {
        if (reassignRoleSlug === role.slug) {
            throw new bad_request_error_1.BadRequestError('Cannot reassign users to the role being deleted');
        }
        const reassignRole = await this.roleRepository.findBySlug(reassignRoleSlug);
        if (!reassignRole) {
            throw new bad_request_error_1.BadRequestError(`Reassignment role "${reassignRoleSlug}" does not exist`);
        }
        if (reassignRole.roleType !== role.roleType) {
            throw new bad_request_error_1.BadRequestError('Reassignment role must be of the same type as the deleted role');
        }
        await this.roleRepository.reassignUsersAndRemove(role, reassignRoleSlug);
    }
    async resolveScopes(scopeSlugs, roleType) {
        if (!scopeSlugs) {
            return undefined;
        }
        ...
        if (resolvedScopes.some((slug) => !permissions_1.CUSTOM_ROLE_SCOPE_WHITELIST[roleType].has(slug))) {
            const invalidScopes = resolvedScopes.filter((slug) => !permissions_1.CUSTOM_ROLE_SCOPE_WHITELIST[roleType].has(slug));
            throw new bad_request_error_1.BadRequestError(`The following scopes are not allowed for ${roleType} roles: ${invalidScopes.join(', ')}`);
        }
        return scopes;
    }
...
exports.RoleService = RoleService = __decorate([
    (0, di_1.Service)(),
    __metadata("design:paramtypes", [backend_common_1.LicenseState, db_1.RoleRepository, db_1.ScopeRepository, role_cache_service_1.RoleCacheService, backend_common_1.Logger, role_deletion_check_proxy_service_1.RoleDeletionCheckProxy])
], RoleService);
