...
let RedisClientService = class RedisClientService extends backend_common_1.TypedEmitter {
    constructor(logger, globalConfig) {
        ...
        this.exitOnRedisUnreachable = !backend_common_1.inTest;
    ...
    createRegularClient({ type, extraOptions }, retryStrategy) {
    ...
    createClusterClient({ type, extraOptions }, retryStrategy) {
        const options = this.getOptions({ extraOptions, retryStrategy });
    ...
    getOptions({ extraOptions, retryStrategy, }) {
        const { username, password, db, tls, tlsConfig, dualStack, keepAlive, keepAliveDelay, keepAliveInterval, reconnectOnFailover, } = this.globalConfig.queue.bull.redis;
        const options = {
            username,
            password,
            db,
