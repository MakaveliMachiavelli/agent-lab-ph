...
const execution_stop_service_1 = require("../scaling/execution-stop.service");
...
let ExecutionService = class ExecutionService {
    constructor(globalConfig, logger, activeExecutions, executionAnnotationRepository, annotationTagMappingRepository, executionRepository, executionPersistence, workflowHistoryRepository, workflowRepository, nodeTypes, waitTracker, workflowRunner, concurrencyControl, license, roleService, workflowSharingService, eventService, executionRedactionServiceProxy, executionStopService, ownershipService) {
        ...
        this.executionStopService = executionStopService;
        this.ownershipService = ownershipService;
    }
    async buildSharingOptions(scope) {
    ...
    async findOne(req, sharedWorkflowIds) {
        ...
        let execution;
        try {
            execution = await this.executionPersistence.findOneInWorkflows(executionId, sharedWorkflowIds, { maxDataSizeBytes: this.globalConfig.executions.maxDisplaySize });
        }
        catch (error) {
            if (error instanceof missing_execution_data_error_1.MissingExecutionDataError) {
                throw new not_found_error_1.NotFoundError('Data for this execution is unavailable. It may have already been deleted based on your data retention settings.');
            }
            throw error;
        }
        if (!execution) {
        ...
        if (!('data' in execution)) {
            throw new n8n_workflow_1.UnexpectedError('Expected execution data for display read');
        }
    ...
    async retry(req, sharedWorkflowIds) {
        const { id: executionId } = req.params;
        const execution = await this.executionPersistence.findWithUnflattenedData(executionId, sharedWorkflowIds);
        if (!execution) {
        ...
        if (execution.finished) {
            throw new conflict_error_1.ConflictError('The execution succeeded, so it cannot be retried.');
        }
        ...
        const { projectId, projectName } = await (0, utils_1.getWorkflowProjectDetailsSafe)(this.ownershipService, execution.workflowId);
        ...
        for (const { id } of executions) {
            try {
                ...
                if (!(e instanceof missing_execution_stop_error_1.MissingExecutionStopError)) {
                    this.logger.warn(`Unexpected error while attempting to stop execution ${id}: ${(0, ensure_error_1.ensureError)(e).message}`);
                }
            }
        }
        return stopped;
    ...
    async findOneInWorkflows(executionId, workflowIds, options) {
        return await this.executionPersistence.findOneInWorkflows(executionId, workflowIds, options);
    }
    async findManyAndCount(workflowIds, options) {
        ...
        const executions = await this.executionPersistence.findManyInWorkflows(workflowIds, listOptions, options.maxDataSizeBytes);
    ...
    async deleteOne(executionId, sharedWorkflowIds) {
        ...
        if (execution.status === 'running') {
            throw new bad_request_error_1.BadRequestError('Cannot delete a running execution');
        }
        if (execution.status === 'new') {
    ...
    async getExecutionTags(executionId, sharedWorkflowIds) {
        const execution = await this.findOneInWorkflows(executionId, sharedWorkflowIds, {
            includeData: false,
            includeAnnotation: false,
        });
    ...
    async updateExecutionTags(executionId, tagIds, sharedWorkflowIds) {
        const execution = await this.findOneInWorkflows(executionId, sharedWorkflowIds, {
            includeData: false,
            includeAnnotation: false,
        });
    ...
    async getExecutedVersions(workflowId) {
        ...
        const versions = await this.workflowHistoryRepository.find({
            where: { workflowId, versionId: (0, db_1.In)(versionIds) },
...
exports.ExecutionService = ExecutionService = __decorate([
    (0, di_1.Service)(),
    __metadata("design:paramtypes", [config_1.GlobalConfig, backend_common_1.Logger, active_executions_1.ActiveExecutions, db_1.ExecutionAnnotationRepository, db_1.AnnotationTagMappingRepository, db_1.ExecutionRepository, execution_persistence_1.ExecutionPersistence, db_1.WorkflowHistoryRepository, db_1.WorkflowRepository, node_types_1.NodeTypes, wait_tracker_1.WaitTracker, workflow_runner_1.WorkflowRunner, concurrency_control_service_1.ConcurrencyControlService, license_1.License, role_service_1.RoleService, workflow_sharing_service_1.WorkflowSharingService, event_service_1.EventService, execution_redaction_proxy_service_1.ExecutionRedactionServiceProxy, execution_stop_service_1.ExecutionStopService, ownership_service_1.OwnershipService])
], ExecutionService);
