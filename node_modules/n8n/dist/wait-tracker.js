...
function isRetryableResumeError(error) {
    return !(error instanceof n8n_workflow_1.UserError || error instanceof n8n_workflow_1.UnexpectedError);
}
let WaitTracker = class WaitTracker {
    constructor(logger, executionRepository, executionPersistence, ownershipService, activeExecutions, workflowRunner, instanceSettings) {
        this.logger = logger;
    ...
    async startExecution(executionId) {
        ...
        if ((0, workflow_helpers_1.shouldRestartParentExecution)(parentExecution)) {
            void this.resumeParentExecution(parentExecution, this.activeExecutions.getPostExecutePromise(executionId), { executionId, workflowId });
        }
    }
    async resumeParentExecution(parentExecution, executePromise, childExecution) {
        try {
            ...
            await this.withRetry(async () => {
                await (0, workflow_helpers_1.updateParentExecutionWithChildResults)(parentExecution.executionId, subworkflowResults, childExecution);
            }, MAX_PARENT_RESUME_ATTEMPTS, isRetryableResumeError);
            await this.withRetry(async () => {
    ...
    async withRetry(operation, maxAttempts, shouldRetry = () => true) {
        for (let attempt = 1;; attempt++) {
            try {
                ...
                if (attempt >= maxAttempts || !shouldRetry(error))
                    throw error;
                await (0, sleep_1.sleep)(100 * 2 ** (attempt - 1));
            }
        }
    }
    stopTracking() {
...
exports.WaitTracker = WaitTracker = __decorate([
    (0, di_1.Service)(),
    __metadata("design:paramtypes", [backend_common_1.Logger, db_1.ExecutionRepository, execution_persistence_1.ExecutionPersistence, ownership_service_1.OwnershipService, active_executions_1.ActiveExecutions, workflow_runner_1.WorkflowRunner, n8n_core_1.InstanceSettings])
], WaitTracker);
