...
const constants_3 = require("../../security-audit/constants");
...
let InstanceRiskReporter = class InstanceRiskReporter {
    constructor(instanceSettings, logger, globalConfig, outboundHttp) {
    ...
    async report(workflows) {
        ...
        if (outdatedState !== null) {
            report.sections.push({
                title: constants_3.INSTANCE_REPORT.SECTIONS.OUTDATED_INSTANCE,
        ...
        if (securitySettings !== null) {
            report.sections.push({
                title: constants_3.INSTANCE_REPORT.SECTIONS.SECURITY_SETTINGS,
                description: 'This n8n instance has the following security settings.',
                recommendation: `Consider adjusting the security settings for your n8n instance based on your needs. See: ${constants_3.ENV_VARS_DOCS_URL}`,
    ...
    getSecuritySettings() {
        ...
        const settings = {};
        settings.features = {
            communityPackagesEnabled: di_1.Container.get(community_packages_config_1.CommunityPackagesConfig).enabled,
    ...
    hasValidatorChild({ node, workflow, }) {
        ...
        if (!childNodeNames)
            return false;
        return childNodeNames.some((name) => workflow.nodes.find((n) => n.name === name && constants_3.WEBHOOK_VALIDATOR_NODE_TYPES.has(n.type)));
    }
    getUnprotectedWebhookNodes(workflows) {
        return workflows.reduce((acc, workflow) => {
            ...
            workflow.nodes.forEach((node) => {
                if (node.type === constants_3.WEBHOOK_NODE_TYPE &&
    ...
    async getOutdatedState() {
        let versions = [];
        const localVersion = constants_2.N8N_VERSION;
        try {
...
exports.InstanceRiskReporter = InstanceRiskReporter = __decorate([
    (0, di_1.Service)(),
    __metadata("design:paramtypes", [n8n_core_1.InstanceSettings, backend_common_1.Logger, config_1.GlobalConfig, backend_network_1.OutboundHttp])
