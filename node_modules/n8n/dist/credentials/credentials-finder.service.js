...
let CredentialsFinderService = class CredentialsFinderService {
    ...
    async findCredentialById(credentialId, options = {}) {
        return await this.credentialsRepository.findOne({
            where: {
                id: credentialId,
                usageScope: options.includeInstanceCredentials ? (0, typeorm_1.In)(['project', 'instance']) : 'project',
            },
        });
    }
    ...
    async findCredentialForUser(credentialsId, user, scopes, options = {}) {
        if (options.includeInstanceCredentials && (0, permissions_1.hasGlobalScope)(user, 'credential:manageInstance')) {
        ...
        if (sharedCredential) {
            if (sharedCredential.credentials.usageScope !== 'project')
                return null;
    ...
    async findCredentialIdsWithScopeForUser(credentialIds, user, scopes) {
        ...
        if (this.hasGlobalReadOnlyAccess(scopes)) {
            const globalCreds = await this.credentialsRepository.find({
                where: { id: (0, typeorm_1.In)(credentialIds), isGlobal: true, usageScope: 'project' },
                select: ['id'],
            });
            for (const gc of globalCreds)
                result.add(gc.id);
        }
        return result;
    }
...
exports.CredentialsFinderService = CredentialsFinderService = __decorate([
    (0, di_1.Service)(),
    __metadata("design:paramtypes", [db_1.SharedCredentialsRepository, db_1.CredentialsRepository, role_service_1.RoleService])
