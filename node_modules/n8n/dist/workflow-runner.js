...
let WorkflowRunner = class WorkflowRunner {
    constructor(logger, errorReporter, activeExecutions, executionRepository, executionPersistence, workflowStaticDataService, nodeTypes, credentialsPermissionChecker, instanceSettings, manualExecutionService, failedRunFactory, eventService, executionsConfig, storageConfig, externalHooks) {
        this.logger = logger;
    ...
    async failExecution(data, executionId, error, responsePromise) {
    ...
    async run(data, loadStaticData, realtime, existingExecution, responsePromise) {
        let establishContextError;
        if (data.executionData?.executionData) {
            const contextWorkflow = new n8n_workflow_1.Workflow({
            ...
            try {
                await (0, n8n_core_1.establishExecutionContext)(contextWorkflow, data.executionData, { encryptedRunnerIdentity: data.encryptedRunnerIdentity }, data.executionMode);
            }
            catch (error) {
                data.executionData.executionData.nodeExecutionStack = [];
                establishContextError = error;
            }
        }
        ...
        if (shouldEnqueue) {
            await this.enqueueExecution(executionId, workflowId, data, loadStaticData, realtime, existingExecution?.executionId);
        }
        else {
    ...
    needsFullExecutionData(executionMode, executionId, forceFullExecutionData) {
        if (forceFullExecutionData)
            return true;
...
exports.WorkflowRunner = WorkflowRunner = __decorate([
    (0, di_1.Service)(),
    __metadata("design:paramtypes", [backend_common_1.Logger, n8n_core_1.ErrorReporter, active_executions_1.ActiveExecutions, db_1.ExecutionRepository, execution_persistence_1.ExecutionPersistence, workflow_static_data_service_1.WorkflowStaticDataService, node_types_1.NodeTypes, pre_execution_checks_1.CredentialsPermissionChecker, n8n_core_1.InstanceSettings, manual_execution_service_1.ManualExecutionService, failed_run_factory_1.FailedRunFactory, event_service_1.EventService, config_1.ExecutionsConfig, n8n_core_1.StorageConfig, external_hooks_1.ExternalHooks])
], WorkflowRunner);
