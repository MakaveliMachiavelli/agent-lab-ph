...
const event_message_mcp_1 = require("../event-message-classes/event-message-mcp");
...
class MessageEventBusLogWriter {
    ...
    static async getInstance(options) {
        if (!MessageEventBusLogWriter.instance) {
            ...
            const logFullBasePath = options?.resolvedPath?.logFullBasePath ??
                (0, backend_common_1.safeJoinPath)(options?.logBasePath ?? di_1.Container.get(n8n_core_1.InstanceSettings).n8nFolder, options?.logBaseName ?? di_1.Container.get(config_1.GlobalConfig).eventBus.logWriter.logBaseName);
    ...
    async spawnThread() {
        ...
        if (backend_common_1.inTest) {
            ...
            workerFileName = (0, backend_common_1.safeJoinPath)(parsedName.dir, `${parsedName.name}-worker${parsedName.ext}`);
        }
    ...
    async readLoggedMessagesFromFile(results, mode, logFileName) {
        if (logFileName && (0, fs_1.existsSync)(logFileName)) {
            try {
                const stream = (0, fs_1.createReadStream)(logFileName);
                ...
                const maxTotalMessagesPerFile = this.globalConfig.eventBus.logWriter.maxTotalMessagesPerFile;
                const baselineCount = results.loggedMessages.length;
                let aborted = false;
                let parseSkipped = 0;
                let parseSkippedSample;
                rl.on('line', (line) => {
                    if (aborted)
                        return;
                    try {
                    ...
                    if (mode !== 'all' &&
                        ...
                        aborted = true;
                        this.logger.warn(`Event log ${logFileName} exceeded ${maxMessagesPerParse} in-memory messages during parse; aborting to prevent out-of-memory. Some unfinished execution recovery may be skipped. Tune via N8N_EVENTBUS_LOGWRITER_MAXMESSAGESPERPARSE.`);
                        rl.close();
                        stream.destroy();
                        return;
                    }
                    if (results.loggedMessages.length - baselineCount + parseSkipped >=
                        maxTotalMessagesPerFile) {
                        aborted = true;
                        this.logger.warn(`Event log ${logFileName} exceeded ${maxTotalMessagesPerFile} total lines during parse; aborting to prevent out-of-memory. Tune via N8N_EVENTBUS_LOGWRITER_MAXTOTALMESSAGESPERFILE.`);
                        rl.close();
                        stream.destroy();
                    }
                });
                ...
                if (parseSkipped > 0) {
                    this.logger.warn(`Event log parse skipped ${parseSkipped} malformed line(s) in ${logFileName}. Sample (truncated): ${parseSkippedSample}`);
                }
            }
            catch {
                this.logger.error(`Error reading logged messages from file: ${logFileName}`);
    ...
    processLoggedLine(line, results, mode) {
    ...
    getEventMessageObjectByType(message) {
        switch (message.__type) {
            ...
            case "$$EventMessageMcp":
                return new event_message_mcp_1.EventMessageMcp(message);
            default:
                return null;
        }
    }
