...
exports.createKnowledgeRetrievalTools = createKnowledgeRetrievalTools;
...
function formatKnowledgeToolError(error) {
...
async function runKnowledgeTool(operation) {
    try {
...
function isKnowledgeToolErrorOutput(output) {
    return (isRecord(output) && typeof output.error === 'string' && typeof output.errorType === 'string');
}
function isGlobKnowledgeFilesResult(output) {
    return isRecord(output) && Array.isArray(output.files) && typeof output.hasMore === 'boolean';
}
function isSearchKnowledgeResult(output) {
    if (!isRecord(output) ||
    ...
    if (output.outputMode === 'count')
        return Array.isArray(output.counts);
    return false;
}
function isReadKnowledgeResult(output) {
...
function toGlobKnowledgeModelOutput(output) {
    if (isKnowledgeToolErrorOutput(output) || !isGlobKnowledgeFilesResult(output))
        return output;
...
function toSearchKnowledgeModelOutput(output) {
    if (isKnowledgeToolErrorOutput(output) || !isSearchKnowledgeResult(output))
        return output;
    if (output.outputMode === 'files_with_matches') {
...
function toReadKnowledgeModelOutput(output) {
    if (isKnowledgeToolErrorOutput(output) || !isReadKnowledgeResult(output))
        return output;
    const perRangeBudget = Math.max(MODEL_OUTPUT_READ_RANGE_MIN_CHARS, Math.floor(MODEL_OUTPUT_READ_TEXT_BUDGET / Math.max(output.ranges.length, 1)));
