...
const searchPathSchema = zod_1.z.preprocess((value) => {
    const values = typeof value === 'string' ? [value] : value;
    if (!Array.isArray(values))
        return values;
    const scoped = values.filter((entry) => !(typeof entry === 'string' && ALL_FILES_PATH_PLACEHOLDERS.has(entry.trim())));
    return scoped.length === 0 ? undefined : scoped;
}, zod_1.z.array(filePathSchema).min(1).max(MAX_SEARCH_PATHS).optional());
...
exports.searchKnowledgeInputSchema = zod_1.z
    ...
    path: searchPathSchema.describe('Optional uploaded knowledge file path or paths to search within. Pass one exact file value or an array of exact file values copied from previous knowledge tool results to scope the search. Omit to search across every uploaded knowledge file.'),
...
exports.globKnowledgeFilesInputSchema = zod_1.z
    .object({
    pattern: globPatternSchema.describe('Filename pattern matched against uploaded knowledge file names, not file contents. Supports * and ? wildcards and is case-insensitive by default. Use `*` to list every uploaded file, `*.pdf` to filter by extension, or name fragments like `*bert*` when the user gives title or filename clues.'),
    limit: zod_1.z
        .number()
        .int()
        .min(1)
    ...
    offset: zod_1.z
        ...
        .min(0)
        .optional()
        .describe('Optional number of matching files to skip, for paging. When hasMore is true, call again with offset = previous offset + number of returned files.'),
