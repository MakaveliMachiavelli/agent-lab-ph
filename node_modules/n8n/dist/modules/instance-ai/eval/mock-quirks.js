...
exports.MOCK_QUIRKS = [
    ...
    {
        service: 'Telegram',
        guidance: 'Telegram has TWO call shapes on the same host (api.telegram.org). Pick by URL path:\n' +
        ...
        rationale: 'Telegram bot workflows commonly do `getFile` → download → process (e.g. voice → Whisper). The JSON envelope and the binary download are different request shapes, and the LLM picks the wrong one without explicit guidance.',
        addedAt: '2026-05-19',
    },
    {
        service: 'Openai',
        ...
        rationale: 'OpenAI mixes JSON, binary, and base64-in-JSON across endpoints, and AI workflow scenarios depend on the transcript text being plausibly downstream-matchable. Image generations are model-dependent: the gpt-image-1 family only ever returns b64_json (no url, no response_format), so a url-shaped mock crashes the n8n OpenAI image node with "first argument must be of type string ... Received undefined". A second observed failure is the mock returning a raw binary Buffer instead of the JSON envelope, crashing the node with "data is not iterable" — the guidance now pins json+b64_json (array of image objects, never binary) for that model family.',
        addedAt: '2026-05-19',
    },
    {
        service: 'Gmail',
    ...
    },
    {
        ...
        rationale: 'Drive file download uses `alt=media` on the same URL as metadata; the LLM otherwise treats every `/files/{id}` as metadata JSON and breaks the download path.',
        addedAt: '2026-05-19',
    },
    {
        service: 'Googleapis',
        endpoint: 'GET /gmail/v1/users/me/messages',
        guidance: 'Gmail messages.list → `{ "messages": [{ "id": "...", "threadId": "..." }], "resultSizeEstimate": <n> }`. The `messages` wrapper key is REQUIRED — never return a bare top-level array (the node reads `response.messages` and silently yields zero items without it). List entries need only `id` + `threadId`; subject/from/body come from the follow-up `GET /gmail/v1/users/me/messages/{id}`, which returns the full message with `payload.headers`.',
    ...
    },
    {
        service: 'Slack',
        hostnames: ['*.slack.com'],
        guidance: 'Slack file endpoints are ALL JSON metadata — NEVER pick `type: "binary"`. The response SHAPE differs per endpoint and matters: the Slack node parses specific keys and silently drops items when the shape is wrong.\n\n' +
    ...
    },
    {
        ...
        rationale: 'Dropbox file downloads happen on a different host; this service quirk covers only the JSON RPC host.',
        addedAt: '2026-05-19',
    },
    {
        service: 'S3',
        ...
        guidance: 'Amazon S3 routes by HTTP method on `<bucket>.s3.amazonaws.com` or `s3.<region>.amazonaws.com`:\n' +
        ...
        rationale: 'S3 mixes binary and empty responses on the same path skeleton; downstream nodes (Extract from File, image processing) expect a real file body for GET.',
        addedAt: '2026-05-19',
    },
    {
        service: 'Generativelanguage',
        hostnames: ['generativelanguage.googleapis.com'],
        guidance: 'Google Gemini `POST /v1beta/models/{model}:generateContent` (also `/v1/...`) → the FULL Gemini envelope: `{ "candidates": [{ "content": { "parts": [{ "text": "<answer>" }], "role": "model" }, "finishReason": "STOP", "index": 0 }], "usageMetadata": { "promptTokenCount": 10, "candidatesTokenCount": 20, "totalTokenCount": 30 } }`. The `candidates[].content.parts[].text` path is REQUIRED — the n8n Gemini node reads exactly that and yields nothing (or crashes) without it. NEVER return a bare `{ "text": ... }`, `{ "content": ... }`, or the answer object at the top level. When the request demands JSON output (`generationConfig.responseMimeType: "application/json"`, a `responseSchema`, or prompt instructions for a JSON document), the JSON goes INSIDE `parts[0].text` as a STRING — still wrapped in the full envelope.',
        rationale: 'The Gemini node parses candidates[].content.parts; bare payload objects yield empty output that downstream IF/parse nodes then misroute. Observed as mock_issue rows in run 29012884140 (whatsapp-faq-assistant, weekly-social-content-scheduler).',
        addedAt: '2026-07-09',
    },
    {
        service: 'Reddit',
        hostnames: ['*.reddit.com', 'reddit.com'],
        guidance: 'Reddit WRITE endpoints wrap the created resource in a `{ "json": { "errors": [], "data": { ... } } }` envelope — the n8n Reddit node reads `response.json.data`, so a bare resource object (no `json` wrapper) yields nothing.\n' +
        ...
        rationale: 'The Reddit node reads response.json.data for api/submit and response.json.data.things[0].data for api/comment; a flat resource object crashes/empties those reads. No quirk existed for Reddit before (TRUST-309).',
        addedAt: '2026-07-16',
    },
    {
        service: 'Hubapi',
...
function hostnameMatchesPattern(pattern, hostname) {
    const parts = pattern
        .split('.')
        .map((p) => (p === '*' ? '[^.]+' : p.replace(/[.+^${}()|[\]\\?*]/g, '\\$&')));
