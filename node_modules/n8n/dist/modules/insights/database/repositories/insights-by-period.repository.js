...
let InsightsByPeriodRepository = class InsightsByPeriodRepository extends typeorm_1.Repository {
    constructor(dataSource, sharedWorkflowRepository) {
    ...
    applyAccessFilter(qb, accessFilter) {
        if (!qb.expressionMap.joinAttributes.some((join) => join.alias.name === 'metadata')) {
            throw new n8n_workflow_1.UnexpectedError('The metadata relation must be joined before the access filter');
        }
        const subquery = this.sharedWorkflowRepository.buildSharedWorkflowIdsSubquery(accessFilter.user, {
    ...
    getPeriodStartExpr(periodUnitToCompactInto, callerTimeZone) {
        ...
        if (dbType === 'postgresdb') {
            if ((0, api_types_1.isValidTimeZone)(callerTimeZone.name)) {
        ...
        const localTruncatedExpr = periodUnitToCompactInto === 'week'
    ...
    async getPreviousAndCurrentPeriodTypeAggregates({ startDate, endDate, projectId, timeZone, accessFilter, }) {
        const cte = (0, insights_by_period_query_helper_1.getDateRangesCommonTableExpressionQuery)({ dbType, startDate, endDate, timeZone });
    ...
    async countInsightsByWorkflowGroups(rawRowsQuery) {
    ...
    async getInsightsByWorkflow({ startDate, endDate, skip = 0, take = 20, sortBy = 'total:desc', projectId, timeZone, accessFilter, }) {
    ...
    async getInsightsByTime({ periodUnit, insightTypes, projectId, startDate, endDate, timeZone, accessFilter, }) {
        const cte = (0, insights_by_period_query_helper_1.getDateRangesCommonTableExpressionQuery)({ dbType, startDate, endDate, timeZone });
        ...
        const callerTimeZone = timeZone
            ? { name: timeZone, offsetMinutes: luxon_1.DateTime.fromJSDate(startDate).setZone(timeZone).offset }
            : undefined;
...
exports.InsightsByPeriodRepository = InsightsByPeriodRepository = __decorate([
    (0, di_1.Service)(),
    __metadata("design:paramtypes", [typeorm_1.DataSource, db_1.SharedWorkflowRepository])
