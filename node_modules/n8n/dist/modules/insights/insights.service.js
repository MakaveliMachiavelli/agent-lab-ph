...
let InsightsService = class InsightsService {
    constructor(insightsByPeriodRepository, compactionService, pruningService, licenseState, instanceSettings, logger, workflowSharingService) {
    ...
    async resolveAccessFilter(user, projectId) {
        if (projectId) {
            const userHasRequiredProjectScopes = await (0, check_access_1.userHasScopes)(user, ['workflow:read'], false, {
                projectId,
            });
            if (!userHasRequiredProjectScopes) {
                throw new forbidden_error_1.ForbiddenError('You do not have access to insights for this project.');
            }
        }
        const workflowReadRoles = await this.workflowSharingService.rolesGrantingScope(user, 'workflow:read');
        return workflowReadRoles && { user, ...workflowReadRoles };
    }
    async getInsightsSummary({ user, startDate, endDate, projectId, timeZone, }) {
    ...
    async getInsightsByWorkflow({ user, skip = 0, take = 10, sortBy = 'total:desc', projectId, startDate, endDate, timeZone, }) {
        const accessFilter = await this.resolveAccessFilter(user, projectId);
    ...
    async getInsightsByTime({ user, insightTypes = Object.keys(insights_shared_1.TypeToNumber), projectId, startDate, endDate, timeZone, }) {
        const accessFilter = await this.resolveAccessFilter(user, projectId);
...
exports.InsightsService = InsightsService = __decorate([
    (0, di_1.Service)(),
    __metadata("design:paramtypes", [insights_by_period_repository_1.InsightsByPeriodRepository, insights_compaction_service_1.InsightsCompactionService, insights_pruning_service_1.InsightsPruningService, backend_common_1.LicenseState, n8n_core_1.InstanceSettings, backend_common_1.Logger, workflow_sharing_service_1.WorkflowSharingService])
], InsightsService);
