...
const flagsSchema = zod_1.z.object({
    ...
    activeState: zod_1.z
        ...
        .describe('Whether to respect the JSON active field. "false" (default) deactivates all imported workflows. "fromJson" activates/deactivates each workflow based on its JSON active field.')
        .default('false'),
});
let ImportWorkflowsCommand = class ImportWorkflowsCommand extends base_command_1.BaseCommand {
    ...
    async run() {
        ...
        if (flags.activeState === 'fromJson' && this.globalConfig.executions.mode !== 'queue') {
            throw new n8n_workflow_1.UserError('The "--activeState=fromJson" flag can only be used when n8n is running in queue or multi-main mode. In regular deployment mode, workflow activation is not supported.');
        }
        if (!flags.input) {
        ...
        const ownerUser = await di_1.Container.get(db_1.UserRepository).findOneByOrFail({
        ...
        await di_1.Container.get(import_service_1.ImportService).importWorkflows(workflows, project.id, userId, {
            activeState: flags.activeState,
        });
        this.reportSuccess(workflows.length);
        di_1.Container.get(event_service_1.EventService).emit('server-cli-import', {
            activeState: flags.activeState,
