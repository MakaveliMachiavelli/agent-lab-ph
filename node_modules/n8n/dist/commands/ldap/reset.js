...
const flagsSchema = zod_1.z.object({
    userId: zod_1.z
        .string()
        .describe('The ID of the user to assign the workflows and credentials owned by the deleted LDAP users to')
        .optional(),
    projectId: zod_1.z
        .string()
        .describe('The ID of the project to assign the workflows and credentials owned by the deleted LDAP users to')
        .optional(),
    deleteWorkflowsAndCredentials: zod_1.z
        .boolean()
        .describe('Delete all workflows and credentials owned by the users that were created by the users managed via LDAP.')
        .optional(),
});
let Reset = class Reset extends base_command_1.BaseCommand {
    ...
    async run() {
        ...
        if (flags.projectId ?? flags.userId) {
            ...
            await di_1.Container.get(ownership_transfer_service_1.OwnershipTransferService).transferAllResources(personalProjectIds, project.id);
        }
        ...
        await di_1.Container.get(ownership_transfer_service_1.OwnershipTransferService).deleteModuleOwnedResources(personalProjectIds);
...
exports.Reset = Reset = __decorate([
    (0, decorators_1.Command)({
        name: 'ldap:reset',
        description: 'Resets the database to the default ldap state.\n\nTHIS DELETES ALL LDAP MANAGED USERS.',
        examples: [
