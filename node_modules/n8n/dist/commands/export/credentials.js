...
const flagsSchema = zod_1.default.object({
    ...
    projectId: zod_1.default.string().describe('Export all credentials in the specified project').optional(),
    output: zod_1.default
        .string()
        .alias('o')
        .describe('Output file name or directory if using separate files')
        .optional(),
    pretty: zod_1.default.boolean().describe('Format the output in an easier to read fashion').optional(),
    separate: zod_1.default
        .boolean()
        .describe('Exports one file per credential (useful for versioning). Must inform a directory via --output.')
        .optional(),
    decrypted: zod_1.default
        .boolean()
        .describe('Exports data decrypted / in plain text. ALL SENSITIVE INFORMATION WILL BE VISIBLE IN THE FILES. Use to migrate from a installation to another that have a different secret key (in the config file).')
        .optional(),
});
let ExportCredentialsCommand = class ExportCredentialsCommand extends base_command_1.BaseCommand {
    async run() {
        ...
        const credentials = await di_1.Container.get(db_1.CredentialsRepository).find({
