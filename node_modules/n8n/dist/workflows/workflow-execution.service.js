...
let WorkflowExecutionService = class WorkflowExecutionService {
    constructor(logger, errorReporter, executionPersistence, workflowRepository, nodeTypes, testWebhooks, workflowRunner, globalConfig, subworkflowPolicyChecker, failedRunFactory, eventService, ownershipService, executionContextService, workflowsConfig, workflowPublishedDataService) {
        this.logger = logger;
    ...
    async runWorkflow(workflowData, node, data, additionalData, mode, responsePromise, deduplicationKey) {
        ...
        const { projectId, projectName } = await (0, utils_1.getWorkflowProjectDetailsSafe)(this.ownershipService, workflowData.id);
        const runData = {
    ...
    async executeManually(workflowData, payload, user, pushRef, n8nAuthCookie) {
        ...
        if (isFullExecutionFromKnownTrigger(payload)) {
            if ((payload.chatSessionId || triggerHasNoPinnedData(workflowData, payload)) &&
    ...
    async executeChatWorkflow(user, workflowData, executionData, httpResponse, streamingEnabled, executionMode = 'chat', pushRef) {
        const { projectId, projectName } = await (0, utils_1.getWorkflowProjectDetailsSafe)(this.ownershipService, workflowData.id);
        const data = {
            userId: user.id,
            executionMode,
    ...
    async loadErrorWorkflowData(workflowId, workflowErrorData) {
        ...
        if (this.workflowsConfig.useWorkflowPublicationService) {
            const publishedData = await this.workflowPublishedDataService.getPublishedWorkflowData(workflowId);
        ...
        const loaded = await this.workflowRepository.get({ id: workflowId }, { relations: ['activeVersion'] });
        if (loaded === null) {
...
exports.WorkflowExecutionService = WorkflowExecutionService = __decorate([
    (0, di_1.Service)(),
    __metadata("design:paramtypes", [backend_common_1.Logger, n8n_core_1.ErrorReporter, execution_persistence_1.ExecutionPersistence, db_1.WorkflowRepository, node_types_1.NodeTypes, test_webhooks_1.TestWebhooks, workflow_runner_1.WorkflowRunner, config_1.GlobalConfig, pre_execution_checks_1.SubworkflowPolicyChecker, failed_run_factory_1.FailedRunFactory, event_service_1.EventService, ownership_service_1.OwnershipService, n8n_core_1.ExecutionContextService, config_1.WorkflowsConfig, workflow_published_data_service_1.WorkflowPublishedDataService])
], WorkflowExecutionService);
function isPartialExecution(payload) {
    return ('runData' in payload && payload.runData !== undefined && payload.destinationNode !== undefined);
}
