...
const workflow_publication_status_service_1 = require("./publication/workflow-publication-status.service");
const workflow_creation_service_1 = require("./workflow-creation.service");
const workflow_entity_mapper_1 = require("./workflow-entity-mapper");
...
let WorkflowsController = class WorkflowsController {
    constructor(logger, authService, enterpriseWorkflowService, namingService, workflowRepository, workflowService, workflowCreationService, workflowExecutionService, license, mailer, projectRepository, projectService, projectRelationRepository, eventService, globalConfig, workflowFinderService, executionService, collaborationService, ssrfConfig, ssrfProtectionService, outboundHttp, workflowPublicationStatusService, ownershipService) {
        ...
        this.workflowPublicationStatusService = workflowPublicationStatusService;
    ...
    async create(req, _res, body) {
        ...
        const newWorkflow = (0, workflow_entity_mapper_1.createWorkflowEntityFromPayload)(body);
    ...
    async update(req, _res, workflowId, body) {
        ...
        const updateData = (0, workflow_entity_mapper_1.createWorkflowEntityFromPayload)(rest);
    ...
    async runManually(req) {
        const parseResult = api_types_1.ManualRunDto.safeParse(req.body);
        ...
        const result = await this.workflowExecutionService.executeManually(dbWorkflow, body, req.user, req.headers['push-ref'], n8nAuthCookie);
        if ('executionId' in result) {
            const { projectId, projectName } = await (0, utils_1.getWorkflowProjectDetailsSafe)(this.ownershipService, dbWorkflow.id);
    ...
    async getPublicationStatus(req, _res, workflowId) {
        if (!this.globalConfig.workflows.useWorkflowPublicationService) {
            throw new not_found_error_1.NotFoundError('Workflow publication status is not available');
        }
        const workflow = await this.workflowFinderService.findWorkflowForUser(workflowId, req.user, [
            'workflow:read',
        ]);
    ...
    async getWorkflowsWithNodesIncluded(req, res) {
        try {
            const hasPermission = (0, permissions_1.hasGlobalScope)(req.user, ['workflow:read']);
            if (!hasPermission) {
    ...
    findSsrfBlockedError(error) {
        let current = (0, ensure_error_1.ensureError)(error);
        for (let depth = 0; depth < 4 && current; depth++) {
            if (current instanceof backend_network_1.SsrfBlockedIpError)
                return current;
            current = (0, ensure_error_1.ensureError)(current.cause);
        }
        return undefined;
    }
};
...
__decorate([
    (0, decorators_1.Get)('/:workflowId/publication-status'),
    (0, decorators_1.ProjectScope)('workflow:read'),
    __param(2, (0, decorators_1.Param)('workflowId')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, Object, String]),
    __metadata("design:returntype", Promise)
], WorkflowsController.prototype, "getPublicationStatus", null);
__decorate([
...
exports.WorkflowsController = WorkflowsController = __decorate([
    ...
    __metadata("design:paramtypes", [backend_common_1.Logger, auth_service_1.AuthService, workflow_service_ee_1.EnterpriseWorkflowService, naming_service_1.NamingService, db_1.WorkflowRepository, workflow_service_1.WorkflowService, workflow_creation_service_1.WorkflowCreationService, workflow_execution_service_1.WorkflowExecutionService, license_1.License, email_1.UserManagementMailer, db_1.ProjectRepository, project_service_ee_1.ProjectService, db_1.ProjectRelationRepository, event_service_1.EventService, config_1.GlobalConfig, workflow_finder_service_1.WorkflowFinderService, execution_service_1.ExecutionService, collaboration_service_1.CollaborationService, config_1.SsrfProtectionConfig, backend_network_1.SsrfProtectionService, backend_network_1.OutboundHttp, workflow_publication_status_service_1.WorkflowPublicationStatusService, ownership_service_1.OwnershipService])
], WorkflowsController);
