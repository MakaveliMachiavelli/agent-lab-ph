...
let BinaryDataController = class BinaryDataController {
    ...
    async getSigned(_, res, { token }) {
        try {
            ...
            if (error instanceof n8n_core_1.FileNotFoundError)
                return res.status(404).end();
            if (error instanceof bad_request_error_1.BadRequestError || error instanceof jsonwebtoken_1.JsonWebTokenError)
                return res.status(400).end(error.message);
            else
                throw error;
        }
    ...
    async setContentHeaders(binaryDataId, action, res, fileName, mimeType) {
        ...
        if (action === 'view' && (!mimeType || !api_types_1.ViewableMimeTypes.includes(mimeType.toLowerCase()))) {
...
__decorate([
    ...
    __metadata("design:returntype", Promise)
], BinaryDataController.prototype, "getSigned", null);
