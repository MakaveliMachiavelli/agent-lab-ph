...
exports.MicrosoftEntraServicePrincipalApi = void 0;
exports.getAccessToken = getAccessToken;
const client_assertion_1 = require("@n8n/utils/client-assertion");
const n8n_workflow_1 = require("n8n-workflow");
const token_request_1 = require("./common/token-request");
...
async function getAccessToken(credentials) {
    ...
    if (!tenantId || !clientId || !hasAuthSecret) {
        throw new n8n_workflow_1.OperationalError('Microsoft Entra credentials are incomplete');
    }
    // Keep the rejection generic — never echo the value.
    if (!TENANT_ID_GUID.test(tenantId) && !TENANT_ID_DOMAIN.test(tenantId)) {
        throw new n8n_workflow_1.OperationalError('Microsoft Entra tenant ID is not a valid GUID or domain');
    }
    ...
    if (normalizedBaseUrl && !Object.hasOwn(LOGIN_HOSTS_BY_GRAPH_URL, normalizedBaseUrl)) {
        throw new n8n_workflow_1.OperationalError('Microsoft Entra Graph API base URL is not a recognized Microsoft cloud');
    }
    ...
    if (authentication === 'certificate') {
        body.append('client_assertion_type', client_assertion_1.CLIENT_ASSERTION_TYPE);
        body.append('client_assertion', (0, client_assertion_1.buildClientAssertion)({ clientId, accessTokenUri: tokenUrl, privateKey, certificate }));
    }
    else {
    ...
    const http = (0, token_request_1.getTokenRequestClient)('fixed-vendor');
    const response = await http.request({
        ...
        json: true,
        timeout: token_request_1.TOKEN_REQUEST_TIMEOUT,
    });
    ...
    if (!hasAccessToken(response)) {
        throw new n8n_workflow_1.OperationalError('Microsoft Entra authentication did not return an access token');
    }
    return response.access_token;
}
class MicrosoftEntraServicePrincipalApi {
...
exports.MicrosoftEntraServicePrincipalApi = MicrosoftEntraServicePrincipalApi;
