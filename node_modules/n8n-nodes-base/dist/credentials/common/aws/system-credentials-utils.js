...
exports.getRoleForServiceAccountCredentials = getRoleForServiceAccountCredentials;
const config_1 = require("@n8n/config");
const di_1 = require("@n8n/di");
const n8n_workflow_1 = require("n8n-workflow");
const promises_1 = require("fs/promises");
const regions_1 = require("./regions");
const system_credentials_sdk_1 = require("./system-credentials-sdk");
 ...
 */
async function getSystemCredentials(region) {
    if (!di_1.Container.get(config_1.SecurityConfig).awsSystemCredentialsAccess) {
        throw new n8n_workflow_1.UserError('Access to AWS system credentials disabled, contact your administrator.');
    }
    ...
    const chain = [
        { source: 'environment', resolve: exports.credentialsResolver.environment },
        {
            source: 'roleForServiceAccount',
            resolve: async () => await getRoleForServiceAccountCredentials(region),
        },
        { source: 'podIdentity', resolve: exports.credentialsResolver.podIdentity },
        { source: 'containerMetadata', resolve: exports.credentialsResolver.containerMetadata },
        { source: 'instanceMetadata', resolve: exports.credentialsResolver.instanceMetadata },
    ];
...
async function getEnvironmentCredentials() {
    if ((0, system_credentials_sdk_1.usesSdk)('environment'))
        return await (0, system_credentials_sdk_1.resolveEnvironmentViaSdk)();
...
async function getInstanceMetadataCredentials() {
    if ((0, system_credentials_sdk_1.usesSdk)('instanceMetadata'))
        return await (0, system_credentials_sdk_1.resolveInstanceMetadataViaSdk)();
    try {
...
async function getContainerMetadataCredentials() {
    if ((0, system_credentials_sdk_1.usesSdk)('containerMetadata'))
        return await (0, system_credentials_sdk_1.resolveContainerMetadataViaSdk)();
    try {
...
async function getPodIdentityCredentials() {
    if ((0, system_credentials_sdk_1.usesSdk)('podIdentity'))
        return await (0, system_credentials_sdk_1.resolvePodIdentityViaSdk)();
 ...
 */
async function getRoleForServiceAccountCredentials(region) {
    if ((0, system_credentials_sdk_1.usesSdk)('roleForServiceAccount'))
        return await (0, system_credentials_sdk_1.resolveWebIdentityViaSdk)(region);
    const iamRole = (0, exports.envGetter)('AWS_ROLE_ARN');
    const webIdentityTokenFile = (0, exports.envGetter)('AWS_WEB_IDENTITY_TOKEN_FILE');
    try {
        if (!iamRole || !webIdentityTokenFile) {
            return null;
        }
        const token = (await (0, promises_1.readFile)(webIdentityTokenFile, 'utf8')).trim();
        if (!token) {
            return null;
        }
        const headers = {
        ...
        const credentialsResponse = await fetch(`https://sts.${region}.${(0, regions_1.getAwsDomain)(region)}`, {
            method: 'POST',
            headers,
            body: body.toString(),
            signal: AbortSignal.timeout(2000),
