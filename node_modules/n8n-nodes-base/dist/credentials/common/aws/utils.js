...
exports.AWS_REGION_SHAPE_PATTERN = exports.awsCredentialsTest = void 0;
exports.assertSupportedAwsRegion = assertSupportedAwsRegion;
exports.validateBedrockEndpointOverride = validateBedrockEndpointOverride;
exports.parseAwsUrl = parseAwsUrl;
exports.uriEncodeS3Pathname = uriEncodeS3Pathname;
exports.awsGetSignInOptionsAndUpdateRequest = awsGetSignInOptionsAndUpdateRequest;
exports.assumeRole = assumeRole;
exports.splitPathAndQuery = splitPathAndQuery;
exports.splitHostPort = splitHostPort;
exports.buildSmithyHttpRequest = buildSmithyHttpRequest;
...
// ── Private ──────────────────────────────────────────────────────────────────
const SUPPORTED_AWS_REGIONS = new Set(regions_1.regions.map((r) => r.name));
/**
 ...
 */
function getAwsSigningService(service) {
 ...
 */
function assertValidRoleCredentials(credentials) {
 ...
 */
function buildMasterCredentials(credentials, region) {
    if (credentials.useSystemCredentialsForRole) {
        return async () => {
            const sys = await (0, system_credentials_utils_1.getSystemCredentials)(region);
            if (!sys) {
 ...
 */
function buildStsRequestHandler(region) {
    const stsTarget = `https://sts.${region}.${(0, regions_1.getAwsDomain)(region)}`;
    const proxyUrl = (0, proxy_1.resolveProxyUrl)(stsTarget);
    if (!proxyUrl)
        return undefined;
    // STS is always HTTPS; one agent backs both http/https slots.
    const proxyAgent = (0, proxy_1.createHttpsProxyAgent)(stsTarget, proxyUrl);
    return new node_http_handler_1.NodeHttpHandler({ httpAgent: proxyAgent, httpsAgent: proxyAgent });
}
// ── Exports ───────────────────────────────────────────────────────────────────
/**
...
function isSupportedAwsRegion(region) {
 ...
 */
function isAwsEndpointHostname(hostname) {
...
function assertSupportedAwsRegion(region) {
    if (typeof region !== 'string' || !isSupportedAwsRegion(region)) {
        throw new n8n_workflow_1.UserError('Unsupported AWS region');
    }
}
/**
 ...
 */
function validateBedrockEndpointOverride(override, region) {
    ...
    try {
        ...
        throw new n8n_workflow_1.UserError('Bedrock endpoint is not a valid URL');
    }
    if (url.protocol !== 'http:' && url.protocol !== 'https:') {
        throw new n8n_workflow_1.UserError('Bedrock endpoint must use the http or https scheme');
    }
 ...
 */
exports.AWS_REGION_SHAPE_PATTERN = /^[a-z]{2,4}(-[a-z]+)+-\d+$/;
/**
 ...
 */
function parseLegacyS3DashRegion(label) {
    if (!label.startsWith('s3-'))
        return null;
    const rest = label.slice(3);
    return exports.AWS_REGION_SHAPE_PATTERN.test(rest) ? rest : null;
}
/**
...
function parseAwsUrl(url) {
    ...
    for (let i = labels.length - 1; i >= 0; i--) {
        if (exports.AWS_REGION_SHAPE_PATTERN.test(labels[i])) {
            regionIdx = i;
            break;
        }
    }
    ...
    if (regionIdx !== -1 &&
        ...
        if (labels[serviceIdx] === 'dualstack')
            serviceIdx--;
        ...
        if (labels[serviceIdx] === 'dualstack' && serviceIdx > 0)
            serviceIdx--;
 ...
 */
function resolveServiceAndRegion(url, service, region) {
    ...
    if (parsed.region) {
        if (isSupportedAwsRegion(parsed.region)) {
            ...
            throw new n8n_workflow_1.UserError(`Unsupported AWS region "${parsed.region}" parsed from endpoint host ${url.hostname}`);
        }
    }
 ...
 */
function uriEncodeS3Pathname(pathname) {
    return pathname
        .split('/')
        .map((segment) => {
        ...
        return encodeURIComponent(decoded).replace(/[!'()*]/g, (char) => `%${char.charCodeAt(0).toString(16).toUpperCase()}`);
    })
        .join('/')
        .replace(/%2F/g, '/');
}
    ...
    if (requestWithUri.uri) {
        ...
        if (!requestOptions.baseURL && !requestOptions.url) {
            ...
            if (service === 'lambda' && credentials.lambdaEndpoint) {
                ...
                const domain = (0, regions_1.getAwsDomain)(region);
            ...
            if (service === 'bedrock' && credentials.bedrockEndpoint) {
                const override = new URL(validateBedrockEndpointOverride(credentials.bedrockEndpoint, region));
...
async function assumeRole(credentials, region) {
    ...
    try {
        ...
        if (err instanceof n8n_workflow_1.UserError)
            throw err;
        const message = err instanceof Error ? err.message : String(err);
        throw new n8n_workflow_1.UserError(`STS AssumeRole failed: ${message}`);
    }
}
...
function splitPathAndQuery(pathWithSearch) {
    const idx = pathWithSearch.indexOf('?');
    if (idx === -1)
        return { path: pathWithSearch, query: {} };
    const query = {};
...
function signWithLegacyAws4(requestOptions, signOpts, securityHeaders, url, method) {
...
function buildSmithyHttpRequest(signOpts, method) {
    ...
    const headers = { host: signOpts.host ?? hostname };
    for (const [k, v] of Object.entries(signOpts.headers ?? {})) {
        const lower = k.toLowerCase();
        if (lower === 'host' || v === undefined || v === null)
            continue;
...
async function signOptions(requestOptions, signOpts, securityHeaders, url, method) {
