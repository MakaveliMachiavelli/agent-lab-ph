...
exports.usesSdk = usesSdk;
exports.resolveEnvironmentViaSdk = resolveEnvironmentViaSdk;
exports.resolveWebIdentityViaSdk = resolveWebIdentityViaSdk;
exports.resolvePodIdentityViaSdk = resolvePodIdentityViaSdk;
exports.resolveContainerMetadataViaSdk = resolveContainerMetadataViaSdk;
exports.resolveInstanceMetadataViaSdk = resolveInstanceMetadataViaSdk;
 ...
 */
function usesSdk(resolver) {
    const raw = (di_1.Container.get(config_1.SecurityConfig).awsSystemCredentialsSdkSources ?? '').trim();
    if (raw === '' || raw === 'all')
        return true;
    if (raw === 'none')
        return false;
...
async function resolveEnvironmentViaSdk() {
...
async function resolveWebIdentityViaSdk(region) {
    ...
    return await runProvider(fromWebToken({
        webIdentityToken,
        roleArn,
...
async function resolvePodIdentityViaSdk() {
    ...
    let awsContainerAuthorizationToken;
    if (tokenFile) {
        try {
            const { readFile } = await import('node:fs/promises');
    ...
    return await runProvider(fromHttp({
...
async function resolveContainerMetadataViaSdk() {
...
async function resolveInstanceMetadataViaSdk() {
...
//# sourceMappingURL=system-credentials-sdk.js.map