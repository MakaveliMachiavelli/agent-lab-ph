...
const SYMMETRIC_SCRYPT_OPTIONS = { N: 65536, r: 8, p: 1, maxmem: 128 * 1024 * 1024 };
async function deriveKey(passphrase, salt, keylen) {
    return await new Promise((resolve, reject) => {
        (0, crypto_1.scrypt)(passphrase, salt, keylen, SYMMETRIC_SCRYPT_OPTIONS, (error, key) => {
            if (error)
                reject(error);
            else
                resolve(key);
...
const versionDescription = {
    ...
    properties: [
        ...
        },
        {
            ...
            default: 'aes-256-gcm',
            description: 'Authenticated cipher to use. The same value must be selected on encrypt and decrypt.',
            displayOptions: {
                show: {
                    action: ['encrypt', 'decrypt'],
                    mode: ['symmetric'],
        ...
        },
        {
            ...
            type: 'string',
            default: '',
            description: 'For Encrypt: the plaintext to encrypt. For Decrypt: the base64 ciphertext produced by this node.',
            displayOptions: {
                show: {
                    action: ['encrypt', 'decrypt'],
                },
    ...
    async execute() {
        ...
        if (action === 'hmac' || action === 'sign' || action === 'encrypt' || action === 'decrypt') {
            ...
            if (action === 'sign') {
                ...
                signPrivateKey = (0, format_pem_block_1.formatPemBlock)(credentials.signPrivateKey);
            }
            if (action === 'encrypt' || action === 'decrypt') {
                const mode = this.getNodeParameter('mode', 0);
                if (mode === 'symmetric') {
                    if (!credentials.encryptionPassphrase) {
                        throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'No encryption passphrase set in credentials. Please add an Encryption Passphrase to your Crypto credentials.');
                    }
                    encryptionPassphrase = credentials.encryptionPassphrase;
                }
                if (mode === 'asymmetric' && action === 'encrypt') {
                    if (!credentials.encryptionPublicKey) {
                        throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'No encryption public key set in credentials. Please add an Encryption Public Key to your Crypto credentials.');
                    }
                    encryptionPublicKey = (0, format_pem_block_1.formatPemBlock)(credentials.encryptionPublicKey, true);
                }
                if (mode === 'asymmetric' && action === 'decrypt') {
                    if (!credentials.encryptionPrivateKey) {
                        throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'No encryption private key set in credentials. Please add an Encryption Private Key to your Crypto credentials.');
                    }
                    encryptionPrivateKey = (0, format_pem_block_1.formatPemBlock)(credentials.encryptionPrivateKey);
                }
            }
        }
        let item;
        for (let i = 0; i < length; i++) {
            try {
                ...
                if (action === 'encrypt') {
                    ...
                    if (mode === 'symmetric') {
                        const cipher = this.getNodeParameter('cipher', i);
                        const keyLength = CIPHER_KEY_LENGTHS[cipher];
                        const salt = (0, crypto_1.randomBytes)(SYMMETRIC_SALT_LENGTH);
                        const iv = (0, crypto_1.randomBytes)(SYMMETRIC_IV_LENGTH);
                        ...
                        const cipherInstance = (0, crypto_1.createCipheriv)(cipher, key, iv);
                        ...
                        try {
                            ...
                            if (opensslError.code === 'ERR_OSSL_RSA_DATA_TOO_LARGE_FOR_KEY_SIZE' ||
                                /data too large/i.test(opensslError.message ?? '')) {
                                throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'Plaintext is too large for the RSA key. Use symmetric mode for larger data.', { itemIndex: i });
                            }
                            throw error;
                        }
                    }
                ...
                if (action === 'decrypt') {
                    ...
                    try {
                        if (mode === 'symmetric') {
                            ...
                            const headerLength = 1 + SYMMETRIC_SALT_LENGTH + SYMMETRIC_IV_LENGTH + SYMMETRIC_AUTH_TAG_LENGTH;
                            if (payload.length < headerLength) {
                                throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'Ciphertext is malformed or truncated', { itemIndex: i });
                            }
                            const version = payload[0];
                            if (version !== SYMMETRIC_FORMAT_VERSION) {
                                throw new n8n_workflow_1.NodeOperationError(this.getNode(), `Unsupported ciphertext version 0x${version.toString(16).padStart(2, '0')}`, { itemIndex: i });
                            }
                        ...
                        if (error instanceof n8n_workflow_1.NodeOperationError)
                            throw error;
                        throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'Decryption failed: wrong passphrase, key, cipher, or corrupted payload', { itemIndex: i });
                    }
                }
                let newItem;
