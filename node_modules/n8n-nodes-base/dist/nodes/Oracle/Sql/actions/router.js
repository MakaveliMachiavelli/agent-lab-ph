...
async function router() {
    ...
    let pool;
    try {
        pool = await transport_1.configureOracleDB.call(this, credentials, options);
    }
    catch (error) {
        if (!continueOnFail)
            throw error;
        const errorItems = items.length > 0
            ? items.map((_item, index) => (0, utils_1.prepareErrorItem)(error, index))
            : [(0, utils_1.prepareErrorItem)(error, 0)];
        return [errorItems];
    }
    const runQueries = utils_1.configureQueryRunner.call(this, node, continueOnFail, pool);
