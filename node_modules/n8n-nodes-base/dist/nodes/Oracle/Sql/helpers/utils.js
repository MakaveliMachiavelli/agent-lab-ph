...
exports.configureTableSchemaUpdater = exports.DEFAULT_STRING_OUT_BIND_MAX_SIZE = void 0;
...
exports.DEFAULT_STRING_OUT_BIND_MAX_SIZE = 4000;
...
async function _getResponseForOutbinds(results, stmtBatching, outputColumns = [], returnData = [], serializeDates = false) {
    if (results.outBinds) {
        ...
        for (let j = 0; j < normalizedRows.length; j++) {
            ...
            if (serializeDates) {
                ...
                const routed = await binary_1.routeBinaryProperties.call(this, row);
                row = routed.json;
                binary = routed.binary;
            }
            const executionData = this.helpers.constructExecutionMetaData((0, utilities_1.wrapData)(row), {
                itemData: { item: j },
            });
...
async function _getResponseForRows(rows, itemIndex, routeBinary) {
    if (!routeBinary) {
        return this.helpers.constructExecutionMetaData((0, utilities_1.wrapData)(rows), {
    ...
    for (const row of rows) {
        ...
        const { json, binary } = await binary_1.routeBinaryProperties.call(this, row);
        const executionData = this.helpers.constructExecutionMetaData((0, utilities_1.wrapData)(json), {
            itemData: { item: itemIndex },
        });
        for (const entry of executionData) {
            if (Object.keys(binary).length) {
...
function createErrorItems(error, items) {
    if (items.length) {
        return items.map((_item, index) => prepareErrorItem(error, index));
    }
    return [prepareErrorItem(error, 0)];
}
async function getConnectionOrError(pool, node, continueOnFail) {
    try {
    ...
    return async (queries, items, options) => {
        ...
        if (stmtBatching === 'transaction' || stmtBatching === 'independently') {
            ...
            if (node.typeVersion >= 1.1) {
                ...
                dateDbTypes.push(oracledb_1.default.DB_TYPE_TIMESTAMP);
            }
            const executeFetchHandler = function (metaData) {
                if (metaData.dbType && dateDbTypes.includes(metaData.dbType)) {
                    return {
                        converter: (val) => {
        ...
        if (stmtBatching === 'single' && queries[0].executeManyValues) {
            ...
            try {
                ...
                if (results.batchErrors) {
                    ...
                    await _getResponseForOutbinds.call(this, results, stmtBatching, queries[0].outputColumns, returnData, serializeDates);
                }
            ...
            try {
                for (let i = 0; i < queries.length; i++) {
                    try {
                        ...
                        await _getResponseForOutbinds.call(this, transactionResults, stmtBatching, outputColumns, resultOutBinds, serializeDates);
                        if (!resultOutBinds.length) {
                            let rowData = transactionResults.rows ?? [];
                            if (!rowData.length) {
                                rowData = [emptyRowData];
                            }
                            const executionData = await _getResponseForRows.call(this, rowData, i, serializeDates);
            ...
            try {
                for (let i = 0; i < queries.length; i++) {
                    try {
                        ...
                        const resultOutBinds = [];
                        await _getResponseForOutbinds.call(this, taskResults, stmtBatching, outputColumns, resultOutBinds, serializeDates);
                        if (!resultOutBinds.length) {
...
function generateBindVariablesList(item, bindParameters, query) {
    ...
    return n8n_workflow_1.safeRegex.replace(`:${escapedName}(?![A-Za-z0-9_$#])`, query, 'g', generatedSqlString);
}
...
function getBindParameters(query, parameterList, options = {}) {
    const bindParameters = {};
    const stringOutBindMaxSize = options.stringOutBindMaxSize ?? exports.DEFAULT_STRING_OUT_BIND_MAX_SIZE;
    for (const item of parameterList) {
        const itemWithOptionalDatatype = item;
        const bindItem = (!itemWithOptionalDatatype.datatype
            ? {
        ...
        if (!bindItem.parseInStatement) {
            ...
            const dir = bindItem.bindDirection === 'in'
