...
exports.processStreamInChunks = processStreamInChunks;
 ...
 */
async function processStreamInChunks(stream, chunkSize, onChunk) {
    const accumulated = [];
                        ...
                        preSend: [
                            ...
                            async function (requestOptions) {
                                    ...
                                    if (binaryData.id) {
                                        ...
                                        content =
                                            Number.isFinite(contentLength) && contentLength === 0
                                                ? Buffer.alloc(0)
                                                : await this.helpers.getBinaryStream(binaryData.id, RESUMABLE_UPLOAD_CHUNK_SIZE);
                                    }
                                    else {
                                ...
                                if (content instanceof stream_1.Readable) {
                                    ...
                                    const objectName = this.getNodeParameter('objectName');
                                    const uploadHeaders = {
                                        ...requestOptions.headers,
                                    ...
                                    if (authenticationMethod === 'serviceAccount') {
                                        const accessToken = await GenericFunctions_1.fetchServiceAccountToken.call(this);
                                        ...
                                        uploadSessionResponse = await this.helpers.httpRequestWithAuthentication.call(this, 'googleCloudStorageOAuth2Api', {
                                            method: 'POST',
                                            url: `/b/${bucketName}/o/`,
                                            baseURL: 'https://storage.googleapis.com/upload/storage/v1',
                                            qs: {
                                    ...
                                    const uploadUrl = uploadSessionResponse.headers.location;
                                    if (!uploadUrl) {
                                        throw new n8n_workflow_1.NodeApiError(this.getNode(), uploadSessionResponse.body);
                                    }
                                    ...
                                    if (Number.isFinite(contentLength)) {
                                        await processStreamInChunks(content, RESUMABLE_UPLOAD_CHUNK_SIZE, async (chunk, offset) => await uploadChunk(chunk, offset, contentLength));
                                    }
                                    else {
                                        let pendingChunk;
                                        let pendingOffset = 0;
                                        await processStreamInChunks(content, RESUMABLE_UPLOAD_CHUNK_SIZE, async (chunk, currentOffset) => {
                                            if (pendingChunk) {
                                        ...
                                        if (pendingChunk) {
                                            await uploadChunk(pendingChunk, pendingOffset, pendingOffset + pendingChunk.length);
                                        }
                                    }
                                    ...
                                    requestOptions.headers = this.getNodeParameter('encryptionHeaders');
