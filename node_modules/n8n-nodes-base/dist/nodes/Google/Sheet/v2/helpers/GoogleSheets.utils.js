...
async function autoMapInputData(sheetNameWithRange, sheet, items, options, prefetchedColumnNames) {
    const returnData = [];
    ...
    const columnNamesForHint = columnNames.filter((name) => name !== GoogleSheets_types_1.ROW_NUMBER);
...
function checkForSchemaChanges(node, columnNames, schema) {
    ...
    const schemaColumns = liveColumns.has(GoogleSheets_types_1.ROW_NUMBER)
        ? schema.map((s) => s.id)
