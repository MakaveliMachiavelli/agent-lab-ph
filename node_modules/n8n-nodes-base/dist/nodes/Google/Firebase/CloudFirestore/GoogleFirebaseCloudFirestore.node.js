        ...
        if (resource === 'document') {
            if (operation === 'get') {
                ...
                await Promise.all(items.map(async (_, i) => {
                    ...
                    const body = (0, query_parameters_1.parseAndResolveQueryParameters)(query, queryParameters, this.getNode(), i);
                    responseData = await GenericFunctions_1.googleApiRequest.call(this, 'POST', `/${projectId}/databases/${database}/documents:runQuery`, body);
                    responseData = responseData.map((element) => {
                        if (element.document) {
