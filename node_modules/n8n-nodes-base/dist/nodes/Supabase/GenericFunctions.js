...
exports.buildGetQuery = exports.buildOrQuery = exports.buildQuery = void 0;
exports.getSchemaHeader = getSchemaHeader;
exports.supabaseApiRequest = supabaseApiRequest;
exports.getFilters = getFilters;
exports.validateCredentials = validateCredentials;
exports.mapPairedItemsFrom = mapPairedItemsFrom;
const n8n_workflow_1 = require("n8n-workflow");
function getSchemaHeader(context, method, contextType) {
...
function getPostgrestOperator(value) {
    ...
    if (condition !== 'fullText') {
        if (!supportedFilterConditions.has(condition)) {
            throw new n8n_workflow_1.UserError(`Unsupported filter condition: "${condition}"`);
        }
        return condition;
    }
    const searchFunction = String(value.searchFunction);
    if (!supportedSearchFunctions.has(searchFunction)) {
        throw new n8n_workflow_1.UserError(`Unsupported search function: "${searchFunction}"`);
    }
    return searchFunction;
}
const buildQuery = (query, value) => query.set(quotePostgrestComponent(value.keyName), `${getPostgrestOperator(value)}.${String(value.keyValue)}`);
exports.buildQuery = buildQuery;
const buildOrQuery = (value) => `${quotePostgrestComponent(value.keyName)}.${getPostgrestOperator(value)}.${quotePostgrestComponent(value.keyValue)}`;
exports.buildOrQuery = buildOrQuery;
const buildGetQuery = (query, value) => query.set(quotePostgrestComponent(value.keyName), `eq.${String(value.keyValue)}`);
