...
const checkResponseModeConfiguration = (context) => {
    ...
    const respondToWebhookNodes = connectedNodes.filter((node) => node.type === 'n8n-nodes-base.respondToWebhook');
    ...
    if (respondToWebhookNodes.length > 0 && !['responseNode', 'streaming'].includes(responseMode)) {
        ...
        const isOwnedByDownstreamWait = (respondNode) => context
            .getParentNodes(respondNode.name, { includeNodeParameters: true })
            .some((node) => descendantNames.has(node.name) &&
            node.type === 'n8n-nodes-base.wait' &&
            !node.disabled &&
            ['webhook', 'form'].includes(node.parameters?.resume ?? '') &&
            node.parameters?.responseMode === 'responseNode');
        const hasUnusedRespondNode = respondToWebhookNodes.some((node) => !isOwnedByDownstreamWait(node));
...
async function validateWebhookAuthentication(ctx, authPropertyName) {
    const authentication = ctx.getNodeParameter(authPropertyName, 'none');
    ...
    if (authentication === 'basicAuth') {
        ...
        if (!providedAuth) {
            ...
            throw new error_1.WebhookAuthorizationError(401, 'Authentication data is wrong!');
        }
    }
        ...
        if (expectedAuth.keyType === 'passphrase') {
            ...
            secretOrPublicKey = (0, format_pem_block_1.formatPemBlock)(expectedAuth.publicKey, true);
        }
        try {
