...
const completionProperties = (0, n8n_workflow_1.updateDisplayOptions)({
    ...
    },
    {
        ...
        description: 'Find the name of input field containing the binary data to return in the Input panel on the left, in the Binary tab. You can provide multiple comma-separated field names.',
...
class Form extends n8n_workflow_1.Node {
    ...
    async webhook(context) {
        ...
        const triggerAuth = context.evaluateExpression(`{{ ${triggerRef}.params.authentication }}`) ?? 'none';
        const authResult = await (0, utils_1.validateFormPageAuth)(context, triggerAuth);
        if (authResult.responded) {
            return { noWebhookResponse: true };
        }
        const triggerIncludeUser = context.evaluateExpression(`{{ ${triggerRef}.params.options?.includeUserInOutput }}`);
        const userForOutput = triggerIncludeUser === false ? undefined : authResult.authedUser;
        const mode = context.evaluateExpression(`{{ ${triggerRef}.first().json.formMode }}`);
        ...
        if (operation === 'completion' && method === 'GET') {
            return await (0, formCompletionUtils_1.renderFormCompletion)(context, res, trigger, authResult.authedUser);
        }
        ...
        if (method === 'GET') {
            return await (0, formNodeUtils_1.renderFormNode)(context, res, trigger, fields, mode, authResult.authedUser);
        }
        let useWorkflowTimezone = context.evaluateExpression(`{{ ${triggerRef}.params.options?.useWorkflowTimezone }}`);
        if (useWorkflowTimezone === undefined && trigger?.typeVersion > 2) {
            useWorkflowTimezone = true;
        }
        const returnItem = await (0, utils_1.prepareFormReturnItem)(context, fields, mode, useWorkflowTimezone, userForOutput);
        return {
