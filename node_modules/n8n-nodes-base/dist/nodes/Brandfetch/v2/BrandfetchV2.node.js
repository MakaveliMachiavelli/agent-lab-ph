            ...
            properties: [
                {
                    ...
                    options: [
                        ...
                        },
                        {
                            ...
                            value: 'data',
                            description: 'Get all available Brandfetch attributes, including name, description, logos, colors, infographics and other data',
                            action: "Return all of a brand's data",
                        },
                        {
                            name: 'Return Brand Context',
                            value: 'context',
                            description: 'Get a structured, narrative-rich brand profile (identity, positioning, voice and visual style) for a domain, suitable for grounding LLMs and generating on-brand content',
                ...
                },
                {
                    ...
                    type: 'string',
                    default: '',
                    description: 'The identifier of the brand. Format depends on the selected Type (e.g. apple.com, AAPL, BTC, US0378331005).',
                    required: true,
                    displayOptions: {
                        show: {
                    ...
                    options: [
                        {
                            name: 'JSON',
                            value: 'json',
                            description: 'Structured JSON object with identity, positioning, and brand voice and style',
                        },
                        {
                            name: 'Markdown',
                            value: 'markdown',
                ...
                },
                {
                    ...
                    type: 'boolean',
                    default: false,
                    description: 'Whether to return the brand context only if one is already cached, responding instantly without crawling the domain. Returns an empty item if no cached context exists.',
                    displayOptions: {
                        show: {
                            operation: ['context'],
                        },
        ...
        for (let i = 0; i < length; i++) {
            try {
                if (operation === 'context') {
                    const domain = this.getNodeParameter('domain', i).trim();
                    const outputFormat = this.getNodeParameter('outputFormat', i);
                    const cachedOnly = this.getNodeParameter('cachedOnly', i);
                    const qs = {};
                    if (cachedOnly) {
                        qs.cachedOnly = true;
                    }
                    const requestOptions = outputFormat === 'markdown'
                        ? { json: false, headers: { accept: 'text/markdown' } }
                        : { headers: { accept: 'application/json' } };
                    const response = await GenericFunctions_1.brandfetchApiRequest.call(this, 'GET', `/context/${encodeURIComponent(domain)}`, {}, qs, undefined, requestOptions);
                    let contextJson;
                    ...
                    const executionData = this.helpers.constructExecutionMetaData(this.helpers.returnJsonArray(contextJson), { itemData: { item: i } });
                    responseData.push.apply(responseData, executionData);
                    continue;
                }
                const type = this.getNodeParameter('type', i);
                const identifier = this.getNodeParameter('identifier', i).trim();
                const resourcePath = `/brands/${encodeURIComponent(type)}/${encodeURIComponent(identifier)}`;
                const response = await GenericFunctions_1.brandfetchApiRequest.call(this, 'GET', resourcePath);
                if (operation === 'data') {
                    ...
                    if (download) {
                        ...
                        for (const logoUrl of logoUrls) {
                            const imageType = logoUrl.type;
                            if (!imageType)
                                continue;
                            const formats = logoUrl.formats ?? [];
                            for (const logoFormat of formats) {
                                const format = logoFormat.format;
                                if (!format || !logoFormat.src)
                                    continue;
                                const suffix = matchCounter === 0 ? '' : `_${matchCounter}`;
                                await GenericFunctions_1.fetchAndPrepareBinaryData.call(this, imageType, format, logoFormat, identifier, newItem, suffix);
                                matchCounter++;
                            }
                        }
...
exports.BrandfetchV2 = BrandfetchV2;
//# sourceMappingURL=BrandfetchV2.node.js.map