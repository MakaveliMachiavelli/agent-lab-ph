...
function assertSafeLogoUrl(rawUrl) {
    if (typeof rawUrl !== 'string' || rawUrl.length === 0) {
        throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'Logo source URL is missing or invalid');
    }
    let parsed;
    try {
        ...
        throw new n8n_workflow_1.NodeOperationError(this.getNode(), `Logo source URL is not a valid URL: ${rawUrl}`);
    }
    if (parsed.protocol !== 'https:') {
        throw new n8n_workflow_1.NodeOperationError(this.getNode(), `Logo source URL must use https (got ${parsed.protocol})`);
    }
    const hostname = parsed.hostname.toLowerCase();
    const isAllowedHost = ALLOWED_LOGO_HOSTNAME_SUFFIXES.some((suffix) => hostname === suffix.slice(1) || hostname.endsWith(suffix));
    if (!isAllowedHost) {
        throw new n8n_workflow_1.NodeOperationError(this.getNode(), `Refusing to download logo from untrusted host: ${hostname}`);
    }
...
async function fetchAndPrepareBinaryData(imageType, imageFormat, logoFormat, identifier, newItem, binaryKeySuffix = '') {
