...
exports.toCronSource = exports.toCronExpression = void 0;
exports.withIntervalDefaults = withIntervalDefaults;
...
exports.resetStaleRecurrence = resetStaleRecurrence;
...
const isScheduleField = (value) => SCHEDULE_FIELDS.some((field) => field === value);
function withIntervalDefaults(interval) {
    ...
    switch (field) {
        case 'cronExpression':
            return { field, expression: interval.expression ?? '' };
        case 'seconds':
 ...
 */
const stableInt = (seed, label, min, max) => {
    const hash = (0, crypto_1.createHash)('sha256').update(`${seed}:${label}`).digest();
    return min + (hash.readUInt32BE(0) % (max - min));
};
const toCronExpression = (interval, nodeKey) => {
    ...
    if (12 % months === 0)
        return `${second} ${minute} ${hour} ${dayOfMonth} */${months} *`;
...
exports.toCronSource = toCronSource;
 ...
 */
function isRecurrenceValueValidForType(typeInterval, value) {
    switch (typeInterval) {
        case 'minutes':
 ...
 */
function resetStaleRecurrence(staticData, rules) {
    rules.forEach(({ recurrence }, index) => {
        ...
        const keepValue = storedSignature === undefined &&
