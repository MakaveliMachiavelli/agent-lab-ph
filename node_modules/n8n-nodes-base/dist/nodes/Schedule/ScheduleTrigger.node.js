...
class ScheduleTrigger {
    ...
    async trigger() {
        ...
        const { interval = [{}] } = this.getNodeParameter('rule', {});
        const intervals = interval.map(GenericFunctions_1.withIntervalDefaults);
        ...
        const executeTrigger = (recurrence, skipRecurrenceCheck = false, scheduledTime) => {
            ...
            const deduplicationKey = workflowId && scheduledTime
                ? `${workflowId}:${nodeId}:${scheduledTime.toISOString()}`
                : undefined;
            this.emit([this.helpers.returnJsonArray([resultData])], 
            /* responsePromise= */ undefined, 
            /* donePromise= */ undefined, deduplicationKey);
        };
        const nodeKey = `${workflowId ?? ''}:${nodeId}`;
        const rules = intervals.map((interval, i) => ({
            interval,
            cronExpression: (0, GenericFunctions_1.toCronExpression)(interval, nodeKey),
        ...
        if (this.getMode() !== 'manual') {
            ...
            (0, GenericFunctions_1.resetStaleRecurrence)(staticData, rules);
            for (const { interval, cronExpression, recurrence } of rules) {
                try {
                    const cron = {
                        expression: cronExpression,
                        recurrence,
                        source: (0, GenericFunctions_1.toCronSource)(interval),
                    };
                    this.helpers.registerCron(cron, (scheduledTime) => executeTrigger(recurrence, /* skipRecurrenceCheck= */ false, scheduledTime));
                }
                catch (error) {
