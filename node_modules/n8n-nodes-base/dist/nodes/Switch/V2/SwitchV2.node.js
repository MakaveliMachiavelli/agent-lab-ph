...
function matchesRegex(value1, value2) {
    const { source, flags } = (0, n8n_workflow_1.parseRegexLiteral)((value2 || '').toString());
    return n8n_workflow_1.safeRegex.test(source, (value1 || '').toString(), flags);
}
class SwitchV2 {
    description;
						...
						if (mode === 'expression') {
							return Array
								.from(
									...
									(_, i) => ({ type: "${n8n_workflow_1.NodeConnectionTypes.Main}", displayName: i.toString() })
								)
						}


						return rules.map(value => {
							return { type: "${n8n_workflow_1.NodeConnectionTypes.Main}", displayName: value.outputKey }
						})
					})($parameter)
				}}`,
            properties: [
