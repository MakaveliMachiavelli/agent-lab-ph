    ...
    .onCase(2, archive));
            ...
            properties: [
                ...
                },
                {
                    ...
                    builderHint: {
                        propertyHint: "Use `rules.values` (NOT `rules.rules`). Each rule needs `outputKey` and a complete `conditions` object with these three sibling keys: `combinator` ('and' | 'or'), `conditions` (array of condition objects), `options` (`{ caseSensitive, leftValue, typeValidation }`). Same shape as IF. Wire rule outputs by zero-based index with `.onCase(index, target)`; `outputKey` is the visible output label, not the `.onCase()` argument. Unwired cases silently drop their items. Default `caseSensitive: false` for string equality, only set `true` when specifically requested or confirmed by the user.",
                    },
                    typeOptions: {
                        multipleValues: true,
                        sortable: true,
                    ...
                    options: [
                        {
                            ...
                            builderHint: {
                                propertyHint: "Set this to `'extra'` before wiring a catch-all/default branch. In rules mode, `'extra'` creates a fallback output at index `rules.values.length`; default `'none'` creates no fallback output and drops unmatched items. Numeric values route unmatched items to an existing rule output and do not create a new port.",
                            },
                            default: 'none',
                        ...
                        },
                        {
                            ...
                            default: '',
                            builderHint: {
                                propertyHint: "Only labels the extra fallback output. Use it together with `fallbackOutput: 'extra'`; it does not create a fallback output by itself.",
                            },
                            displayOptions: {
                                show: {
                                    fallbackOutput: ['extra'],
