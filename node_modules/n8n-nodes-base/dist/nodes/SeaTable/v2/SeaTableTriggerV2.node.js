    ...
    async poll() {
        ...
        if (event === 'newAsset') {
            ...
            sqlResult = await GenericFunctions_1.seaTableApiRequest.call(this, ctx, 'POST', endpoint, {
                sql: `SELECT _id, _ctime, _mtime, \`${(0, GenericFunctions_1.escapeSqlIdentifier)(assetColumn)}\` FROM \`${(0, GenericFunctions_1.escapeSqlIdentifier)(tableName)}\` WHERE \`${(0, GenericFunctions_1.escapeSqlIdentifier)(assetColumn)}\` IS NOT NULL ORDER BY _mtime DESC LIMIT ${limit}`,
        ...
        // View => use getRows.
            ...
            const endpoint = '/api-gateway/api/v2/dtables/{{dtable_uuid}}/sql';
            const sqlQuery = `SELECT * FROM \`${(0, GenericFunctions_1.escapeSqlIdentifier)(tableName)}\` WHERE ${filterField} BETWEEN "${(0, moment_timezone_1.default)(startDate).format('YYYY-MM-D HH:mm:ss')}" AND "${(0, moment_timezone_1.default)(endDate).format('YYYY-MM-D HH:mm:ss')}" ORDER BY ${filterField} DESC LIMIT ${limit}`;
            sqlResult = await GenericFunctions_1.seaTableApiRequest.call(this, ctx, 'POST', endpoint, {
                sql: sqlQuery,
                convert_keys: options.convert ?? true,
            });
