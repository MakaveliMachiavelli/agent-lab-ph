...
async function execute(index) {
    ...
    let sqlQuery = `SELECT * FROM \`${(0, GenericFunctions_1.escapeSqlIdentifier)(tableName)}\` WHERE \`${(0, GenericFunctions_1.escapeSqlIdentifier)(searchColumn)}\``;
    if (options.insensitive) {
        searchTermString = searchTermString.toLowerCase();
        sqlQuery = `SELECT * FROM \`${(0, GenericFunctions_1.escapeSqlIdentifier)(tableName)}\` WHERE lower(\`${(0, GenericFunctions_1.escapeSqlIdentifier)(searchColumn)}\`)`;
    }
    const wildcard = options.wildcard ?? true;
    const escapedSearchTerm = (0, GenericFunctions_1.escapeSqlString)(searchTermString);
    if (wildcard)
        sqlQuery = sqlQuery + ' LIKE "%' + escapedSearchTerm + '%"';
    else if (!wildcard)
        sqlQuery = sqlQuery + ' = "' + escapedSearchTerm + '"';
