...
exports.commonProperties = exports.messageProperty = exports.sharedVersionDescription = void 0;
exports.getStructuredOutputSchema = getStructuredOutputSchema;
exports.execute = execute;
const n8n_workflow_1 = require("n8n-workflow");
const node_crypto_1 = __importDefault(require("node:crypto"));
const generateSchemaFromExample_1 = require("./generateSchemaFromExample");
/** Description fields that are identical across versions. */
exports.sharedVersionDescription = {
    hidden: true,
    defaults: {
...
function parseManualOutputSchema(ctx, itemIndex, rawSchema) {
    ...
    if (typeof rawSchema === 'object') {
        ...
        if (!rawSchema.trim()) {
            throw new n8n_workflow_1.NodeOperationError(ctx.getNode(), 'Output schema is empty. Provide a JSON Schema or turn off "Require Specific Output Format".', { itemIndex });
        }
        try {
            parsed = (0, n8n_workflow_1.jsonParse)(rawSchema);
        }
        catch (error) {
            throw new n8n_workflow_1.NodeOperationError(ctx.getNode(), `Output schema is not valid JSON: ${error.message}`, { itemIndex });
        }
    }
    else {
        throw new n8n_workflow_1.NodeOperationError(ctx.getNode(), 'Output schema is not valid JSON. Provide a JSON Schema or turn off "Require Specific Output Format".', { itemIndex });
    }
    if (typeof parsed !== 'object' || parsed === null || Array.isArray(parsed)) {
        throw new n8n_workflow_1.NodeOperationError(ctx.getNode(), 'Output schema must be a JSON Schema object', {
            itemIndex,
        });
    }
    return parsed;
 ...
 */
function getStructuredOutputSchema(ctx, itemIndex) {
    ...
    if (typeVersion >= 3) {
        ...
        if (schemaType === 'fromJson') {
            ...
            if (!exampleString.trim()) {
                throw new n8n_workflow_1.NodeOperationError(ctx.getNode(), 'JSON example is empty. Provide an example object or turn off "Require Specific Output Format".', { itemIndex });
            }
            let schema;
            try {
                schema = (0, generateSchemaFromExample_1.generateSchemaFromExample)(exampleString, true);
            }
            catch (error) {
                throw new n8n_workflow_1.NodeOperationError(ctx.getNode(), `JSON example is not valid JSON: ${error.message}`, { itemIndex });
            }
            if (schema.type !== 'object') {
                throw new n8n_workflow_1.NodeOperationError(ctx.getNode(), 'JSON example must be a JSON object (e.g. { "result": "value" }), not an array or single value', { itemIndex });
            }
            return schema;
        }
    }
...
function getAgentSource(ctx, itemIndex) {
    ...
    if (agentSource === 'inline') {
        ...
        const value = typeof raw === 'string'
            ? (0, n8n_workflow_1.jsonParse)(raw, {
        ...
        if (typeof value !== 'object' ||
            ...
            throw new n8n_workflow_1.NodeOperationError(ctx.getNode(), 'Inline agent is not configured. Open the node to set up the agent, or switch to a saved agent.', { itemIndex });
        }
...
async function execute() {
    ...
    const executionId = this.getExecutionId() ?? node_crypto_1.default.randomUUID();
    ...
    for (let i = 0; i < loopCount; i++) {
        try {
            ...
            if (sessionIdOverride && sessionIdOverride.length > SESSION_ID_MAX_LENGTH) {
                throw new n8n_workflow_1.NodeOperationError(this.getNode(), `Session ID must be at most ${SESSION_ID_MAX_LENGTH} characters (got ${sessionIdOverride.length})`, { itemIndex: i });
            }
            if (!prompt.trim()) {
