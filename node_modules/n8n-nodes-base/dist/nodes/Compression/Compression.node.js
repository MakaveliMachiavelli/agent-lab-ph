...
const BoundedGunzip_1 = require("./decompress/BoundedGunzip");
const BoundedUntar_1 = require("./decompress/BoundedUntar");
const BoundedUnzip_1 = require("./decompress/BoundedUnzip");
 ...
 */
async function createTar(files, gzipOutput) {
    ...
    for (const { fileName, data } of files) {
        const entry = new ReadEntry(new Header({ path: fileName, size: data.length, mode: 0o644, type: 'File' }));
        entry.end(data);
        pack.write(entry);
    }
    pack.end();
    return await (0, consumers_1.buffer)(pack);
}
...
class Compression {
    ...
    async execute() {
        ...
        const { maxDecompressedSize, maxZipEntries } = di_1.Container.get(config_1.CompressionNodeConfig);
            ...
            try {
                if (operation === 'decompress') {
                    ...
                    for (const [index, binaryPropertyName] of binaryPropertyNames.entries()) {
                        ...
                        const isTar = fileExtension === 'tar' ||
                            fileExtension === 'tgz' ||
                            (binaryData.fileName?.toLowerCase().endsWith('.tar.gz') ?? false);
                        if (fileExtension === 'zip' || isTar) {
                            const files = isTar
                                ? await (0, BoundedUntar_1.boundedUntar)(binaryDataBuffer, maxDecompressedSize, maxZipEntries)
                                : await (0, BoundedUnzip_1.boundedUnzip)(binaryDataBuffer, maxDecompressedSize, maxZipEntries);
                            ...
                            const file = await (0, BoundedGunzip_1.boundedGunzip)(binaryDataBuffer, maxDecompressedSize);
                            ...
                            binaryObject[propertyName] = await this.helpers.prepareBinaryData(file, fileName, mimeType);
                            if (!fileExtension) {
                            ...
                            throw new n8n_workflow_1.NodeOperationError(this.getNode(), `Unsupported archive format ".${fileExtension}" for binary data ${binaryPropertyName}`, {
                                description: 'The Decompress operation supports the following formats: zip, gzip, tar, tar.gz and tgz',
                            });
                        }
                    }
                    returnData.push({
                ...
                if (operation === 'compress') {
                    ...
                    if (outputFormat === 'tar' || outputFormat === 'targz') {
                        ...
                        const fileName = this.getNodeParameter('fileName', tarOptionsIndex);
                        const binaryPropertyOutput = this.getNodeParameter('binaryPropertyOutput', tarOptionsIndex);
