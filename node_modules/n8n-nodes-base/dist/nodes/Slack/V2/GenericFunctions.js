...
exports.toMultiOptionsCsv = toMultiOptionsCsv;
exports.throwOnSlackApiError = throwOnSlackApiError;
...
exports.searchContextItems = searchContextItems;
...
function toMultiOptionsCsv(value) {
...
responseData) {
    if (responseData.error === 'paid_teams_only') {
        ...
        throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'This Slack operation requires a user token', {
 ...
 */
async function searchContextItems(body, maxResults) {
    const returnData = [];
    let cursor;
    do {
        ...
        const messages = (0, get_1.default)(responseData, 'results.messages') ?? [];
        returnData.push(...messages);
        cursor = (0, get_1.default)(responseData, 'response_metadata.next_cursor');
...
function createSendAndWaitMessageBody(context) {
    ...
    const hmacSecret = captureResponder ? di_1.Container.get(n8n_core_1.InstanceSettings).hmacSignatureSecret : '';
    const body = {
        channel: target,
        blocks: [
            {
            ...
            },
            {
                ...
                elements: config.options.map((option) => ({
                    ...
                    ...(captureResponder
                        ? {
                            action_id: option.approved ? MessageInterface_1.HITL_APPROVE_ACTION_ID : MessageInterface_1.HITL_DECLINE_ACTION_ID,
                            value: (0, n8n_core_1.buildHitlCallbackReference)(executionId, option.approved ? 'a' : 'd', hmacSecret),
                        }
                        : { url: option.url }),
                })),
            },
