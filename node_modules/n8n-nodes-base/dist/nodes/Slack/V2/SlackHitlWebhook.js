...
exports.slackSendAndWaitWebhook = slackSendAndWaitWebhook;
...
async function extractSlackResponder(context, payload) {
    ...
    if (user.id) {
        try {
            const info = (await GenericFunctions_1.slackApiRequest.call(context, 'GET', '/users.info', {}, { user: user.id }));
...
async function notifyNotAuthorized(context, payload, text) {
    ...
    if (!channel || !user)
        return;
    try {
        await GenericFunctions_1.slackApiRequest.call(context, 'POST', '/chat.postEphemeral', {
            channel,
            user,
            text,
        });
...
async function lockSlackMessage(context, payload, blocks, text) {
    ...
    if (channel && ts) {
        try {
            await GenericFunctions_1.slackApiRequest.call(context, 'POST', '/chat.update', {
                channel,
                ts,
                text,
                blocks,
 ...
 */
async function slackSendAndWaitWebhook() {
    ...
    if (!(0, n8n_core_1.isSlackInteractionRequest)(req)) {
        return await utils_1.sendAndWaitWebhook.call(this);
    }
    ...
    if (!signingSecret || !(await SlackTriggerHelpers_1.verifySignature.call(this, credentialType))) {
    ...
    const payload = typeof body.payload === 'string'
        ? (0, n8n_workflow_1.jsonParse)(body.payload, { fallbackValue: {} })
        : body;
    ...
    const parsed = (0, n8n_core_1.parseHitlCallbackReference)(action?.value ?? '');
    const approved = parsed?.decision === 'a' && action?.action_id === MessageInterface_1.HITL_APPROVE_ACTION_ID;
