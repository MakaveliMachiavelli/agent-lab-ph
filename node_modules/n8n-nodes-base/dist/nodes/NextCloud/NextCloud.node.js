            ...
            try {
                ...
                if (['file', 'folder'].includes(resource)) {
                    if (operation === 'copy') {
                        ...
                        //         share
                        // ----------------------------------
                        const shareType = this.getNodeParameter('shareType', i);
                        const sharePath = this.getNodeParameter('path', i);
                        if (shareType === 200) {
...
<d:propfind xmlns:d="DAV:" xmlns:oc="http://owncloud.org/ns">
  <d:prop><oc:fileid/></d:prop>
                ...
                try {
                    responseData = await GenericFunctions_1.nextCloudApiRequest.call(this, requestMethod, endpoint, body, headers, encoding, qs, useWebDavEndpoint);
                }
                catch (error) {
                ...
                if (resource === 'file' && operation === 'download') {
                    ...
                    if (shareType === 200) {
                        ...
                        if (typeof responseData !== 'string') {
                            throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'Could not retrieve internal link: unexpected response type from NextCloud', { itemIndex: i });
                        }
                        const propfindData = await new Promise((resolve, reject) => {
                            (0, xml2js_1.parseString)(responseData, {
                                explicitArray: false,
                                ...
                                if (!data || typeof data !== 'object') {
                                    return reject(new n8n_workflow_1.NodeOperationError(this.getNode(), 'Could not retrieve internal link: invalid XML response structure', { itemIndex: i }));
                                }
                                resolve(data);
                            });
                        });
                        const multistatus = propfindData['d:multistatus'];
                        if (!multistatus) {
                            throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'Could not retrieve internal link: malformed PROPFIND response', { itemIndex: i });
                        }
                        ...
                        if (typeof fileid !== 'string' || fileid.length === 0) {
                            throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'Could not retrieve internal link: oc:fileid not found in PROPFIND response', { itemIndex: i });
                        }
                        const webDavBase = credentials.webDavUrl.replace(/\/remote\.php\/webdav\/?$/, '');
                        if (webDavBase === credentials.webDavUrl) {
                            throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'WebDAV URL must end with /remote.php/webdav for generating an internal link. Please check your Nextcloud credentials.', { itemIndex: i });
                        }
                        const internalLink = `${webDavBase}/f/${fileid}`;
                        const executionData = this.helpers.constructExecutionMetaData((0, utilities_1.wrapData)({ link: internalLink }), { itemData: { item: i } });
                        returnData.push(...executionData);
                    }
                    else {
                        if (typeof responseData !== 'string') {
                            throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'Unexpected response type from NextCloud OCS share endpoint', { itemIndex: i });
                        }
                        const jsonResponseData = await new Promise((resolve, reject) => {
                            (0, xml2js_1.parseString)(responseData, {
                                ...
                                if (!data?.ocs?.data || typeof data.ocs.data !== 'object') {
                                    return reject(new n8n_workflow_1.NodeApiError(this.getNode(), { error: 'Invalid OCS response structure' }));
                                }
