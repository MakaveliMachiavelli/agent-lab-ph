...
exports.DEFAULT_PARTITIONS_CONSUMED_CONCURRENTLY = exports.DEFAULT_BATCH_SIZE = void 0;
exports.consumeTopic = consumeTopic;
 ...
 */
exports.DEFAULT_PARTITIONS_CONSUMED_CONCURRENTLY = 1;
// ---------------------------------------------------------------------------
// Entry point
// ---------------------------------------------------------------------------
/**
 ...
 */
async function consumeTopic(consumer, options) {
    ...
    const pauseBeforeRetry = async () => {
        if (signal.aborted)
            return;
        await Promise.race([(0, sleep_1.sleep)(settings.errorRetryDelay), closed]);
    };
    const context = {
        ...settings,
        parseMessage,
    ...
    return {
        close: async () => {
            ...
            await (0, utils_1.withTimeout)(consumer.disconnect(), CLOSE_TIMEOUT_MS, 'Kafka consumer did not disconnect in time');
        },
    };
}
// ---------------------------------------------------------------------------
 ...
 */
async function processBatch({ batch, resolveOffset, commitOffsetsIfNecessary, isRunning, isStale }, context) {
    ...
    for (let i = 0; i < messages.length; i += batchSize) {
        ...
        let items;
        try {
            items = await Promise.all(chunk.map(async (message) => await parseMessage(message, batch.topic)));
        }
        catch (error) {
...
function resolveSettings(options) {
    ...
    return {
        batchSize: positiveCount(options.batchSize, exports.DEFAULT_BATCH_SIZE, 'Batch Size', logger),
        partitionsConsumedConcurrently: positiveCount(options.partitionsConsumedConcurrently, exports.DEFAULT_PARTITIONS_CONSUMED_CONCURRENTLY, 'Parallel Processing', logger),
        errorRetryDelay: (0, utils_1.resolveRetryDelay)(options.errorRetryDelay, logger),
    };
}
/**
 ...
 */
function positiveCount(value, fallback, name, logger) {
    if (value === undefined)
        return fallback;
...
//# sourceMappingURL=ConsumeTopic.js.map