...
exports.manualRunGroupId = manualRunGroupId;
exports.explainManualRunGroupDenial = explainManualRunGroupDenial;
exports.toConsumerOptions = toConsumerOptions;
exports.toEmitterOptions = toEmitterOptions;
 ...
 */
function manualRunGroupId(configured, isManualRun) {
    return isManualRun ? `${MANUAL_RUN_PREFIX(configured)}${(0, node_crypto_1.randomUUID)()}` : configured;
}
 ...
 * @param isManualRun - Whether this is an editor test run
 */
function explainManualRunGroupDenial(error, configuredGroupId, isManualRun) {
    if (!isManualRun || !GROUP_AUTHORIZATION_FAILED.test(error.message))
        return error;
    return new n8n_workflow_1.UserError('Kafka refused the consumer group used for a test run', {
 ...
 */
function toConsumerOptions(options, groupId, executionTimeoutSeconds, logger) {
    ...
    if (fromWorkflow) {
        ...
        deadlineMs = consumer_1.DEFAULT_EXECUTION_TIMEOUT_SECONDS * 1000;
    }
 ...
 * @param executionTimeoutSeconds - `getWorkflowSettings().executionTimeout`, raw
 */
function toEmitterOptions(options, resolveOffsetMode, allowedStatuses, executionTimeoutSeconds) {
    return {
        resolveOffsetMode,
        allowedStatuses,
