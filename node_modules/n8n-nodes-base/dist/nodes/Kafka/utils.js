...
exports.MAX_TIMER_DELAY_MS = exports.DEFAULT_ERROR_RETRY_DELAY_MS = void 0;
exports.resolveRetryDelay = resolveRetryDelay;
exports.formatAndValidatePem = formatAndValidatePem;
exports.resolveKafkaSsl = resolveKafkaSsl;
exports.createConfig = createConfig;
exports.createConsumerConfig = createConsumerConfig;
exports.configureMessageParser = configureMessageParser;
exports.toUserFacingConsumerError = toUserFacingConsumerError;
exports.connectEventListeners = connectEventListeners;
exports.disconnectEventListeners = disconnectEventListeners;
exports.sanitizeRegistryError = sanitizeRegistryError;
exports.getSchemaRegistryOptions = getSchemaRegistryOptions;
...
exports.stopAndDisconnectConsumer = stopAndDisconnectConsumer;
exports.runWithHeartbeat = runWithHeartbeat;
const confluent_schema_registry_1 = require("@kafkajs/confluent-schema-registry");
const format_pem_block_1 = require("@n8n/utils/format-pem-block");
...
exports.DEFAULT_ERROR_RETRY_DELAY_MS = 5000;
/**
 ...
 */
exports.MAX_TIMER_DELAY_MS = 2_147_483_647;
/** True for a delay `setTimeout` would honour as written. Zero is fine. */
function isUsableDelay(value) {
    return Number.isFinite(value) && value >= 0 && value <= exports.MAX_TIMER_DELAY_MS;
}
/**
 ...
 */
function resolveRetryDelay(value, logger) {
    if (value === undefined)
        return exports.DEFAULT_ERROR_RETRY_DELAY_MS;
    if (typeof value !== 'number' || !isUsableDelay(value)) {
        logger?.warn(`Kafka "Retry Delay on Error" of ${String(value)} cannot be used as a delay, falling back to ${exports.DEFAULT_ERROR_RETRY_DELAY_MS}ms`);
        return exports.DEFAULT_ERROR_RETRY_DELAY_MS;
    }
    return value;
}
 ...
 */
function formatAndValidatePem(value, fieldName) {
    const formatted = (0, format_pem_block_1.formatPemBlock)(value);
    ...
    if (!/-----BEGIN ([A-Z0-9 ]+)-----[\s\S]+-----END \1-----/.test(formatted)) {
        throw new n8n_workflow_1.UserError(`The Kafka ${fieldName} is not a valid PEM block`, {
            level: 'warning',
            description: 'Paste the full PEM, including the "-----BEGIN ...-----" and "-----END ...-----" lines.',
        });
    }
    return formatted;
}
/** kafkajs's `tls.ConnectionOptions` fields take Buffers, the v2 library takes strings. */
function toPemBuffer(value, fieldName) {
 ...
 */
function resolveKafkaSsl(credentials) {
    if (!credentials.ssl)
        return false;
    ...
    if (!cert && !ca && !allowUnauthorized)
        return true;
    const sslOptions = {};
    if (ca)
        sslOptions.ca = [toPemBuffer(ca, 'CA certificate')];
    if (cert)
        sslOptions.cert = toPemBuffer(cert, 'client certificate');
    if (key)
        sslOptions.key = toPemBuffer(key, 'client private key');
    ...
    return async (message, messageTopic) => {
        ...
        if (registry) {
            try {
                ...
                logger.warn('Could not decode message with Schema Registry, returning original message', sanitizeRegistryError(error));
            }
        }
 ...
 */
function toUserFacingConsumerError(node, error) {
    if (/compression not implemented/i.test(error.message)) {
        return new n8n_workflow_1.NodeOperationError(node, 'Kafka topic uses an unsupported compression codec', {
            description: 'This topic contains messages compressed with LZ4, Snappy, or ZSTD, which the Kafka Trigger cannot decode (only GZIP and uncompressed messages are supported). Set the producer to use gzip or no compression to consume this topic.',
        });
    }
    return error;
}
 ...
 */
function connectEventListeners(consumer, logger, onFatalCrash) {
    ...
    const onCrash = consumer.on(consumer.events.CRASH, (event) => {
        ...
        if (!restart) {
            onFatalCrash?.((0, ensure_error_1.ensureError)(error));
        }
    });
    return [
        onConnected,
...
function sanitizeRegistryError(error) {
    const ensured = (0, ensure_error_1.ensureError)(error);
    ...
    const scrubbed = (0, scrub_secrets_1.scrubSecretsInText)(withoutUserinfo);
    // Length capped last, so the cap applies to the redacted text.
    const message = scrubbed.length > MAX_REGISTRY_ERROR_MESSAGE_LENGTH
 ...
 */
async function getSchemaRegistryOptions(ctx, fallbackUrl) {
    const emptyConfigError = () => new n8n_workflow_1.NodeOperationError(ctx.getNode(), 'Select a Schema Registry credential or enter a Schema Registry URL');
    ...
    const credentials = await ctx.getCredentials('schemaRegistryApi');
    ...
    if (credentials.authentication === 'basicAuth') {
        if (!(credentials.username && credentials.password)) {
            throw new n8n_workflow_1.NodeOperationError(ctx.getNode(), 'Username and password are required for Schema Registry Basic Auth');
        }
 ...
 * @param fallbackUrl - The `schemaRegistryUrl` node parameter, used when no credential is selected
 */
async function createSchemaRegistry(ctx, fallbackUrl) {
    ...
    if (!filter) {
        ...
        return new confluent_schema_registry_1.SchemaRegistry(options);
    }
    ...
    try {
        ...
        throw new n8n_workflow_1.NodeOperationError(ctx.getNode(), 'Verify your Schema Registry configuration', {
    ...
    const agent = parsed.protocol === 'https:'
        ? new node_https_1.default.Agent({ lookup: filter.createSecureLookup() })
        : new node_http_1.default.Agent({ lookup: filter.createSecureLookup() });
    return new confluent_schema_registry_1.SchemaRegistry({ ...options, host: parsed.href, agent });
}
/**
 ...
 */
async function setSchemaRegistry(ctx) {
 ...
 */
function configureDataEmitter(ctx, options, nodeVersion, closeSignal) {
    ...
    const errorRetryDelay = options.errorRetryDelay ?? exports.DEFAULT_ERROR_RETRY_DELAY_MS;
    ...
    const abortPromise = new Promise((_, reject) => {
        closeSignal.addEventListener('abort', () => reject(new n8n_workflow_1.OperationalError('Trigger closed before the execution finished, offsets not resolved.')), { once: true });
    });
    ...
    return async (dataArray) => {
        ...
        try {
            ...
            if (!closeSignal.aborted) {
                await Promise.race([(0, sleep_1.sleep)(errorRetryDelay), abortPromise.catch(() => undefined)]);
            }
    ...
    try {
        return await Promise.race([
            ...
            new Promise((_, reject) => {
                timer = setTimeout(() => reject(new n8n_workflow_1.OperationalError(message)), ms);
            }),
        ]);
    }
    finally {
 ...
 */
async function stopAndDisconnectConsumer(consumer, logger, timeoutMs) {
    let teardownError;
    ...
    try {
        ...
        teardownError = (0, ensure_error_1.ensureError)(error);
    }
    if (!stopSettled) {
        void stopPromise
            .catch(() => undefined)
    ...
    try {
        await withTimeout(consumer.disconnect(), timeoutMs, 'Kafka consumer did not disconnect in time');
    }
    catch (error) {
        teardownError ??= (0, ensure_error_1.ensureError)(error);
    }
