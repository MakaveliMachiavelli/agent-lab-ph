        ...
        if (operation === 'executeQuery') {
            ...
            try {
                const queryQueue = items.map(async (_, index) => {
                    ...
                    const { query, values } = (0, utils_1.prepareSafeQuery)(rawQuery, queryReplacement);
            ...
            try {
                ...
                const insertSQL = `INSERT ${insertPriority || ''} ${insertIgnore ? 'IGNORE' : ''} INTO ${(0, utils_1.escapeSqlIdentifier)(table)}(${columns.map(utils_1.escapeSqlIdentifier).join(',')}) VALUES ${items
            ...
            try {
                ...
                const updateSQL = `UPDATE ${(0, utils_1.escapeSqlIdentifier)(table)} SET ${columns
                    .map((column) => `${(0, utils_1.escapeSqlIdentifier)(column)} = ?`)
                    .join(',')} WHERE ${(0, utils_1.escapeSqlIdentifier)(updateKey)} = ?;`;
