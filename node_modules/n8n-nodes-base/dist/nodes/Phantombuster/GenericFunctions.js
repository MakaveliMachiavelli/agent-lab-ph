...
exports.phantombusterStreamingRequest = phantombusterStreamingRequest;
 ...
 */
async function phantombusterStreamingRequest(options, onMessage) {
    let stream;
    try {
    ...
    try {
        for await (const line of stream) {
            ...
            if (message.type === 'error') {
                const reason = typeof message.data === 'string' ? message.data : JSON.stringify(message.data);
                throw new n8n_workflow_1.NodeApiError(this.getNode(), { message: reason }, { message: reason, description: 'Phantombuster agent reported an error' });
            }
        ...
        if (code && DISCONNECT_CODES.includes(code))
            return null;
        if (error instanceof n8n_workflow_1.NodeApiError)
            throw error;
        throw new n8n_workflow_1.NodeApiError(this.getNode(), error);
 ...
 * Yields line by line
 */
async function streamingRequest(credentialsType, options) {
    const MAX_BUFFER_SIZE = 1_048_576; // 1 Mb
    const node = this.getNode();
    const abortSignal = 'getExecutionCancelSignal' in this ? this.getExecutionCancelSignal() : undefined;
