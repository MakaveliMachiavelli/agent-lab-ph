...
exports.applyLargeNumbersReceive = applyLargeNumbersReceive;
exports.createReceiveHandler = createReceiveHandler;
...
function applyLargeNumbersReceive(e) {
    if (!e.result)
        return;
...
function createReceiveHandler(largeNumbersOutput) {
    return (e) => {
...
async function configurePostgres(credentials, options = {}) {
    ...
    const fallBackHandler = async (abortController) => {
        ...
        if (typeof options.nodeVersion === 'number' && options.nodeVersion >= 2.7) {
            ...
            dbConfig.types = (0, postgres_1.getDateAsStringTypeParsers)(pgp);
        }
            ...
            [pgp.pg.types.builtins.TIMESTAMP, pgp.pg.types.builtins.TIMESTAMPTZ].forEach((type) => {
                pgp.pg.types.setTypeParser(type, postgres_1.parseDateToISO);
            });
        }
        if (!credentials.sshTunnel) {
            ...
            if (credentials.sshAuthenticateWith === 'privateKey' && credentials.privateKey) {
                credentials.privateKey = (0, format_pem_block_1.formatPemBlock)(credentials.privateKey);
            }
