...
function mapProperties(properties, timezone, version = 1) {
    return properties
        ...
        return [name, getPropertyKeyValue.call(this, property, type, timezone, version)];
    })
        .filter(([, value]) => value)
        .reduce((obj, [key, value]) => {
        (0, n8n_workflow_1.setSafeObjectProperty)(obj, key, value);
        return obj;
    }, {});
}
...
const foldedSnakeCase = (value) => (0, change_case_1.snakeCase)(value.replace(/[^\x20-\x7E]/g, ' '));
function prepend(stringKey, properties, version) {
    // Pre-v3 restores the old ASCII key shape so existing workflows keep resolving.
    const toKey = version >= 3 ? change_case_1.snakeCase : foldedSnakeCase;
    for (const key of Object.keys(properties)) {
...
function simplifyObjects(objects, download = false, version = 2) {
    ...
    for (const { object, id, properties, parent, title, name, json, binary, url } of objects) {
        if (object === 'page' && (parent?.type === 'page_id' || parent?.type === 'workspace')) {
            ...
            results.push({
                id,
                ...(notV1 ? { name: getPropertyTitle(properties) } : {}),
                ...(notV1 ? { url } : {}),
                ...(notV1
                    ? { ...prepend('property', simplifyProperties(properties), version) }
            ...
            results.push({
                json: {
                    id: json.id,
                    ...(notV1 ? { name: getPropertyTitle(json.properties) } : {}),
                    ...(notV1 ? { url: json.url } : {}),
                    ...(notV1
                        ? {
                            ...prepend('property', simplifyProperties(json.properties), version),
                        }
                ...
                if (txt.textType === 'mention' && txt.mentionType === 'database') {
                    if (typeof txt.database === 'object' && txt.database.__rl) {
                        if (txt.database.__regex) {
                            const extracted = n8n_workflow_1.safeRegex.exec(txt.database.__regex, txt.database.value);
