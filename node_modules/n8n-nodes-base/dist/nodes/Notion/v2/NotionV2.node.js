    ...
    async execute() {
                ...
                for (let i = 0; i < itemsLength; i++) {
                    try {
                        ...
                        if (returnAll) {
                            ...
                            responseData = await GenericFunctions_1.notionApiRequestAllItems.call(this, 'results', 'GET', `/blocks/${blockId}/children`, {}, { page_size: Math.min(limit, 100), limit });
                            if (fetchNestedBlocks) {
                                responseData = await GenericFunctions_1.notionApiRequestGetBlockChildrens.call(this, responseData, [], limit);
                            }
                        }
        ...
        if (resource === 'database') {
            if (operation === 'get') {
                ...
                for (let i = 0; i < itemsLength; i++) {
                    try {
                        const databaseId = (0, GenericFunctions_1.extractResourceId)(this.getNodeParameter('databaseId', i, '', { extractValue: true }));
                ...
                for (let i = 0; i < itemsLength; i++) {
                    try {
                        ...
                        if (returnAll) {
                            ...
                            responseData = await GenericFunctions_1.notionApiRequestAllItems.call(this, 'results', 'POST', '/search', body, { limit });
                        }
                        if (simple) {
                ...
                for (let i = 0; i < itemsLength; i++) {
                    try {
                        ...
                        if (returnAll) {
                            ...
                            const limit = this.getNodeParameter('limit', i);
                            body.page_size = Math.min(limit, 100);
                            responseData = await GenericFunctions_1.notionApiRequestAllItems.call(this, 'results', 'POST', `/databases/${databaseId}/query`, body, { limit });
                        }
                        if (download) {
