...
exports.mapDataSourceFilter = mapDataSourceFilter;
exports.mapDataSourceFilters = mapDataSourceFilters;
exports.dataSourceSearchFilterDescriptions = dataSourceSearchFilterDescriptions;
...
const ROLLUP_FILTER_HINT = 'Provide the value for the Notion rollup filter object, for example {"number":{"greater_than":10}} or {"any":{"rich_text":{"contains":"Task"}}}.';
function conditionOptions(type) {
    return (CONDITION_OPTIONS[type] ?? []).map((entry) => ({
        name: (0, change_case_1.capitalCase)(entry),
        value: entry,
    }));
}
function typedConditionOptions(types) {
...
function mapDateValue(value, timezone) {
    if (typeof value !== 'string')
        return '';
    if (!value)
        return '';
    return Number.isNaN(Date.parse(value)) ? value : moment_timezone_1.default.tz(value, timezone).utc().format();
}
function getFilterType(type) {
    return TYPE_TO_FILTER_TYPE[type] ?? type;
}
function assertConditionAllowed(filterType, condition) {
    const allowedConditions = CONDITION_OPTIONS[filterType];
    if (!allowedConditions?.includes(condition)) {
        throw new Error(`The condition "${condition}" is not supported for Notion ${filterType} filters`);
    }
}
function conditionValue(filterType, condition, filter, timezone) {
    if (EMPTY_CONDITIONS.includes(condition))
        return true;
    if (RELATIVE_DATE_CONDITIONS.includes(condition))
        return {};
    switch (filterType) {
        case 'checkbox':
...
function typeCondition(filterType, condition, filter, timezone) {
    assertConditionAllowed(filterType, condition);
    return { [condition]: conditionValue(filterType, condition, filter, timezone) };
}
function mapRollupFilter(filter) {
    if (typeof filter.rollupJson !== 'string' || !filter.rollupJson)
        return {};
    try {
        return JSON.parse(filter.rollupJson);
    }
    catch {
        throw new Error('Rollup Filter (JSON) must be valid JSON');
    }
}
function mapDataSourceFilter(filter, timezone) {
    if (typeof filter.key !== 'string')
        return {};
    const { name, type } = (0, utils_1.splitPropertyKey)(filter.key);
    ...
    if (filterType === 'formula') {
        ...
        return {
            property: name,
            formula: {
                [formulaType]: typeCondition(formulaType === 'string' ? 'rich_text' : formulaType, filter.condition, filter, timezone),
            },
        };
    }
    return {
...
function mapDataSourceFilters(filters, matchType, timezone) {
...
function dataSourceSearchFilterDescriptions() {
    const propertyTypes = [
        'checkbox',
        'created_by',
        'created_time',
    ...
    return [
            ...
            options: [
                {
                    ...
                    values: [
                        ...
                        ...typedConditionOptions(propertyTypes.filter((type) => type !== 'formula' && type !== 'rollup')),
                        {
                            ...
                            options: FORMULA_RETURN_TYPES.map((type) => ({
                                name: (0, change_case_1.capitalCase)(type),
                                value: type,
                            })),
                            default: 'string',
                        },
                        ...FORMULA_RETURN_TYPES.flatMap((returnType) => typedConditionOptions([returnType === 'string' ? 'rich_text' : returnType]).map((condition) => ({
                            ...condition,
                        ...
                        },
                        {
                            ...
                            type: 'string',
                            default: '',
                            description: 'ISO 8601 date or a Notion relative date value like today, tomorrow, yesterday, one_week_ago, or one_month_from_now',
                            displayOptions: {
                                show: {
                        ...
                        },
                        {
                            ...
                            type: 'string',
                            default: '',
                            description: 'Option name. For select, status, and multi-select filters that support multiple values, separate names with commas.',
                            displayOptions: {
        ...
        {
            displayName: 'See <a href="https://developers.notion.com/reference/filter-data-source-entries" target="_blank">Notion guide</a> to creating data source filters',
            name: 'jsonNotice',
            type: 'notice',
            displayOptions: {
...
//# sourceMappingURL=DataSourceFilters.js.map