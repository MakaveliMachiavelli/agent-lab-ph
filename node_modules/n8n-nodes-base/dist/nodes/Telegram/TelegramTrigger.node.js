...
const tokens_1 = require("./hitl/tokens");
const triggerUtils_1 = require("./util/triggerUtils");
/**
...
function hitlForwardUrl(ownWebhookUrl) {
    ...
    const endpoints = di_1.Container.get(config_1.GlobalConfig).endpoints;
    ...
    return `${parsed.origin}${prefix}/${endpoints.webhookWaiting}${n8n_core_1.TELEGRAM_HITL_WEBHOOK_SUFFIX}`;
}
    ...
    async webhook() {
        ...
        if (nodeVersion > 1) {
            ...
            if (secretBuffer.byteLength !== headerSecretBuffer.byteLength ||
                !crypto_1.default.timingSafeEqual(secretBuffer, headerSecretBuffer)) {
        ...
        if (bodyData.callback_query?.data?.startsWith(n8n_core_1.HITL_CALLBACK_PREFIX)) {
            ...
            try {
                ...
                if (ownWebhookUrl) {
                    await this.helpers.httpRequest({
                        ...
                        json: true,
                        headers: {
                            'x-telegram-bot-api-secret-token': (0, tokens_1.deriveHitlSecretToken)(credentials.accessToken),
                        },
        ...
        if (additionalFields.download) {
            const downloadFilesResult = await (0, triggerUtils_1.downloadFile)(this, credentials, bodyData, additionalFields);
