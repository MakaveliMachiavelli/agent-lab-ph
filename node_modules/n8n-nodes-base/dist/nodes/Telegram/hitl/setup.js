...
exports.prepareChatApproval = prepareChatApproval;
 ...
 * unrelated entirely).
 */
function isLikelyTriggerWebhook(url, origin, reverseProxyPrefix) {
    try {
        const parsed = new URL(url);
        if (parsed.origin !== origin)
            return false;
        const liveWebhookSegment = `${reverseProxyPrefix}/${di_1.Container.get(config_1.GlobalConfig).endpoints.webhook}/`;
 ...
 */
function getWaitingWebhookBase(context) {
    ...
    if (!signedUrl.pathname.endsWith(suffix)) {
        ...
        context.logger.warn(`Could not determine the waiting-webhook base path from the signed resume URL (${signedUrl.pathname}); the "Approve Within Chat" fixed endpoint URL may be incorrect.`);
 ...
 */
async function prepareChatApproval(context) {
    // --- applicability: is chat approval actually requested and usable here? ---
    const chatApproval = context.getNodeParameter('chatApproval', 0, false);
    if (!chatApproval)
        return false;
    ...
    if (instanceUrl.protocol !== 'https:' || isLoopbackHost(instanceUrl.hostname)) {
        context.logger.warn('"Approve Within Chat" requires this n8n instance to be reachable over public HTTPS; falling back to link buttons.');
        return false;
    }
    // --- resolve the fixed endpoint we'd need to claim or coexist with ---
    const fixedEndpointUrl = `${origin}${path}${n8n_core_1.TELEGRAM_HITL_WEBHOOK_SUFFIX}`;
    ...
    let allowedUpdates;
    try {
        const webhookInfo = (await GenericFunctions_1.apiRequest.call(context, 'POST', 'getWebhookInfo', {}));
        currentUrl = webhookInfo.result?.url ?? '';
        allowedUpdates = webhookInfo.result?.allowed_updates ?? [];
    }
    catch (error) {
        context.logger.warn(`Could not check the Telegram bot's webhook status; falling back to link buttons: ${error.message}`);
        return false;
    }
    // --- decide: register the webhook, coexist with a Trigger, or refuse ---
    if (currentUrl === '' || currentUrl === fixedEndpointUrl) {
        try {
            await GenericFunctions_1.apiRequest.call(context, 'POST', 'setWebhook', {
                url: fixedEndpointUrl,
                secret_token: (0, tokens_1.deriveHitlSecretToken)(accessToken),
            ...
            context.logger.warn(`Could not register the Telegram bot's webhook; falling back to link buttons: ${error.message}`);
            return false;
        }
        return true;
    }
    ...
    const waitingSegment = `/${di_1.Container.get(config_1.GlobalConfig).endpoints.webhookWaiting}`;
    ...
    if (isLikelyTriggerWebhook(currentUrl, origin, reverseProxyPrefix)) {
        ...
        throw new n8n_workflow_1.NodeOperationError(context.getNode(), 'The Telegram Trigger claiming this bot was activated before one-tap approval support. Re-activate that workflow (or update the trigger to the latest version) so callback buttons reach n8n.');
    }
    throw new n8n_workflow_1.NodeOperationError(context.getNode(), `This bot's webhook is already claimed by another system (URL: ${currentUrl}). Use a different bot for one-tap approval, or turn off "Approve Within Chat".`);
}
