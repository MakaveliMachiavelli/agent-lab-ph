...
async function twitterApiRequest(method, resource, body = {}, qs = {}, fullOutput, uri, option = {}) {
    ...
    try {
        ...
        if (error.error?.required_enrollment === 'Appropriate Level of API Access') {
            throw new n8n_workflow_1.NodeOperationError(this.getNode(), error.error.detail ??
    ...
    if (tweetId.mode === 'id') {
        ...
        try {
            ...
            if (!/(twitter|x).com$/.test(url.hostname)) {
                throw new n8n_workflow_1.UserError('Invalid domain');
            }
            const parts = url.pathname.split('/');
            if (parts.length !== 4 || parts[2] !== 'status' || !/^\d+$/.test(parts[3])) {
                throw new n8n_workflow_1.UserError('Invalid path');
            }
            return parts[3];
        }
        catch (error) {
            throw new n8n_workflow_1.UserError('Not a valid tweet url', { level: 'warning', cause: error });
        }
    }
    else {
        throw new n8n_workflow_1.UserError(`The mode ${tweetId.mode} is not valid!`, { level: 'warning' });
    }
}
async function returnIdFromUsername(usernameRlc) {
    ...
    if (usernameRlc.mode === 'username' ||
        ...
        throw new n8n_workflow_1.UserError(`The username mode ${usernameRlc.mode} is not valid!`, {
            level: 'warning',
        });
}
