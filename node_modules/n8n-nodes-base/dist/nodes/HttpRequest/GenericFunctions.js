...
exports.getAllowedDomains = exports.updadeQueryParameterConfig = exports.setAgentOptions = exports.prepareRequestBody = exports.binaryContentTypes = exports.getOAuth2AdditionalParameters = exports.REDACTED = exports.replaceNullValues = void 0;
...
function redactString(str, secrets) {
    return secrets.reduce((safe, secret) => safe.split(secret).join(exports.REDACTED), str);
}
...
function getSecrets(credentials) {
...
const setAgentOptions = (requestOptions, sslCertificates) => {
    if (sslCertificates) {
        ...
        if (sslCertificates.ca)
            agentOptions.ca = (0, format_pem_block_1.formatPemBlock)(sslCertificates.ca);
        if (sslCertificates.cert)
            agentOptions.cert = (0, format_pem_block_1.formatPemBlock)(sslCertificates.cert);
        if (sslCertificates.key)
            agentOptions.key = (0, format_pem_block_1.formatPemBlock)(sslCertificates.key);
        if (sslCertificates.passphrase)
            agentOptions.passphrase = (0, format_pem_block_1.formatPemBlock)(sslCertificates.passphrase);
        requestOptions.agentOptions = agentOptions;
    }
};
exports.setAgentOptions = setAgentOptions;
const updadeQueryParameterConfig = (version) => {
    if (version < 4.3) {
...
exports.updadeQueryParameterConfig = updadeQueryParameterConfig;
const getAllowedDomains = (node, credentialData) => (0, n8n_workflow_1.getCredentialAllowedDomains)({ node, credentialData, surface: 'HTTP Request or GraphQL' });
