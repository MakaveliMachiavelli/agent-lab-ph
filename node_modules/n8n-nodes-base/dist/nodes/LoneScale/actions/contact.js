...
async function enrich(i) {
    ...
    const responseData = await GenericFunctions_1.lonescaleApiRequest.call(this, 'POST', '/trigger/enrich/sync', body);
    const contacts = responseData.contacts ?? [];
    return this.helpers.constructExecutionMetaData(this.helpers.returnJsonArray(contacts), {
        itemData: { item: i },
    });
}
async function source(i) {
    ...
    if (!companyDomain && !companyName && !companyLinkedinUrl) {
        throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'Provide at least one company identifier: domain, name or Linkedin URL', { itemIndex: i });
    }
    ...
    const responseData = await GenericFunctions_1.lonescaleApiRequest.call(this, 'POST', '/trigger/contact-sourcing/sync', body);
    const contacts = responseData.contacts ?? [];
    return this.helpers.constructExecutionMetaData(this.helpers.returnJsonArray(contacts), {
        itemData: { item: i },
    });
