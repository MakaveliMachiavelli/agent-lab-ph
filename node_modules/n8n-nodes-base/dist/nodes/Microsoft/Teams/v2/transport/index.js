...
exports.SP_HIDE = exports.SERVICE_PRINCIPAL_AUTH = void 0;
exports.getTeamsCredentialType = getTeamsCredentialType;
exports.joinedTeamsEndpoint = joinedTeamsEndpoint;
exports.validateTeamsId = validateTeamsId;
exports.buildTeamsPath = buildTeamsPath;
exports.validateTaskBodyIdsUnderSp = validateTaskBodyIdsUnderSp;
 ...
 */
exports.SP_HIDE = { '/authentication': [exports.SERVICE_PRINCIPAL_AUTH] };
/**
 ...
 */
function getTeamsCredentialType() {
    ...
    return selected === 'microsoftOAuth2Api' || selected === exports.SERVICE_PRINCIPAL_AUTH
        ? selected
 ...
 */
function joinedTeamsEndpoint() {
    return getTeamsCredentialType.call(this) === exports.SERVICE_PRINCIPAL_AUTH
        ? '/v1.0/teams'
        : '/v1.0/me/joinedTeams';
}
 ...
 */
function validateTeamsId(id, node) {
    const value = String(id ?? '').trim();
    if (value === '') {
        throw new n8n_workflow_1.NodeOperationError(node, 'A required ID is empty', {
            description: 'Set the team, channel, plan, bucket or task ID and try again.',
        });
    }
    if (/^\.+$/.test(value)) {
        throw new n8n_workflow_1.NodeOperationError(node, 'The ID is not valid', {
 ...
 */
function buildTeamsPath(segments) {
 ...
 */
function validateTaskBodyIdsUnderSp(ids) {
    if (getTeamsCredentialType.call(this) !== exports.SERVICE_PRINCIPAL_AUTH)
        return;
    const node = this.getNode();
    if (ids.planId !== undefined)
        validateTeamsId(ids.planId, node);
    if (ids.bucketId !== undefined)
        validateTeamsId(ids.bucketId, node);
}
        ...
        if (isServicePrincipal) {
            ...
            try {
                ...
                if (typeof resource === 'string' && resource !== '') {
                    nodeResource = (0, utilities_1.capitalize)(resource);
                }
            }
            catch {
            ...
            throw new n8n_workflow_1.NodeApiError(this.getNode(), sanitizedError, errorOptions);
        }
        const errorOptions = {};
...
async function microsoftApiRequestAllItems(propertyName, method, endpoint, body = {}, query = {}, limit) {
    const returnData = [];
    let responseData;
    let uri;
    do {
        // `@odata.nextLink` already carries the query params; don't re-send them
        responseData = await microsoftApiRequest.call(this, method, endpoint, body, uri ? {} : query, uri);
