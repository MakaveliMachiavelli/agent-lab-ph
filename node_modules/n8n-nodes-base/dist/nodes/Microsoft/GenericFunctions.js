...
exports.stampItemIndexOnError = stampItemIndexOnError;
exports.validateUserTargetId = validateUserTargetId;
 ...
 */
function stampItemIndexOnError(error, itemIndex) {
    if (error instanceof n8n_workflow_1.NodeError && error.context?.itemIndex === undefined) {
 ...
 */
function validateUserTargetId(id, node, messages = DEFAULT_MESSAGES) {
    if (id === '') {
        throw new n8n_workflow_1.NodeOperationError(node, messages.required.message, {
            description: messages.required.description,
        });
    }
    if (/^\.+$/.test(id)) {
        throw new n8n_workflow_1.NodeOperationError(node, messages.dotsOnly.message, {
            description: messages.dotsOnly.description,
        });
    }
    const valid = USER_TARGET_GUID.test(id) || USER_TARGET_UPN.test(id);
    if (!valid) {
        throw new n8n_workflow_1.NodeOperationError(node, messages.invalid.message, {
