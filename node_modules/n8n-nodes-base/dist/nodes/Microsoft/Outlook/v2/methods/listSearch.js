...
async function search(resource, nameProperty, filter, paginationToken) {
    let response = {};
    if (paginationToken) {
        response = await transport_1.microsoftApiRequest.call(this, 'GET', '', 0, undefined, undefined, paginationToken);
    }
    else {
        const qs = {
        ...
        response = await transport_1.microsoftApiRequest.call(this, 'GET', resource, 0, undefined, qs);
    }
    return {
...
async function searchDrafts(filter, paginationToken) {
    ...
    if (paginationToken) {
        ...
        response = await transport_1.microsoftApiRequest.call(this, 'GET', '/messages', 0, undefined, qs);
    }
    return {
        results: response.value.map((entry) => {
            return {
...
async function searchEvents(filter, paginationToken) {
    ...
    if (paginationToken) {
        ...
        response = await transport_1.microsoftApiRequest.call(this, 'GET', `/calendars/${calendarId}/events`, 0, undefined, qs);
    }
    return {
        results: response.value.map((entry) => {
            return {
...
async function searchFolders(filter, paginationToken) {
    ...
    if (paginationToken) {
        ...
        response = await transport_1.microsoftApiRequest.call(this, 'GET', '/mailFolders', 0, undefined, qs);
    }
    let folders = await transport_1.getSubfolders.call(this, response.value, 0, true);
    if (filter) {
...
async function searchAttachments(paginationToken) {
    ...
    if (paginationToken) {
        ...
        response = await transport_1.microsoftApiRequest.call(this, 'GET', `/messages/${messageId}/attachments`, 0, undefined, qs);
    }
    return {
        results: response.value.map((entry) => {
            return {
