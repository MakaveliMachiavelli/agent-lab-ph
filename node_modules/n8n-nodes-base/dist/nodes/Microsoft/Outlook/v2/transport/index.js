...
exports.getOutlookCredentialType = getOutlookCredentialType;
 ...
 */
function getOutlookCredentialType() {
 ...
 */
function resolveMailbox(credentialType, itemIndex) {
    ...
    const value = (0, n8n_workflow_1.isResourceLocatorValue)(raw) ? raw.value : raw;
    ...
    (0, utils_1.validateMailbox)(mailbox, this.getNode());
    return mailbox;
}
...
async function microsoftApiRequest(method, resource, itemIndex, body = {}, qs = {}, uri, headers = {}, option = { json: true }) {
    ...
    try {
        ...
        if (((error.message || '').toLowerCase().includes('bad request') ||
            ...
            try {
                updatedError = utils_1.prepareApiError.call(this, error, itemIndex);
            }
            catch (e) { }
            if (updatedError)
                throw updatedError;
...
async function microsoftApiRequestAllItems(propertyName, method, endpoint, itemIndex, body = {}, query = {}, headers = {}) {
    const returnData = [];
    let responseData;
    let nextLink;
    query.$top = 100;
...
async function downloadAttachments(messages, prefix, itemIndex) {
    const elements = [];
...
async function getMimeContent(messageId, binaryPropertyName, itemIndex, outputFileName) {
...
async function getSubfolders(folders, itemIndex, addPathToDisplayName = false) {
