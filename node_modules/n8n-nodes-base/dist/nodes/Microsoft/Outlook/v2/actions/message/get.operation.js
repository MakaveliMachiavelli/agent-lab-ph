...
async function execute(index) {
    ...
    responseData = await transport_1.microsoftApiRequest.call(this, 'GET', `/messages/${messageId}`, index, undefined, qs);
    if (output === 'simple') {
    ...
    if (options.downloadAttachments) {
        ...
        executionData = await transport_1.downloadAttachments.call(this, responseData, prefix, index);
    }
    else {
    ...
    if (options.getMimeContent) {
        const { binaryPropertyName, outputFileName } = options.getMimeContent
            .values;
        const binary = await transport_1.getMimeContent.call(this, messageId, binaryPropertyName, index, outputFileName);
