...
async function execute(index) {
    ...
    const includeRecurringInstances = this.getNodeParameter('includeRecurringInstances', index);
    if (output === 'fields') {
    ...
    if (includeRecurringInstances) {
        qs.startDateTime = this.getNodeParameter('startDateTime', index);
        qs.endDateTime = this.getNodeParameter('endDateTime', index);
    }
    else {
    ...
    if (fromAllCalendars) {
        const response = await transport_1.microsoftApiRequest.call(this, 'GET', '/calendars', index, undefined, {
            $select: 'id',
        });
    ...
    for (const calendarId of calendars) {
        ...
        if (returnAll) {
            const response = await transport_1.microsoftApiRequestAllItems.call(this, 'value', 'GET', endpoint, index, undefined, qs);
            ...
            if (qs.$top <= 0)
                break;
            const response = await transport_1.microsoftApiRequest.call(this, 'GET', endpoint, index, undefined, qs);
