...
async function execute(index, items) {
    ...
    const attachmentDetails = await transport_1.microsoftApiRequest.call(this, 'GET', `/messages/${messageId}/attachments/${attachmentId}`, index, undefined, { $select: 'id,name,contentType' });
    let mimeType;
    if (attachmentDetails.contentType) {
        mimeType = attachmentDetails.contentType;
    }
    const fileName = attachmentDetails.name;
    const response = await transport_1.microsoftApiRequest.call(this, 'GET', `/messages/${messageId}/attachments/${attachmentId}/$value`, index, undefined, {}, undefined, {}, { encoding: null, resolveWithFullResponse: true });
    const newItem = {
