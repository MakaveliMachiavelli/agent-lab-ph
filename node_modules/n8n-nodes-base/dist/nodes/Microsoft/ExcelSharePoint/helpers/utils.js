...
exports.resolveWorkbookRoot = resolveWorkbookRoot;
 ...
 */
function validatePathSegment(node, label, value) {
    const trimmed = String(value ?? '').trim();
    if (trimmed === '') {
        throw new n8n_workflow_1.NodeOperationError(node, `The '${label}' parameter is empty`, {
            description: 'Set the value and try again.',
        });
    }
    if (/^\.+$/.test(trimmed)) {
        throw new n8n_workflow_1.NodeOperationError(node, `The '${label}' value is not valid`, {
 ...
 */
async function resolveWorkbookRoot(itemIndex = 0, workbookRootCache = new Map(), siteIdCache = new Map()) {
    ...
    if (workbook.mode === 'url') {
        if (workbookValue === '') {
            throw new n8n_workflow_1.NodeOperationError(this.getNode(), "The 'Workbook' parameter is empty", {
        ...
        const shareId = `u!${Buffer.from(workbookValue)
            ...
            .replace(/\+/g, '-')
            .replace(/\//g, '_')}`;
        const driveItem = await transport_1.microsoftApiRequest.call(this, 'GET', `/v1.0/shares/${shareId}/driveItem`, {}, { $select: 'id,parentReference' });
        const itemId = String(driveItem.id ?? '');
        const parent = driveItem.parentReference;
        if (itemId === '' || !parent?.siteId || !parent.driveId) {
            throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'The workbook address could not be resolved', {
 ...
 */
async function resolveSiteId(itemIndex, cache = new Map()) {
    ...
    try {
        ...
        throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'The site address is not valid', {
...
async function runPerItem(items, produce) {
    const returnData = [];
...
async function fetchCollection(itemIndex, endpoint, qs, filterPage) {
    const keep = filterPage ?? ((items) => items);
    if (this.getNodeParameter('returnAll', itemIndex)) {
        return keep(await (transport_1.microsoftApiRequestAllItems).call(this, endpoint, qs));
    }
    ...
    do {
        const response = uri
            ? await (transport_1.microsoftApiRequest).call(this, 'GET', '', {}, {}, uri)
            : await (transport_1.microsoftApiRequest).call(this, 'GET', endpoint, {}, { ...qs, $top: limit });
