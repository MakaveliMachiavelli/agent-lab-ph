...
exports.executeMatchUpdate = executeMatchUpdate;
 ...
 */
async function executeMatchUpdate(items, appendUnmatched) {
    const returnData = [];
    try {
        ...
        const workbookRoot = await utils_2.resolveWorkbookRoot.call(this, 0);
        const worksheetId = (0, utils_2.validatePathSegment)(this.getNode(), 'Sheet', this.getNodeParameter('worksheet', 0, '', { extractValue: true }));
        ...
        if (range !== '' && dataMode !== 'raw') {
            worksheetData = await transport_1.microsoftApiRequest.call(this, 'GET', `${sheetPath}/range(address='${range}')`);
        }
        if (range === '') {
            const qs = {};
            if (dataMode === 'raw') {
            ...
            worksheetData = await transport_1.microsoftApiRequest.call(this, 'GET', `${sheetPath}/usedRange`, {}, qs);
            range = String(worksheetData.address).split('!')[1];
        }
        if (dataMode === 'raw') {
            const rawRows = (0, utilities_1.processJsonInput)(this.getNodeParameter('data', 0), 'Data');
            const rawQs = {};
            if (rawData && options.fields) {
                rawQs.$select = options.fields;
            }
            const responseData = await (transport_1.microsoftApiRequest).call(this, 'PATCH', `${sheetPath}/range(address='${range}')`, { values: rawRows }, rawQs);
            returnData.push(...utils_1.prepareOutput.call(this, this.getNode(), responseData, { rawData, dataProperty }));
            return returnData;
        }
        ...
        if (dataMode === 'define') {
            updateSummary = utils_1.updateByDefinedValues.call(this, items.length, sheetValues, updateAll);
        }
        else {
            ...
            updateSummary = (0, utils_1.updateByAutoMaping)(items, sheetValues, columnToMatchOn, updateAll);
        }
        ...
        if (appendUnmatched && updateSummary.appendData.length) {
            ...
            if (!appendAfterSelectedRange) {
                const usedRange = await transport_1.microsoftApiRequest.call(this, 'GET', `${sheetPath}/usedRange`, {}, {
                    $select: 'address',
                });
                lastRow = (0, utils_1.parseAddress)(String(usedRange.address)).cellTo.row;
            }
        ...
        const responseData = await (transport_1.microsoftApiRequest).call(this, 'PATCH', `${sheetPath}/range(address='${range}')`, { values: updateSummary.updatedData }, patchQs);
        ...
        returnData.push(...this.helpers.constructExecutionMetaData(this.helpers.returnJsonArray({ error: message }), {
            itemData: (0, utilities_1.generatePairedItemData)(items.length),
        }));
    }
    return returnData;
}
