...
exports.ONEDRIVE_ILLEGAL_FILE_NAME_CHARS = void 0;
exports.validateOneDriveFileName = validateOneDriveFileName;
exports.getOneDriveCredentialType = getOneDriveCredentialType;
exports.validateResourceTargetId = validateResourceTargetId;
exports.getServicePrincipalResourceRoot = getServicePrincipalResourceRoot;
exports.driveEndpoint = driveEndpoint;
exports.resolveDriveScopeRoot = resolveDriveScopeRoot;
 ...
 */
exports.ONEDRIVE_ILLEGAL_FILE_NAME_CHARS = ['"', '*', ':', '<', '>', '?', '/', '\\', '|'];
/**
 ...
 */
function validateOneDriveFileName(node, fileName, itemIndex) {
    if (fileName === undefined || fileName.trim() === '') {
        throw new n8n_workflow_1.NodeOperationError(node, 'File name must be set!', { itemIndex });
    }
    const illegalChars = exports.ONEDRIVE_ILLEGAL_FILE_NAME_CHARS.filter((char) => fileName.includes(char));
    if (illegalChars.length > 0) {
        throw new n8n_workflow_1.NodeOperationError(node, `The file name "${fileName}" contains characters that OneDrive doesn't allow: ${illegalChars.join(' ')}`, {
            itemIndex,
            description: `OneDrive file names can't contain any of these characters: ${exports.ONEDRIVE_ILLEGAL_FILE_NAME_CHARS.join(' ')}. Remove them from the file name and try again.` +
 ...
 * selected.
 */
function getOneDriveCredentialType() {
 ...
 */
function validateResourceTargetId(target, id, node) {
    ...
    if (target === 'user') {
        (0, GenericFunctions_1.validateUserTargetId)(id, node);
        return;
    }
    if (target !== 'drive') {
        throw new n8n_workflow_1.NodeOperationError(node, 'The target ID is not valid', {
            description: 'Unknown target type.',
        });
    }
    // Drive target: empty → dots-only → drive-id shape, with drive-specific wording.
    if (id === '') {
        throw new n8n_workflow_1.NodeOperationError(node, 'A target ID is required for the Service Principal', {
 ...
 */
function getServicePrincipalResourceRoot(target, rawId, node) {
 ...
 */
function resolveDriveScopeRoot(isPoll, itemIndex = 0) {
    ...
    const target = isPoll
        ? this.getNodeParameter('resourceTarget', 'user')
        : this.getNodeParameter('resourceTarget', itemIndex, 'user');
    const id = isPoll
        ? this.getNodeParameter(`${target}Target`, '', { extractValue: true })
        : this.getNodeParameter(`${target}Target`, itemIndex, '', { extractValue: true });
 ...
 */
async function microsoftApiRequest(method, resource, body = {}, qs = {}, uri, headers = {}, option = { json: true }, driveScopeRoot) {
    ...
    if (!uri && driveScopeRoot) {
        ...
        if (!resource.startsWith('/drive')) {
            ...
            throw new n8n_workflow_1.OperationalError('microsoftApiRequest: a scoped resource must start with "/drive"');
        }
...
async function microsoftApiRequestAllItems(propertyName, method, endpoint, body = {}, query = {}, driveScopeRoot) {
    const returnData = [];
    let responseData;
    let uri;
    query.$top = 100;
    do {
        ...
        responseData = await microsoftApiRequest.call(this, method, endpoint, body, query, uri, {}, { json: true }, driveScopeRoot);
...
async function microsoftApiRequestAllItemsSkip(propertyName, method, endpoint, body = {}, query = {}, driveScopeRoot) {
    const returnData = [];
    let responseData;
    query.$top = 100;
    query.$skip = 0;
    do {
        responseData = await microsoftApiRequest.call(this, method, endpoint, body, query, undefined, {}, { json: true }, driveScopeRoot);
...
async function getPath(itemId) {
    ...
    const responseData = (await microsoftApiRequest.call(this, 'GET', `/drive/items/${encodeURIComponent(itemId)}`, {}, {}, undefined, {}, { json: true }, driveScopeRoot));
