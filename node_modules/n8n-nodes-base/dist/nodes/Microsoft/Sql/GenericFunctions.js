...
async function executeSqlQueryAndPrepareResults(pool, rawQuery, itemIndex, queryValues = [], nodeVersion) {
    const request = pool.request();
    ...
    if (queryValues.length > 0) {
        ...
        for (let i = queryValues.length; i >= 1; i--) {
            const paramName = `p${i}`;
            processedQuery = n8n_workflow_1.safeRegex.replace(`\\$${i}(?!\\d)`, processedQuery, 'g', `@${paramName}`);
