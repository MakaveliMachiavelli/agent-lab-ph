...
exports.REQUIRED_PERMISSIONS = exports.DEFAULT_GRAPH_BASE_URL = exports.SERVICE_PRINCIPAL_AUTH = void 0;
exports.getSharePointCredentialType = getSharePointCredentialType;
...
exports.SERVICE_PRINCIPAL_AUTH = 'microsoftEntraServicePrincipalApi';
exports.DEFAULT_GRAPH_BASE_URL = 'https://graph.microsoft.com';
...
exports.REQUIRED_PERMISSIONS = Object.freeze({
    'file:download': {
    ...
    'item:get': {
        ...
        application: 'Sites.Read.All (or Sites.Selected granted for this site, or Files.Read.All for document-library items)',
    },
    'item:delete': {
        delegated: 'Sites.ReadWrite.All (or Files.ReadWrite.All for document-library items)',
        application: 'Sites.ReadWrite.All (or Sites.Selected granted for this site, or Files.ReadWrite.All for document-library items)',
    },
});
function getSharePointCredentialType() {
    ...
    return selected === exports.SERVICE_PRINCIPAL_AUTH ? exports.SERVICE_PRINCIPAL_AUTH : 'microsoftOAuth2Api';
}
...
function lookupRequiredPermissions() {
    try {
        ...
        if (typeof resource === 'string' && typeof operation === 'string') {
            return exports.REQUIRED_PERMISSIONS[`${resource}:${operation}`];
        }
    }
    catch {
...
function nodeResourceName() {
    try {
        ...
        if (typeof resource === 'string' && resource !== '') {
            return (0, utilities_1.capitalize)(resource);
        }
    }
    catch {
        // Load-options contexts may not expose this parameter
...
function servicePrincipalApiError(error) {
    const rawCode = error.httpCode ?? error.statusCode;
    const httpCode = rawCode === undefined || rawCode === null ? undefined : Number(rawCode);
    ...
    return new n8n_workflow_1.NodeApiError(this.getNode(), sanitizedError, errorOptions);
}
function delegatedApiError(error) {
    const errorOptions = {};
    ...
    return new n8n_workflow_1.NodeApiError(this.getNode(), error, errorOptions);
}
async function microsoftApiRequest(method, resource, body = {}, qs = {}, uri, headers = {}) {
    ...
    const isServicePrincipal = credentialType === exports.SERVICE_PRINCIPAL_AUTH;
    const credentials = await this.getCredentials(credentialType);
    const baseUrl = (typeof credentials.graphApiBaseUrl === 'string' && credentials.graphApiBaseUrl !== ''
        ? credentials.graphApiBaseUrl
        : exports.DEFAULT_GRAPH_BASE_URL).replace(/\/+$/, '');
    ...
    if (new URL(target).origin !== new URL(baseUrl).origin) {
        throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'Refusing to send credentials to an unexpected host');
    }
...
async function microsoftApiRequestAllItems(propertyName, method, endpoint, body = {}, qs = {}, limit, headers = {}) {
    const returnData = [];
    let uri;
    do {
        ...
        const responseData = await microsoftApiRequest.call(this, method, endpoint, body, uri ? {} : qs, uri, headers);
