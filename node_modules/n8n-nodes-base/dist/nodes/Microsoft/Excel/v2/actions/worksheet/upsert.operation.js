...
async function execute(items) {
    ...
    try {
        ...
        if (range && dataMode !== 'raw') {
            worksheetData = await transport_1.microsoftApiRequest.call(this, 'PATCH', `/drive/items/${workbookId}/workbook/worksheets/${worksheetId}/range(address='${range}')`, undefined, undefined, undefined, undefined, 0);
        }
        ...
        if (range === '') {
            ...
            worksheetData = await transport_1.microsoftApiRequest.call(this, 'GET', `/drive/items/${workbookId}/workbook/worksheets/${worksheetId}/usedRange`, undefined, query, undefined, undefined, 0);
        ...
        if (dataMode === 'raw') {
            ...
            responseData = await transport_1.microsoftApiRequest.call(this, 'PATCH', `/drive/items/${workbookId}/workbook/worksheets/${worksheetId}/range(address='${range}')`, { values }, undefined, undefined, undefined, 0);
        }
        if (dataMode !== 'raw' &&
        ...
        if (updateSummary.appendData.length) {
            ...
            if (nodeVersion > 2 && !appendAfterSelectedRange) {
                const { address } = await transport_1.microsoftApiRequest.call(this, 'GET', `/drive/items/${workbookId}/workbook/worksheets/${worksheetId}/usedRange`, undefined, { select: 'address' }, undefined, undefined, 0);
                const usedRange = (0, utils_1.parseAddress)(address);
                lastRow = usedRange.cellTo.row;
            }
            range = `${cellFrom.value}:${cellTo.column}${Number(lastRow) + appendValues.length}`;
        }
        responseData = await transport_1.microsoftApiRequest.call(this, 'PATCH', `/drive/items/${workbookId}/workbook/worksheets/${worksheetId}/range(address='${range}')`, { values: updateSummary.updatedData }, undefined, undefined, undefined, 0);
