...
exports.getExcelCredentialType = getExcelCredentialType;
...
exports.resolveScopeRoot = resolveScopeRoot;
...
function getExcelCredentialType() {
 ...
 */
function resolveScopeRoot(itemIndex) {
...
async function microsoftApiRequest(method, resource, body = {}, qs = {}, uri, headers = {}, itemIndex) {
    ...
    if (!uri && isServicePrincipal) {
        ...
        if (driveScopeRoot) {
            ...
            if (!resource.startsWith('/drive')) {
                throw new n8n_workflow_1.OperationalError(`microsoftApiRequest: scoped resource must start with "/drive" (got "${resource}")`);
            }
...
async function microsoftApiRequestAllItems(propertyName, method, endpoint, body = {}, query = {}, itemIndex) {
    const returnData = [];
    let responseData;
    let uri;
    query.$top = 100;
...
async function microsoftApiRequestAllItemsSkip(propertyName, method, endpoint, body = {}, query = {}, itemIndex) {
    const returnData = [];
    let responseData;
    query.$top = 100;
    query.$skip = 0;
