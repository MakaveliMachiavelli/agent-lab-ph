...
exports.rowToExecutionData = exports.configuredInputs = void 0;
...
exports.getNodeInputsData = getNodeInputsData;
exports.modifySelectQuery = modifySelectQuery;
...
function checkMatchFieldsInput(data) {
    if (data.length === 1 && data[0].field1 === '' && data[0].field2 === '') {
        throw new n8n_workflow_1.UserError('You need to define at least one pair of fields in "Fields to Match" to match on', { level: 'warning' });
    }
    for (const [index, pair] of data.entries()) {
        if (pair.field1 === '' || pair.field2 === '') {
            throw new n8n_workflow_1.UserError(`You need to define both fields in "Fields to Match" for pair ${index + 1},
				 field 1 = '${pair.field1}'
...
function checkInput(input, fields, disableDotNotation, inputLabel) {
    for (const field of fields) {
        ...
        if (!isPresent) {
            throw new n8n_workflow_1.UserError(`Field '${field}' is not present in any of items in '${inputLabel}'`, {
                level: 'warning',
            });
        }
    }
...
function getNodeInputsData() {
    const returnData = [];
    const inputs = n8n_workflow_1.NodeHelpers.getConnectionTypes(this.getNodeInputs()).filter((type) => type === n8n_workflow_1.NodeConnectionTypes.Main);
...
exports.rowToExecutionData = rowToExecutionData;
