...
const query_parameters_1 = require("../../utils/query-parameters");
...
class MongoDb {
            ...
            async mongoDbCredentialTest(credential) {
                ...
                try {
                    ...
                    if (!databases.map((db) => db.name).includes(database)) {
                        throw new n8n_workflow_1.UserError(`Database "${database}" does not exist`, {
                            level: 'warning',
                        });
                    }
                    await client.close();
    ...
    async execute() {
        ...
        const { database, connectionString } = (0, GenericFunctions_1.validateAndResolveMongoCredentials)(node, credentials);
        ...
        try {
            ...
            if (operation === 'aggregate') {
                for (let i = 0; i < itemsLength; i++) {
                    try {
                        const queryParameter = (0, query_parameters_1.parseAndResolveQueryParameters)(this.getNodeParameter('query', i), this.getNodeParameter('queryParameters', i, '[]'), node, i);
            ...
            if (operation === 'findOneAndReplace') {
                ...
                if (nodeVersion >= 1.3) {
                    for (let i = 0; i < itemsLength; i++) {
                        const fields = (0, GenericFunctions_1.prepareFields)(this.getNodeParameter('fields', i));
                        const useDotNotation = this.getNodeParameter('options.useDotNotation', i, false);
                        const dateFields = (0, GenericFunctions_1.prepareFields)(this.getNodeParameter('options.dateFields', i, ''));
                        const updateKey = (this.getNodeParameter('updateKey', i) || '').trim();
                        const updateOptions = this.getNodeParameter('upsert', i)
                            ? { upsert: true }
                            : undefined;
                        try {
                            const [item] = (0, GenericFunctions_1.prepareItems)({
                                items: [items[i]],
                                fields,
                                updateKey,
                                useDotNotation,
                            ...
                            if (!item) {
                                throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'Item is missing the updateKey field', { itemIndex: i });
                            }
                ...
                if (nodeVersion >= 1.3) {
                    ...
                    for (let i = 0; i < itemsLength; i++) {
                        try {
                            ...
                            const useDotNotation = this.getNodeParameter('options.useDotNotation', i, false);
                            const dateFields = (0, GenericFunctions_1.prepareFields)(this.getNodeParameter('options.dateFields', i, ''));
                            const [insertItem] = (0, GenericFunctions_1.prepareItems)({
                                items: [items[i]],
                                fields,
                                updateKey: '',
                                useDotNotation,
        ...
        if (nodeVersion >= 1.4) {
            return [await GenericFunctions_1.serializeMongoItems.call(this, returnData)];
        }
