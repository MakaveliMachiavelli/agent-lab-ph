/**
 * Salesforce Node - Version 1.1
 * Discriminator: resource=contact, operation=create
 */


interface Credentials {
  salesforceOAuth2Api: CredentialReference;
  salesforceJwtApi: CredentialReference;
}

/** Represents a contact, which is an individual associated with an account */
export type SalesforceV11ContactCreateParams = {
  resource: 'contact';
  operation: 'create';
/**
 * OAuth Authorization Flow
 * @default oAuth2
 */
    authentication?: 'oAuth2' | 'jwt' | Expression<string>;
/**
 * Required. Last name of the contact. Limited to 80 characters.
 */
    lastname: string | Expression<string>;
/**
 * Additional Fields
 * @default {}
 */
    additionalFields?: {
    /** The account that is the parent of this contact
     * @searchListMethod searchAccounts
     * @default {"mode":"list","value":""}
     */
    acconuntId?: { __rl: true; mode: 'list' | 'id'; value: string; cachedResultName?: string };
    /** The name of the assistant
     */
    assistantName?: string | Expression<string>;
    /** The telephone number of the assistant
     */
    'Assistant Phone'?: string | Expression<string>;
    /** The birth date of the contact
     */
    birthdate?: string | Expression<string>;
    /** Filter by custom fields
     * @default {}
     */
    customFieldsUi?: {
        /** Custom Field
     */
    customFieldsValues?: Array<{
      /** The ID of the field to add custom field to. Choose from the list, or specify an ID using an &lt;a href="https://docs.n8n.io/code/expressions/"&gt;expression&lt;/a&gt;.
       * @loadOptionsMethod getCustomFields
       */
      fieldId?: string | Expression<string>;
      /** The value to set on custom field
       */
      value?: string | Expression<string>;
    }>;
  };
    /** The department of the contact
     */
    department?: string | Expression<string>;
    /** A description of the contact. Label is Contact Description. Limit: 32 KB.
     */
    description?: string | Expression<string>;
    /** Email address for the contact
     */
    email?: string | Expression<string>;
    /** If bounce management is activated and an email sent to the contact bounces, the date and time the bounce occurred
     */
    otherPostalCode?: string | Expression<string>;
    /** If bounce management is activated and an email sent to the contact bounces, the reason the bounce occurred
     */
    emailBouncedReason?: string | Expression<string>;
    /** Whether the contact does not want to receive email from Salesforce
     * @default false
     */
    hasOptedOutOfEmail?: boolean | Expression<boolean>;
    /** Fax number for the contact. Label is Business Fax.
     */
    fax?: string | Expression<string>;
    /** First name of the contact. Maximum size is 40 characters.
     */
    firstName?: string | Expression<string>;
    /** Middle name of the contact. Maximum size is 40 characters.
     */
    middleName?: string | Expression<string>;
    /** Name suffix of the contact. Maximum size is 40 characters.
     */
    suffix?: string | Expression<string>;
    /** Home telephone number for the contact
     */
    homePhone?: string | Expression<string>;
    /** References the ID of a contact in Data.com. If a contact has a value in this field, it means that a contact was imported as a contact from Data.com.
     */
    jigsaw?: string | Expression<string>;
    /** Source from which the lead was obtained. Choose from the list, or specify an ID using an &lt;a href="https://docs.n8n.io/code/expressions/"&gt;expression&lt;/a&gt;.
     * @loadOptionsMethod getLeadSources
     */
    leadSource?: string | Expression<string>;
    /** Mailing City
     */
    mailingCity?: string | Expression<string>;
    /** Mailing Country
     */
    mailingCountry?: string | Expression<string>;
    /** Contact’s mobile phone number
     */
    mobilePhone?: string | Expression<string>;
    /** Mailing Postal Code
     */
    mailingPostalCode?: string | Expression<string>;
    /** Mailing State
     */
    mailingState?: string | Expression<string>;
    /** Street address for mailing address
     */
    mailingStreet?: string | Expression<string>;
    /** Other City
     */
    otherCity?: string | Expression<string>;
    /** Other Country
     */
    otherCountry?: string | Expression<string>;
    /** Telephone for alternate address
     */
    otherPhone?: string | Expression<string>;
    /** Other Postal Code
     */
    otherPostalCode?: string | Expression<string>;
    /** Other State
     */
    otherState?: string | Expression<string>;
    /** Street for alternate address
     */
    otherStreet?: string | Expression<string>;
    /** The user who owns the contact
     * @searchListMethod searchUsers
     * @default {"mode":"list","value":""}
     */
    owner?: { __rl: true; mode: 'list' | 'id'; value: string; cachedResultName?: string };
    /** Phone number for the contact
     */
    phone?: string | Expression<string>;
    /** Choose from the list, or specify an ID using an &lt;a href="https://docs.n8n.io/code/expressions/"&gt;expression&lt;/a&gt;
     * @loadOptionsMethod getRecordTypes
     */
    recordTypeId?: string | Expression<string>;
    /** Honorific abbreviation, word, or phrase to be used in front of name in greetings, such as Dr. or Mrs.
     */
    salutation?: string | Expression<string>;
    /** The contact's personal pronouns
     */
    pronouns?: string | Expression<string>;
    /** The contact's gender identity
     */
    genderIdentity?: string | Expression<string>;
    /** Title of the contact such as CEO or Vice President
     */
    title?: string | Expression<string>;
  };
};

export type SalesforceV11ContactCreateOutput = {
  id?: string;
  success?: boolean;
};

export type SalesforceV11ContactCreateNode = {
  type: 'n8n-nodes-base.salesforce';
  version: 1.1;
  config: NodeConfig<SalesforceV11ContactCreateParams> & { credentials?: Credentials };
  output?: Items<SalesforceV11ContactCreateOutput>;
};