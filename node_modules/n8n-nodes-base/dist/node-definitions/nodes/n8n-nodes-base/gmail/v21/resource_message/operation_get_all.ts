 ...
 * @builderHint Keep true by default. When true, returns lightweight metadata (labels, subject, from, to, snippet) per message. When false, fetches and parses the full raw email for every message (adds html, text, textAsHtml, headers, attachments), which uses much more memory and is a common cause of out-of-memory crashes. Only set false when the email body is actually required, e.g. for AI classification, summarization, or content processing.
 * @default true
 */
...
export type GmailV21MessageGetAllNode = {
  ...
  version: 2.1;
  config: NodeConfig<GmailV21MessageGetAllParams> & { credentials?: Credentials };
