/**
 * ServiceNow Node - Version 1
 * Discriminator: resource=user, operation=update
 */


interface Credentials {
  serviceNowOAuth2Api: CredentialReference;
  serviceNowBasicApi: CredentialReference;
}

export type ServiceNowV1UserUpdateParams = {
  resource: 'user';
  operation: 'update';
/**
 * Authentication method to use
 * @default oAuth2
 */
    authentication?: 'basicAuth' | 'oAuth2' | Expression<string>;
/**
 * Unique identifier of the user
 */
    id: string | Expression<string>;
/**
 * Update Fields
 * @default {}
 */
    updateFields?: {
    /** Whether to activate the user
     * @default false
     */
    active?: boolean | Expression<boolean>;
    /** The Building address
     */
    building?: string | Expression<string>;
    /** City of the user
     */
    city?: string | Expression<string>;
    /** The name of the company for the user
     */
    company?: string | Expression<string>;
    /** Country of the user
     */
    country?: string | Expression<string>;
    /** Department of the user
     */
    department?: string | Expression<string>;
    /** The email address associated with the user
     */
    email?: string | Expression<string>;
    /** The first name of the user
     */
    first_name?: string | Expression<string>;
    /** The gender of the user
     */
    gender?: string | Expression<string>;
    /** Home phone of the user
     */
    home_phone?: string | Expression<string>;
    /** The last name of the user
     */
    last_name?: string | Expression<string>;
    /** Location of the user
     */
    location?: string | Expression<string>;
    /** Manager of the user
     */
    manager?: string | Expression<string>;
    /** The middle name of the user
     */
    middle_name?: string | Expression<string>;
    /** Mobile phone number of the user
     */
    mobile_phone?: string | Expression<string>;
    /** The user's password
     */
    user_password?: string | Expression<string>;
    /** Whether to require a password reset when the user logs in
     * @default false
     */
    password_needs_reset?: boolean | Expression<boolean>;
    /** The main phone number of the user
     */
    phone?: string | Expression<string>;
    /** Roles of the user. Choose from the list, or specify IDs using an &lt;a href="https://docs.n8n.io/code/expressions/"&gt;expression&lt;/a&gt;.
     * @loadOptionsMethod getUserRoles
     * @default []
     */
    roles?: string[];
    /** Source
     */
    source?: string | Expression<string>;
    /** State for the user
     */
    state?: string | Expression<string>;
    /** Street information for the user separated by comma
     */
    street?: string | Expression<string>;
    /** A username associated with the user (e.g. user_name.123)
     */
    user_name?: string | Expression<string>;
    /** Zip code for the user
     */
    zip?: string | Expression<string>;
  };
};

export type ServiceNowV1UserUpdateNode = {
  type: 'n8n-nodes-base.serviceNow';
  version: 1;
  config: NodeConfig<ServiceNowV1UserUpdateParams> & { credentials?: Credentials };
};