...
// src/utils.ts
function renderMessageToLinearMarkdown(message, formatConverter) {
  const card = extractCard(message);
  const rendered = card ? cardToLinearMarkdown(card) : formatConverter.renderPostable(message);
  return convertEmojiPlaceholders(rendered, "linear");
}
function assertAgentSessionThread(thread) {
...
var PROFILE_URL_REGEX = /^https:\/\/linear\.app\/\S+\/profiles\/([^/?#]+)/;
...
function calculateExpiry(expiresIn) {
  return typeof expiresIn === "number" ? Date.now() + expiresIn * 1e3 : null;
}

// src/index.ts
var COMMENT_SESSION_THREAD_PATTERN = /^([^:]+):c:([^:]+):s:([^:]+)$/;
var COMMENT_THREAD_PATTERN = /^([^:]+):c:([^:]+)$/;
var ISSUE_SESSION_THREAD_PATTERN = /^([^:]+):s:([^:]+)$/;
var INSTALLATION_KEY_PREFIX = "linear:installation";
var INSTALLATION_REFRESH_BUFFER_MS = 5 * 60 * 1e3;
function parseEnvClientCredentialScopes(value) {
  if (!value) {
    return void 0;
  }
...
var LinearAdapter = class {
  ...
  constructor(config = {}) {
    ...
    const webhookSecret = webhookVerifier ? void 0 : config.webhookSecret ?? process.env.LINEAR_WEBHOOK_SECRET;
    ...
    const encryptionKey = config.encryptionKey ?? process.env.LINEAR_ENCRYPTION_KEY;
    if (encryptionKey) {
    ...
    const clientCredentialsClientId = process.env.LINEAR_CLIENT_CREDENTIALS_CLIENT_ID;
    const clientCredentialsClientSecret = process.env.LINEAR_CLIENT_CREDENTIALS_CLIENT_SECRET;
   ...
   */
  async withConnectClient(organizationId, fn) {
    const provider = this.accessTokenProvider;
    if (!provider) {
      return await fn();
    }
  ...
  normalizeClientCredentials(clientCredentials, source) {
   ...
   */
  get organizationId() {
    const organizationId = this.requestContext.getStore()?.installation.organizationId ?? this.defaultOrganizationId;
    if (!organizationId) {
      throw new AuthenticationError(
        "linear",
   ...
   */
  get botUserId() {
    const id = this.requestContext.getStore()?.installation.botUserId ?? this.defaultBotUserId;
    if (!id) {
  ...
  installationKey(organizationId) {
   ...
   */
  async setInstallation(organizationId, installation) {
    if (!this.chat) {
      throw new ValidationError2(
        "linear",
    ...
    await this.chat.getState().set(this.installationKey(organizationId), dataToStore);
   ...
   */
  async getInstallation(organizationId) {
    if (!this.chat) {
      throw new ValidationError2(
        "linear",
        "Adapter not initialized. Ensure chat.initialize() has been called first."
    ...
    if (contextInstallation && contextInstallation.installation.organizationId === organizationId) {
      return contextInstallation.installation;
    }
    const stored = await this.chat.getState().get(this.installationKey(organizationId));
    if (!stored) {
      return null;
    }
   ...
   */
  encryptInstallation(installation) {
   ...
   */
  decryptInstallation(stored) {
    const accessToken = this.maybeDecrypt(stored.accessToken);
    const refreshToken = stored.refreshToken === void 0 ? void 0 : this.maybeDecrypt(stored.refreshToken);
    return {
   ...
   */
  async deleteInstallation(organizationId) {
    if (!this.chat) {
      throw new ValidationError2(
        "linear",
        "Adapter not initialized. Ensure chat.initialize() has been called first."
   ...
   */
  async withInstallation(organizationId, fn) {
    if (this.isConnectMode()) {
      const orgId = typeof organizationId === "string" ? organizationId : organizationId.organizationId;
      return await this.withConnectClient(orgId, fn);
    }
    if (!this.isMultiTenantMode()) {
      return await fn();
    }
    const installation = typeof organizationId === "string" ? await this.requireInstallation(organizationId) : await this.refreshInstallation(organizationId);
    const context = {
      installation,
   ...
   */
  async fetchClientIdentity(client) {
   ...
   */
  async fetchOAuthToken(body, errorMessage) {
  ...
  async refreshInstallation(installation) {
    ...
    if (installation.expiresAt !== null && installation.expiresAt > Date.now() + INSTALLATION_REFRESH_BUFFER_MS) {
      return installation;
    }
    const token = await this.fetchOAuthToken(
      new URLSearchParams({
   ...
   */
  async requireInstallation(organizationId) {
  ...
  async refreshClientCredentialsToken() {
    ...
    this.accessTokenExpiry = typeof data.expires_in === "number" ? Date.now() + data.expires_in * 1e3 - 36e5 : null;
    this.logger.info("Linear client credentials token obtained", {
      expiresIn: typeof data.expires_in === "number" ? `${Math.round(data.expires_in / 86400)} days` : "unknown"
    });
  }
  /**
   ...
   */
  async parseMessageFromComment(comment, issueId, agentSessionId) {
    let user;
    if (comment.userId) {
      ...
      user = {
        ...
        avatarUrl: fetchedUser.avatarUrl ?? void 0
      };
    } else {
      ...
      user = {
        ...
        email: void 0,
        avatarUrl: botActor.avatarUrl ?? void 0
      };
    }
    const commentData = {
      id: comment.id,
   ...
   */
  parseMessageFromAgentSessionEvent(payload) {
    ...
    if (payload.action === "prompted") {
      ...
      const content = agentActivity.content;
      const commentData = {
      ...
      return this.parseMessage({
        ...
        comment: commentData,
        agentSessionId: payload.agentSession.id,
        agentSessionPromptContext: payload.promptContext ?? void 0
      });
    }
   ...
   */
  async parseMessageFromAgentActivity(threadId, result) {
   ...
   */
  async handleVerifiedWebhook(request, options) {
   ...
   */
  async dispatchWebhookEvent(payload, options) {
    switch (payload.type) {
      case "OAuthApp":
  ...
  async onCommentEvent(payload, options) {
  ...
  async onAgentSessionEvent(payload, options) {
    await this.withWebhookInstallation(payload.organizationId, () => {
   ...
   */
  async withWebhookInstallation(organizationId, fn) {
    if (this.isConnectMode()) {
  ...
  handleCommentCreated(payload, options) {
    ...
    const comment = {
      ...
      user: {
        ...
        avatarUrl: data.user.avatarUrl ?? void 0
      }
    };
   ...
   */
  handleAgentSessionEvent(payload, options) {
    if (!this.chat) {
      this.logger.warn(
   ...
   */
  async streamInAgentSession(decoded, textStream, _options) {
    const renderer = new StreamingMarkdownRenderer();
    const client = this.getClient();
    assertAgentSessionThread(decoded);
    let lastAppended = "";
    const flushMarkdown = async (type, markdown = renderer.getCommittableText(), force = false) => {
   ...
   */
  async streamAsComment(threadId, textStream, options) {
    const intervalMs = options?.updateIntervalMs ?? 500;
    let rawMessage = await this.postMessage(threadId, "...");
   ...
   */
  async fetchAgentSessionMessages(thread, options) {
    ...
    return {
      messages,
      nextCursor: childrenConnection.pageInfo.hasNextPage ? childrenConnection.pageInfo.endCursor ?? void 0 : void 0
    };
  }
  /**
   * Fetch top-level comments on an issue.
   */
  async fetchIssueComments(issueId, options) {
    ...
    return {
      messages,
      nextCursor: commentsConnection.pageInfo.hasNextPage ? commentsConnection.pageInfo.endCursor ?? void 0 : void 0
    };
  }
  /**
   * Fetch a comment thread (root comment + its children/replies).
   */
  async fetchCommentThread(issueId, commentId, options) {
    const linear = this.getClient();
   ...
   */
  async commentsToMessages(comments, issueId, agentSessionId) {
    const messages = [];
  ...
  parseMessage(raw) {
    ...
    return new Message({
      ...
      threadId: this.encodeThreadId({
        ...
        agentSessionId: raw.kind === "agent_session_comment" ? raw.agentSessionId : void 0
      }),
      text,
      formatted,
      author: {
  ...
  async fetchSubject(raw) {
    ...
    try {
      ...
      return {
        ...
        status: state?.name,
        url: issue.url,
        assignee: assignee ? { id: assignee.id, name: assignee.displayName } : void 0,
