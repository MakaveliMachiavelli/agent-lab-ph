...
exports.resolveSupportedCredentialActivation = resolveSupportedCredentialActivation;
            ...
            return actualValues.every((propertyValue) => {
                ...
                if (key === 'regex') {
                    return (typeof propertyValue === 'string' &&
                        safe_regex_1.safeRegex.test(targetValue, propertyValue));
                }
                if (key === 'exists') {
    ...
    if (parameterMode.validation) {
        for (const validation of parameterMode.validation) {
            if (validation && validation.type === 'regex') {
                ...
                if (!safe_regex_1.safeRegex.test(`^${regexValidation.properties.regex}$`, valueToValidate)) {
...
function isNodeWithWorkflowSelector(node) {
    return [
        constants_1.EXECUTE_WORKFLOW_NODE_TYPE,
        constants_1.WORKFLOW_TOOL_LANGCHAIN_NODE_TYPE,
        constants_1.RETRIEVER_WORKFLOW_LANGCHAIN_NODE_TYPE,
    ].includes(node.type);
}
/**
 ...
 */
function resolveSupportedCredentialActivation(nodeTypeDescription, node, isSupported, preferredType) {
