...
class Expression {
    ...
    static async initExpressionEngine(options) {
        ...
        if (!this.vmEvaluator) {
            ...
            this.vmEvaluator = new ExpressionEvaluator({
                ...
                hooks: expression_sandboxing_1.expressionSandboxHooks,
                logger: LoggerProxy,
    ...
    async acquireIsolate() {
        if (Expression.vmEvaluator)
            return await Expression.vmEvaluator.acquire(this);
        return false;
    }
    ...
    convertObjectValueToString(value) {
        if (value instanceof luxon_1.DateTime && value.invalidReason !== null) {
            throw new errors_1.UserError('invalid DateTime');
        }
        if (value === null) {
            return 'null';
        }
    ...
    resolveSimpleParameterValue(parameterValue, data, returnObjectAsString = false) {
        ...
        if (typeof returnValue === 'function') {
            if (returnValue.name === 'DateTime')
                throw new errors_1.UserError('this is a DateTime, please access its methods');
            throw new errors_1.UserError('this is a function, please add ()');
        }
    ...
    renderExpression(expression, data) {
        ...
        if (Expression.expressionEngine === 'vm' && !IS_FRONTEND) {
            if (!Expression.vmEvaluator) {
                throw new errors_1.UnexpectedError('The VM expression engine has not been initialized. Call Expression.initExpressionEngine() during application startup.');
            }
            try {
        ...
        try {
            ...
            if (isExpressionError(error))
                throw error;
            if (isSyntaxError(error))
                throw new errors_1.UserError('invalid syntax');
            if (isTypeError(error) && IS_FRONTEND && error.message.endsWith('is not a function')) {
                ...
                if (!match?.groups?.msg)
                    return null;
                throw new errors_1.UserError(match.groups.msg);
            }
        }
        return null;
    }
