...
exports.executeFilterConditionAsync = executeFilterConditionAsync;
...
function prepareFilterCondition(condition, filterOptions, metadata = {}) {
...
function evaluateExistsOperation(condition, exists) {
...
function prepareStringFilterValues(condition, prepared) {
...
function executeFilterConditionWithRegexTest(condition, filterOptions, metadata, regexTest) {
    const { operator } = condition;
        ...
        case 'string': {
            ...
            switch (condition.operator.operation) {
                ...
                case 'notEndsWith':
                    return !left.endsWith(right);
                case 'regex': {
                    const { source, flags } = (0, safe_regex_1.parseRegexLiteral)(right);
...
function executeFilterCondition(condition, filterOptions, metadata = {}) {
    return executeFilterConditionWithRegexTest(condition, filterOptions, metadata, (pattern, input, flags) => safe_regex_1.safeRegex.test(pattern, input, flags));
}
/**
 ...
 */
async function executeFilterConditionAsync(condition, filterOptions, regexTest, metadata = {}) {
    const { operator } = condition;
    const prepared = prepareFilterCondition(condition, filterOptions, metadata);
    ...
    if (operator.type !== 'string') {
        return executeFilterConditionWithRegexTest(condition, filterOptions, metadata, 
