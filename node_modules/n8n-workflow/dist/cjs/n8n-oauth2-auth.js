...
function sendUnauthorizedResponse(resp, code, prmUrl, realm, error) {
    const authenticateHeader = `Bearer realm="${realm}", resource_metadata="${prmUrl}"` + (error ? `, error="${error}"` : '');
    resp.writeHead(code, {
 ...
 */
const n8nOAuth2Auth = async (context, options) => {
    const webhookUrl = context.getWebhookResourceUrl('default');
    if (!webhookUrl) {
        throw new errors_1.UnexpectedError('Webhook URL is not available');
    }
...
exports.n8nOAuth2Auth = n8nOAuth2Auth;
//# sourceMappingURL=n8n-oauth2-auth.js.map