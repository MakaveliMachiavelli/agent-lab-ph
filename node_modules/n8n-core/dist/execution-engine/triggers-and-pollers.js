...
let TriggersAndPollers = class TriggersAndPollers {
    async runTriggerFunction(workflow, node, getTriggerFunctions, additionalData, mode, activation) {
        const triggerFunctions = getTriggerFunctions(workflow, node, additionalData, mode, activation);
        const nodeType = workflow.nodeTypes.getByNameAndVersion(node.type, node.typeVersion);
        if (!nodeType.trigger) {
            throw new n8n_workflow_1.UnexpectedError('Node type does not have a trigger function defined', {
    ...
    wrapCloseFunctionInIsolate(workflow, response) {
        const closeFunction = response?.closeFunction;
        if (response && closeFunction) {
            response.closeFunction = async () => await workflow.expression.withIsolate(async () => await closeFunction.call(response));
        }
        return response;
    }
    async runPollFunction(workflow, node, pollFunctions) {
        const nodeType = workflow.nodeTypes.getByNameAndVersion(node.type, node.typeVersion);
        if (!nodeType.poll) {
            throw new n8n_workflow_1.UnexpectedError('Node type does not have a poll function defined', {
                extra: { nodeName: node.name },
                tags: { nodeType: node.type },
            });
        }
