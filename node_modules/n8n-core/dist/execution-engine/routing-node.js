...
const get_additional_keys_1 = require("./node-execution-context/utils/get-additional-keys");
class RoutingNode {
    ...
    async runNode() {
        ...
        for (let itemIndex = 0; itemIndex < items.length; itemIndex++) {
            ...
            const additionalKeys = (0, get_additional_keys_1.getAdditionalKeys)(additionalData, mode, runExecutionData);
            if (nodeType.description.requestDefaults) {
                for (const key of Object.keys(nodeType.description.requestDefaults)) {
                    ...
                    value = this.getParameterValue(value, itemIndex, runIndex, executeData, { ...additionalKeys, $credentials: credentials, $version: node.typeVersion }, false);
            ...
            for (const property of nodeType.description.properties) {
                ...
                value = this.getParameterValue(value, itemIndex, runIndex, executeData, { ...additionalKeys, $credentials: credentials, $version: node.typeVersion }, false);
                const tempOptions = this.getRequestOptionsFromParameters(itemContext[itemIndex].thisArgs, property, itemIndex, runIndex, '', {
                    ...additionalKeys,
            ...
            if (proxy) {
                ...
                for (const property of proxyProperties) {
                    if (!(property in proxyParsed) ||
                        ...
                        throw new n8n_workflow_1.NodeOperationError(node, 'The proxy is not valid', {
                            runIndex,
                            itemIndex,
            ...
            const allowedDomains = credentials
                ? (0, n8n_workflow_1.getCredentialAllowedDomains)({ node, credentialData: credentials })
                : undefined;
            if (allowedDomains) {
    ...
    getRequestOptionsFromParameters(executeSingleFunctions, nodeProperties, itemIndex, runIndex, path, additionalKeys) {
        ...
        if (nodeProperties.routing) {
            ...
            if (nodeProperties.routing.send) {
                ...
                if (propertyName !== undefined) {
                    propertyName = this.getParameterValue(propertyName, itemIndex, runIndex, executeSingleFunctions.getExecuteData(), { ...additionalKeys, $value: parameterValue }, true);
        ...
        if (nodeProperties.type === 'options') {
            ...
            if (selectedOption.length) {
                const tempOptions = this.getRequestOptionsFromParameters(executeSingleFunctions, selectedOption[0], itemIndex, runIndex, `${basePath}${nodeProperties.name}`, { ...additionalKeys, $value: optionValue, $version: node.typeVersion });
            ...
            for (const propertyOption of nodeProperties.options) {
                if (Object.keys(value).includes(propertyOption.name) &&
                    ...
                    const tempOptions = this.getRequestOptionsFromParameters(executeSingleFunctions, propertyOption, itemIndex, runIndex, `${basePath}${nodeProperties.name}`, { ...additionalKeys, $version: node.typeVersion });
                    this.mergeOptions(returnData, tempOptions);
                }
            }
        }
            ...
            for (const propertyOptions of nodeProperties.options) {
                ...
                for (let i = 0; i < value.length; i++) {
                    for (const option of propertyOptions.values) {
                        const tempOptions = this.getRequestOptionsFromParameters(executeSingleFunctions, option, itemIndex, runIndex, nodeProperties.typeOptions?.multipleValues ? `${loopBasePath}[${i}]` : loopBasePath, {
                            ...(additionalKeys || {}),
                            $index: i,
                            $parent: value[i],
                        });
