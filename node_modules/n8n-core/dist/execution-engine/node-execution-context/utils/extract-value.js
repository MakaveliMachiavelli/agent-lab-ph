...
function findPropertyFromParameterName(parameterName, nodeType, node, nodeParameters) {
    const property = n8n_workflow_1.NodeHelpers.findDisplayedProperty(parameterName, nodeType.description.properties, nodeParameters, node, nodeType.description);
    if (!property) {
        throw new n8n_workflow_1.UserError('Could not find property', { extra: { parameterName } });
    }
    return property;
}
function executeRegexExtractValue(value, regex, flags, parameterName, parameterDisplayName) {
    const extracted = n8n_workflow_1.safeRegex.exec(regex, value, flags);
    if (!extracted) {
    ...
    if (extracted.length < 2 || extracted.length > 2) {
        throw new n8n_workflow_1.WorkflowOperationError(`Property "${parameterName}" has an invalid extractValue regex "${regex}". extractValue expects exactly one group to be returned.`);
    }
    return extracted[1];
}
function regexPatternAndFlags(regex) {
    if (typeof regex === 'string')
        return { pattern: regex, flags: undefined };
...
function extractValueRLC(value, property, parameterName) {
    ...
    if (typeof value.value !== 'string') {
        ...
        throw new n8n_workflow_1.UserError("ERROR: This parameter's value is invalid. Please enter a valid mode.", {
            extra: { parameter: property.displayName, modeProp: modeProp.displayName },
        });
    }
    if (modeProp.extractValue.type !== 'regex') {
        throw new n8n_workflow_1.UnexpectedError('Property with unknown `extractValue`', {
...
function extractValueFilter(value, property, parameterName, itemIndex) {
    ...
    if (property.extractValue?.type) {
        throw new n8n_workflow_1.UserError(`Property "${parameterName}" has an invalid extractValue type. Filter parameters only support extractValue: true`, { extra: { parameter: parameterName } });
    }
...
function extractValueOther(value, property, parameterName) {
    ...
    if (typeof value !== 'string') {
        ...
        n8n_workflow_1.LoggerProxy.error(`Only strings can be passed to extractValue. Parameter "${parameterName}" passed "${typeName}"`);
        throw new n8n_workflow_1.UserError("This parameter's value is invalid", {
