...
function isTool(value) {
    if (value instanceof tools_1.StructuredTool || value instanceof tools_1.Tool)
        return true;
    if (value instanceof ai_tool_types_1.StructuredToolkit)
        return false;
...
function mapResult(result) {
    ...
    if (result === undefined) {
        ...
        throw new n8n_workflow_1.UserError('A connected tool returned an engine request to its parent agent, which is only supported for top-level node execution.', {
            description: 'If you are seeing this from a nested AgentToolV3 sub-agent, update n8n — recent versions resolve sub-agent engine requests inline.',
        });
    }
        ...
        for (let tryIndex = 0; tryIndex < maxTries; tryIndex++) {
            ...
            if (tryIndex !== 0) {
                ...
                if (waitBetweenTries !== 0) {
                    try {
                        await (0, sleep_1.sleep)(waitBetweenTries, abortSignal);
                    }
                    catch (abortError) {
                        return 'Error during node execution: Execution was cancelled';
                    }
    ...
    for (const connectedNode of connectedNodes) {
        ...
        if (!connectedNodeType.supplyData) {
            if (connectedNodeType.description.outputs.includes(n8n_workflow_1.NodeConnectionTypes.AiTool)) {
                ...
                throw new n8n_workflow_1.UnexpectedError('Node does not have a `supplyData` method defined', {
