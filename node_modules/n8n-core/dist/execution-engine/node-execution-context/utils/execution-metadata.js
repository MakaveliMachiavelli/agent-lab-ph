...
function setWorkflowExecutionMetadata(executionData, key, value) {
    ...
    if (key.replace(/[\p{L}\p{N}\p{M}_]/gu, '').length !== 0) {
        throw new invalid_execution_metadata_error_1.InvalidExecutionMetadataError('key', key, `Custom data key can only contain letters, numbers, and underscores (key "${key}")`);
    }
