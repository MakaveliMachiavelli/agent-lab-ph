    ...
    if (binaryData instanceof http_1.IncomingMessage) {
        if (!filePath) {
            try {
                ...
                let urlPathname;
                if (responseUrl) {
                    const parsed = new url_1.URL(responseUrl);
                    parsed.username = '';
                    parsed.password = '';
...
const getBinaryHelperFunctions = ({ executionId, restApiUrl }, workflowId) => ({
    ...
    getBinaryStream,
    getBinaryMetadata,
    binaryToBuffer: backend_network_1.binaryToBuffer,
    binaryToString: backend_network_1.binaryToString,
    ...
    copyBinaryFile: async () => {
        throw new n8n_workflow_1.UserError('`copyBinaryFile` has been removed. Please upgrade this node.');
    },
});
