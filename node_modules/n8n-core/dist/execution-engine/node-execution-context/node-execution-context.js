...
const custom_data_1 = require("./utils/custom-data");
...
class NodeExecutionContext {
    ...
    get customData() {
        if (!this.runExecutionData) {
            throw new n8n_workflow_1.UnexpectedError('Cannot access customData: runExecutionData is not available in this context');
        }
        return (0, custom_data_1.createExecutionCustomData)({
    ...
    getSignedResumeUrl(parameters = {}) {
        ...
        const signature = (0, signature_helpers_1.generateUrlSignature)(urlForSigning, this.instanceSettings.hmacSignatureSecret);
        baseURL.searchParams.set(constants_1.WAITING_TOKEN_QUERY_PARAM, signature);
    ...
    assertCredentialUsableByNode(type, nodeType) {
        if (!this.additionalData.credentialsHelper.isCredentialUsableByNode(type, nodeType)) {
            throw new n8n_workflow_1.NodeOperationError(this.node, `Credential type "${type}" is restricted to specific nodes`, { level: 'warning' });
        }
    }
    async _getCredentials(type, executeData, connectionInputData, itemIndex) {
        ...
        if (additionalData.evalLlmMockHandler && !node.credentials?.[type]) {
            ...
            if (!hasOtherCreds) {
                return (await additionalData.credentialsHelper.getDecrypted(additionalData, { id: null, name: type }, type, mode, executeData));
            }
        }
        const nodeType = workflow.nodeTypes.getByNameAndVersion(node.type, node.typeVersion);
        const fullAccess = constants_1.FULL_ACCESS_NODE_TYPES.has(node.type);
    ...
    _getNodeParameter(parameterName, itemIndex, fallbackValue, options) {
        ...
        if (value === undefined) {
            throw new n8n_workflow_1.UserError(`Could not get parameter "${parameterName}"`, {
                extra: { parameterName },
            });
        }
...
__decorate([
    ...
    __metadata("design:type", Object),
    __metadata("design:paramtypes", [])
], NodeExecutionContext.prototype, "customData", null);
__decorate([
    decorators_1.Memoized,
    __metadata("design:type", Object),
    __metadata("design:paramtypes", [])
