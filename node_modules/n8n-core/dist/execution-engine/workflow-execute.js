...
function normalizeUnhandledAxiosError(error, node) {
    if ((0, backend_network_1.isAxiosError)(error)) {
...
class WorkflowExecute {
    ...
    run({ workflow, startNode, destinationNode, pinData, triggerToStartFrom, additionalRunFilterNodes, }) {
        ...
        if (startNode === undefined) {
            throw new n8n_workflow_1.UserError('No node to start the workflow from could be found');
        }
        let runNodeFilter;
        if (destinationNode) {
            runNodeFilter = [
    ...
    runPartialWorkflow2(workflow, runData, pinData = {}, dirtyNodeNames = [], destinationNode, agentRequest) {
        ...
        if (trigger === undefined) {
            ...
            if (!startNode) {
                throw new n8n_workflow_1.UserError("Connect a trigger and make sure it's enabled to run this node");
            }
            trigger = startNode;
        }
        const filteredGraph = (0, partial_execution_utils_1.filterDisabledNodes)(graph);
        if (destination.disabled) {
            throw new n8n_workflow_1.UserError('Cannot execute a disabled node');
        }
        graph = (0, partial_execution_utils_1.findSubgraph)({ graph: filteredGraph, destination, trigger });
    ...
    async executeNode(workflow, node, nodeType, customOperation, additionalData, mode, runExecutionData, runIndex, connectionInputData, inputData, executionData, abortSignal, subNodeExecutionResults) {
        ...
        try {
            if (customOperation) {
                ...
                data = (0, n8n_workflow_1.isNodeClassInstance)(nodeType)
                    ...
                    if (closingErrors.length > 0) {
                        closingError =
                            closingErrors[0] instanceof Error
                                ? closingErrors[0]
                                : new n8n_workflow_1.UnexpectedError("Error on execution node's close function(s)", {
    ...
    async executeTriggerNode(workflow, node, additionalData, mode, inputData, abortSignal) {
        if (mode === 'manual') {
            const triggerResponse = await di_1.Container.get(triggers_and_pollers_1.TriggersAndPollers).runTriggerFunction(workflow, node, NodeExecuteFunctions.getExecuteTriggerFunctions, additionalData, mode, 'manual');
    ...
    async runNode(workflow, executionData, runExecutionData, runIndex, additionalData, mode, abortSignal, subNodeExecutionResults) {
        ...
        if (nodeType.supplyData) {
            throw new n8n_workflow_1.UnexpectedError(`The node "${node.type}" has a "supplyData" method but no "execute" method.`);
        }
    ...
    checkForWorkflowIssues(workflow) {
        ...
        if (workflowIssues !== null) {
            throw new workflow_has_issues_error_1.WorkflowHasIssuesError(workflowIssues, workflow.nodes);
        }
    }
    setupExecution() {
        this.status = 'running';
        ...
        return new p_cancelable_1.default(async (resolve, _reject, onCancel) => {
            ...
            const returnPromise = (async () => {
                        ...
                        try {
                            ...
                            if (pinData && !executionNode.disabled && pinData[executionNode.name] !== undefined) {
                                ...
                                while (nodeFailed && tryIndex !== maxTries - 1) {
                                    await (0, sleep_1.sleep)(waitBetweenTries);
                            ...
                            for (outputIndex in workflow.connectionsBySourceNode[executionNode.name].main) {
                                ...
                                for (connectionData of workflow.connectionsBySourceNode[executionNode.name].main[outputIndex] ?? []) {
                                    if (!Object.hasOwn(workflow.nodes, connectionData.node)) {
                                        throw new n8n_workflow_1.UnexpectedError('Destination node not found', {
                                            extra: {
                ...
                try {
                    ...
                    di_1.Container.get(error_reporter_1.ErrorReporter).error(error);
                }
            });
