...
const validate_node_description_1 = require("./validate-node-description");
function toJSON() {
    return {
        ...this,
...
class DirectoryLoader {
    ...
    loadClass(sourcePath) {
        ...
        try {
            ...
            throw error instanceof TypeError
                ? new n8n_workflow_1.UserError('Class could not be found. Please check if the class is named correctly.', {
                    extra: { className },
                })
                : error;
        }
    ...
    loadNodeFromFile(filePath, packageVersion) {
        ...
        if ('nodeVersions' in tempNode) {
            ...
            if (currentVersionNode.hasOwnProperty('executeSingle')) {
                throw new n8n_workflow_1.UserError('"executeSingle" has been removed. Please update the code of this node to use "execute" instead.', { extra: { nodeType } });
            }
        }
        else {
        ...
        this.getVersionedNodeTypeAll(tempNode).forEach(({ description }) => {
            (0, validate_node_description_1.validateNodeDescription)(description);
    ...
    getIconPath(icon, filePath) {
        ...
        if (!(0, backend_common_1.isContainedWithin)(this.directory, absoluteIconPath)) {
    ...
    unloadAll() {
        ...
        const filesToUnload = Object.keys(require.cache).filter((filePath) => rootsToUnload.some((root) => filePath.startsWith(root)));
        filesToUnload.forEach((filePath) => {
            delete require.cache[filePath];
        });
    }
    getSymlinkedPackageRoots() {
        const nodeModulesDir = path.join(this.directory, 'node_modules');
        let entries;
        try {
            entries = (0, node_fs_1.readdirSync)(nodeModulesDir, { withFileTypes: true });
        }
        catch {
            return [];
        }
    ...
    resolveSymlinkedRoots(scopeDir) {
        let entries;
        try {
            entries = (0, node_fs_1.readdirSync)(scopeDir, { withFileTypes: true });
        }
        catch {
            return [];
        }
    ...
    pushResolvedRoot(roots, entryPath) {
        try {
            const realPath = (0, node_fs_1.realpathSync)(entryPath);
