...
class CredentialDataError extends n8n_workflow_1.OperationalError {
...
class Credentials extends n8n_workflow_1.ICredentials {
    ...
    async updateData(toUpdate, toDelete = []) {
        const updatedData = { ...(await this.getData()), ...toUpdate };
    ...
    async getData() {
        ...
        let parsed;
        try {
            parsed = (0, n8n_workflow_1.jsonParse)(decryptedData);
        }
        catch (cause) {
    ...
    getDataToSave() {
        if (this.data === undefined) {
            throw new n8n_workflow_1.UnexpectedError('No credentials were set to save.');
        }
        return {
            id: this.id,
            name: this.name,
