...
let InstanceSettings = class InstanceSettings {
    ...
    async initialize(repo, { canSeed = true } = {}) {
        await this.initSecret(repo, 'instance.id', process.env.N8N_INSTANCE_ID, canSeed, () => this.instanceId, (v) => {
            this.instanceId = v;
        });
        await this.initSecret(repo, 'signing.hmac', process.env.N8N_HMAC_SIGNATURE_SECRET, canSeed, () => this.hmacSignatureSecret, (v) => {
            this.hmacSignatureSecret = v;
        });
    }
    async initSecret(repo, type, envValue, canSeed, get, set) {
        if (envValue) {
            set(envValue);
            return;
        }
    ...
    ensureSettingsFilePermissions() {
        ...
        if (this.isWindows())
            return;
        const permissionsResult = (0, result_1.toResult)(() => {
        ...
        const chmodResult = (0, result_1.toResult)(() => (0, node_fs_1.chmodSync)(this.settingsFile, 0o600));
        if (!chmodResult.ok) {
...
exports.InstanceSettings = InstanceSettings = __decorate([
    (0, di_1.Service)(),
    __metadata("design:paramtypes", [config_1.InstanceSettingsConfig, backend_common_1.Logger])
], InstanceSettings);
