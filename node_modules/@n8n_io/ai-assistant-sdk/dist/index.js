"use strict";
var __defProp = Object.defineProperty;
var __getOwnPropDesc = Object.getOwnPropertyDescriptor;
var __getOwnPropNames = Object.getOwnPropertyNames;
var __hasOwnProp = Object.prototype.hasOwnProperty;
var __export = (target, all) => {
  for (var name in all)
    __defProp(target, name, { get: all[name], enumerable: true });
};
var __copyProps = (to, from, except, desc) => {
  if (from && typeof from === "object" || typeof from === "function") {
    for (let key of __getOwnPropNames(from))
      if (!__hasOwnProp.call(to, key) && key !== except)
        __defProp(to, key, { get: () => from[key], enumerable: !(desc = __getOwnPropDesc(from, key)) || desc.enumerable });
  }
  return to;
};
var __toCommonJS = (mod) => __copyProps(__defProp({}, "__esModule", { value: true }), mod);

// src/index.ts
var src_exports = {};
__export(src_exports, {
  APIResponseError: () => APIResponseError,
  AiAssistantClient: () => AiAssistantClient,
  AuthError: () => AuthError
});
module.exports = __toCommonJS(src_exports);

// package.json
var package_default = {
  name: "@n8n_io/ai-assistant-sdk",
  version: "1.26.0",
  description: "n8n AI assistant SDK",
  author: "",
  private: false,
  license: "LicenseRef-n8n-enterprise",
  engines: {
    node: ">=20.15",
    pnpm: ">=8.14"
  },
  main: "./dist/index.js",
  types: "./dist/index.d.ts",
  module: "./dist/index.mjs",
  packageManager: "pnpm@10.18.0",
  scripts: {
    typecheck: "tsc --noEmit",
    build: "tsup",
    format: "prettier --write . --ignore-path ../.prettierignore",
    lint: "eslint ./src --quiet -c ../.eslintrc.js",
    start: "node ./dist/index.js",
    test: "node --import tsx --test test/**.test.ts",
    "test:dev": "node --import tsx --test --watch test/**.test.ts",
    watch: "tsup src/index.ts --watch"
  },
  devDependencies: {
    "@faker-js/faker": "^8.4.1",
    "@types/node": "^22.4.0",
    eslint: "^9.6.0",
    "eslint-config-prettier": "^9.1.0",
    "eslint-plugin-prettier": "^5.1.3",
    nock: "^14.0.0-beta.9",
    prettier: "^3.3.2",
    tsup: "^8.1.0",
    tsx: "^4.17.0",
    typescript: "^5.5.3"
  }
};

// src/error.ts
var ApplicationError = class extends Error {
  level;
  name;
  statusCode;
  constructor(message, { level, name, statusCode } = {}) {
    super(message);
    this.level = level ?? "error";
    this.name = name ?? "AIServiceSDKError";
    this.statusCode = statusCode;
  }
};
var APIResponseError = class extends ApplicationError {
  /**
   * Machine-readable error class from the response body's `error.type`, when present
   * (e.g. `'quota_exhausted'`). Lets consumers branch on the code instead of the message.
   */
  errorCode;
  constructor(message, statusCode, errorCode) {
    const isClientError = statusCode !== void 0 && statusCode >= 400 && statusCode < 500;
    super(message, {
      name: "AIServiceAPIResponseError",
      level: isClientError ? "warning" : "error",
      statusCode
    });
    this.errorCode = errorCode;
  }
};
var AuthError = class extends ApplicationError {
  constructor(message) {
    super(message, { name: "AuthError", level: "warning" });
  }
};

// src/index.ts
var isObjectWithErrorMessage = (data) => {
  return typeof data === "object" && data !== null && "message" in data && typeof data?.message === "string";
};
var isObjectWithNestedErrorMessage = (data) => {
  return typeof data === "object" && data !== null && "error" in data && isObjectWithErrorMessage(data.error);
};
var extractApiErrorMessage = (data, fallback) => {
  if (isObjectWithNestedErrorMessage(data)) {
    return data.error.message;
  }
  if (isObjectWithErrorMessage(data)) {
    return data.message;
  }
  return fallback;
};
var extractApiError = (data, fallback) => {
  const message = extractApiErrorMessage(data, fallback);
  if (typeof data !== "object" || data === null) {
    return { message };
  }
  const record = data;
  const nestedType = typeof record.error === "object" && record.error !== null ? record.error.type : void 0;
  const code = typeof record.code === "string" ? record.code : typeof nestedType === "string" ? nestedType : void 0;
  return { message, code };
};
var DEFAULT_SERVICE_BASE_URL = "https://ai-assistant.n8n.io";
var AiAssistantClient = class {
  licenseCert;
  consumerId;
  n8nVersion;
  instanceId;
  baseUrl = DEFAULT_SERVICE_BASE_URL;
  logLevel = "info";
  activeToken;
  /**
   * Create a client for the AI service.
   * @param licenseCert - The license certificate. You can get it from the n8n.
   * @param consumerId - The consumer ID.
   * @param n8nVersion - The n8n version.
   * @param instanceId - The n8n instance ID.
   * @param baseUrl - The base URL of the AI service API.
   * @returns {RequestHandler}
   */
  constructor({
    licenseCert,
    consumerId,
    n8nVersion,
    instanceId,
    baseUrl,
    logLevel
  }) {
    this.licenseCert = licenseCert;
    this.consumerId = consumerId;
    this.n8nVersion = n8nVersion;
    this.instanceId = instanceId;
    this.baseUrl = baseUrl ?? this.baseUrl;
    this.logLevel = logLevel ?? this.logLevel;
    this.debug("Initializing AI Assistant Service Client", {
      baseUrl: this.baseUrl,
      consumerId: this.consumerId,
      n8nVersion: this.n8nVersion,
      licenseCert: this.licenseCert.substring(0, 5)
    });
  }
  async chat(payload, user) {
    return await this.postRequest("/v1/chat", payload, user);
  }
  async applySuggestion(payload, user) {
    const response = await this.postRequest("/v1/chat/apply-suggestion", payload, user);
    const data = await response.json();
    if (isValidApplySuggestionResponse(data)) {
      return data;
    }
    throw new APIResponseError("Invalid response from assistant service");
  }
  async askAi(payload, user) {
    {
      const response = await this.postRequest("/v1/ask-ai", payload, user);
      const data = await response.json();
      if (isValidAskAiResponse(data)) {
        return data;
      }
      throw new APIResponseError("Invalid response from assistant service");
    }
  }
  async generateAiCreditsCredentials(user) {
    {
      const url = `${this.baseUrl}/v1/ai-credits/credentials`;
      try {
        const response = await fetch(url, {
          headers: this.getHeaders(user),
          method: "POST",
          body: JSON.stringify({
            licenseCert: this.licenseCert
          })
        });
        if (!response.ok) {
          const errorData = await response.json().catch(() => null);
          const errorMessage = extractApiErrorMessage(
            errorData,
            `Failed to generate AI credits credentials: ${response.statusText}`
          );
          throw new APIResponseError(errorMessage, response.status);
        }
        const data = await response.json();
        if (isValidAiCreditsResponse(data)) {
          return data;
        }
        throw new APIResponseError("Invalid response from assistant service");
      } catch (error) {
        if (error instanceof APIResponseError) {
          throw error;
        }
        if (isObjectWithErrorMessage(error)) {
          throw new APIResponseError(error.message);
        } else {
          throw new APIResponseError("unknown error");
        }
      }
    }
  }
  getApiProxyBaseUrl() {
    return this.baseUrl + "/v1/api-proxy";
  }
  getSandboxProxyBaseUrl() {
    return this.baseUrl + "/v1/sandbox-proxy";
  }
  async getSandboxProxyConfig() {
    const url = `${this.baseUrl}/v1/sandbox-proxy/config`;
    try {
      const response = await fetch(url);
      if (!response.ok) {
        const errorData = await response.json().catch(() => null);
        const errorMessage = isObjectWithErrorMessage(errorData) ? errorData.message : `Failed to get sandbox proxy config: ${response.statusText}`;
        throw new APIResponseError(errorMessage, response.status);
      }
      const data = await response.json();
      if (typeof data === "object" && data && "image" in data && typeof data.image === "string") {
        return data;
      }
      throw new APIResponseError("Invalid response from sandbox proxy config endpoint");
    } catch (error) {
      if (error instanceof APIResponseError) {
        throw error;
      }
      if (isObjectWithErrorMessage(error)) {
        throw new APIResponseError(error.message);
      }
      throw new APIResponseError("unknown error");
    }
  }
  /**
   * Update the license certificate and clear the active token.
   * This should be called when the license is renewed or changed.
   * @param licenseCert - The new license certificate.
   */
  updateLicenseCert(licenseCert) {
    this.licenseCert = licenseCert;
    this.activeToken = void 0;
  }
  async getBuilderApiProxyToken(user, options) {
    const url = `${this.baseUrl}/v1/builder/api-proxy-token`;
    return await this.request(
      url,
      {
        headers: this.getHeaders(user),
        method: "POST",
        body: JSON.stringify({
          licenseCert: this.licenseCert,
          userMessageId: options?.userMessageId
        })
      },
      isValidBuilderApiProxyTokenResponse,
      "Failed to get builder API proxy token"
    );
  }
  async markBuilderSuccess(user, headers) {
    const url = `${this.baseUrl}/v1/builder/success`;
    return await this.request(
      url,
      {
        headers: {
          ...this.getHeaders(user),
          "x-authorization": headers.Authorization
        },
        body: JSON.stringify({
          licenseCert: this.licenseCert
        }),
        method: "POST"
      },
      isValidBuilderInstanceCreditsResponse,
      "Failed to mark builder success"
    );
  }
  async getBuilderInstanceCredits(user) {
    const url = `${this.baseUrl}/v1/builder/usage`;
    return await this.request(
      url,
      {
        headers: this.getHeaders(user),
        method: "POST",
        body: JSON.stringify({
          licenseCert: this.licenseCert
        })
      },
      isValidBuilderInstanceCreditsResponse,
      "Failed to get builder usage"
    );
  }
  /**
   * Request an api-proxy token for the Instance AI credit pool. Mirrors
   * {@link getBuilderApiProxyToken} but the issued token bills against the IA pool.
   */
  async getInstanceAiApiProxyToken(user, options) {
    const url = `${this.baseUrl}/v1/instance-ai/api-proxy-token`;
    return await this.request(
      url,
      {
        headers: this.getHeaders(user),
        method: "POST",
        body: JSON.stringify({
          licenseCert: this.licenseCert,
          userMessageId: options?.userMessageId
        })
      },
      isValidInstanceAiApiProxyTokenResponse,
      "Failed to get instance AI API proxy token"
    );
  }
  /**
   * Claim decimal credits for Instance AI token usage from a completed/stopped/errored run.
   * Idempotent per `dedupeId` (the service dedupes replays).
   */
  async markInstanceAiTokenUsage(user, headers, payload) {
    const url = `${this.baseUrl}/v1/instance-ai/claim-tokens`;
    return await this.request(
      url,
      {
        headers: {
          ...this.getHeaders(user),
          "x-authorization": headers.Authorization
        },
        body: JSON.stringify({
          licenseCert: this.licenseCert,
          dedupeId: payload.dedupeId,
          usage: payload.usage,
          threadId: payload.threadId
        }),
        method: "POST"
      },
      isValidInstanceAiClaimTokensResponse,
      "Failed to claim instance AI token usage"
    );
  }
  async getInstanceAiCredits(user) {
    const url = `${this.baseUrl}/v1/instance-ai/usage`;
    return await this.request(
      url,
      {
        headers: this.getHeaders(user),
        method: "POST",
        body: JSON.stringify({
          licenseCert: this.licenseCert
        })
      },
      isValidInstanceAiCreditsResponse,
      "Failed to get instance AI usage"
    );
  }
  /**
   * Forfeit the remaining Instance AI quota because the instance has activated (INS-1082).
   * What counts as "activated" is decided by the caller; the service only applies the outcome,
   * and refuses it outside a trial plan. Idempotent, so this can be re-asserted freely.
   */
  async lockInstanceAiQuota(user, options = {}) {
    const url = `${this.baseUrl}/v1/instance-ai/lock-quota`;
    return await this.request(
      url,
      {
        headers: this.getHeaders(user),
        method: "POST",
        body: JSON.stringify({
          licenseCert: this.licenseCert,
          ...options.activatedAt !== void 0 ? { activatedAt: options.activatedAt } : {}
        })
      },
      isValidInstanceAiLockQuotaResponse,
      "Failed to lock instance AI quota"
    );
  }
  getHeadersWithAuthToken(user) {
    return {
      authorization: `Bearer ${this.activeToken}`,
      ...this.getHeaders(user)
    };
  }
  getHeaders(user) {
    return {
      "Content-Type": "application/json",
      "x-consumer-id": this.consumerId,
      "x-user-id": user.id,
      "x-sdk-version": package_default.version,
      "x-n8n-version": this.n8nVersion,
      "x-instance-id": this.instanceId
    };
  }
  /**
   * Shared fetch + error handling for JSON endpoints: throws APIResponseError on a non-ok
   * status (using the body's `message` when present, else `${errorPrefix}: ${statusText}`)
   * or when the parsed body fails `isValid`, and normalizes thrown errors to APIResponseError.
   */
  async request(url, init, isValid, errorPrefix) {
    try {
      const response = await fetch(url, init);
      if (!response.ok) {
        const errorData = await response.json();
        const { message, code } = extractApiError(
          errorData,
          `${errorPrefix}: ${response.statusText}`
        );
        throw new APIResponseError(message, response.status, code);
      }
      const data = await response.json();
      if (!isValid(data)) {
        throw new APIResponseError("Invalid response from assistant service");
      }
      return data;
    } catch (error) {
      if (error instanceof APIResponseError) {
        throw error;
      }
      if (isObjectWithErrorMessage(error)) {
        throw new APIResponseError(error.message);
      }
      throw new APIResponseError("unknown error");
    }
  }
  async refreshAuthToken() {
    const response = await fetch(`${this.baseUrl}/auth/token`, {
      method: "POST",
      body: JSON.stringify({ licenseCert: this.licenseCert }),
      headers: { "Content-Type": "application/json" }
    });
    const data = await response.json().catch(() => null);
    if (typeof data === "object" && data && "accessToken" in data && data.accessToken && typeof data.accessToken === "string") {
      this.activeToken = data.accessToken;
      return;
    }
    const errorMessage = isObjectWithErrorMessage(data) ? data.message : "Unknown error";
    throw new AuthError(`Could not retrieve access token: ${errorMessage}`);
  }
  async postRequest(endpoint, payload, user) {
    if (!this.activeToken) {
      await this.refreshAuthToken();
    }
    if (!this.activeToken) {
      throw new AuthError("No token to call assistant service");
    }
    const url = `${this.baseUrl}${endpoint}`;
    let response = await fetch(url, {
      headers: this.getHeadersWithAuthToken(user),
      method: "POST",
      body: JSON.stringify(payload)
    });
    if (!response.ok && response.status === 401) {
      await this.refreshAuthToken();
      response = await fetch(url, {
        headers: this.getHeadersWithAuthToken(user),
        method: "POST",
        body: JSON.stringify(payload)
      });
    }
    if (!response.ok) {
      const error = await response.json().catch(() => null);
      this.debug(`API Error ${JSON.stringify(error)}`);
      const message = isObjectWithErrorMessage(error) ? error.message : response.statusText;
      throw new APIResponseError(message, response.status);
    }
    return response;
  }
  debug(message, debugInfo) {
    if (this.logLevel === "debug") {
      console.debug(formatLog(message), formatLog(JSON.stringify(debugInfo)));
    }
  }
};
function formatLog(message) {
  return `[ai-assistant-sdk] ${message}`;
}
function isValidApplySuggestionResponse(response) {
  return typeof response === "object" && !!response && "parameters" in response && "sessionId" in response;
}
function isValidAskAiResponse(response) {
  return typeof response === "object" && !!response && "code" in response;
}
function isValidAiCreditsResponse(response) {
  return typeof response === "object" && !!response && "apiKey" in response && "url" in response;
}
function isValidBuilderApiProxyTokenResponse(response) {
  return typeof response === "object" && !!response && "accessToken" in response && "tokenType" in response && typeof response.accessToken === "string" && typeof response.tokenType === "string";
}
function isValidBuilderInstanceCreditsResponse(response) {
  return typeof response === "object" && !!response && "creditsQuota" in response && "creditsClaimed" in response && typeof response.creditsQuota === "number" && typeof response.creditsClaimed === "number";
}
function isValidInstanceAiApiProxyTokenResponse(response) {
  return typeof response === "object" && !!response && "accessToken" in response && "tokenType" in response && typeof response.accessToken === "string" && typeof response.tokenType === "string";
}
function isValidInstanceAiClaimTokensResponse(response) {
  return typeof response === "object" && !!response && "delta" in response && "creditsClaimed" in response && "creditsQuota" in response && typeof response.delta === "number" && typeof response.creditsClaimed === "number" && typeof response.creditsQuota === "number";
}
function isValidInstanceAiCreditsResponse(response) {
  return typeof response === "object" && !!response && "creditsQuota" in response && "creditsClaimed" in response && typeof response.creditsQuota === "number" && typeof response.creditsClaimed === "number" && // Accept-if-absent: a service predating the activation lock omits it entirely.
  (!("quotaLocked" in response) || typeof response.quotaLocked === "boolean");
}
function isValidInstanceAiLockQuotaResponse(response) {
  return typeof response === "object" && !!response && "creditsQuota" in response && "creditsClaimed" in response && "quotaLocked" in response && typeof response.creditsQuota === "number" && typeof response.creditsClaimed === "number" && typeof response.quotaLocked === "boolean";
}
// Annotate the CommonJS export names for ESM import in node:
0 && (module.exports = {
  APIResponseError,
  AiAssistantClient,
  AuthError
});
